...
- (4) Add operation to `crea::protocol::operation`
...
- (4a) The file `operations.hpp` defines the `crea::protocol::operation`
...
[implementation](https://github.com/creativechain/crea/blob/a0a69b10189d93a9be4da7e7fd9d5358af956e34/libraries/chain/database.cpp#L364)
of `get_account(name)`.
...
- (9a) Add `smt_token_object_type` to `enum object_type` in `crea_objects.hpp` and add
to `FC_REFLECT_ENUM` bubble list at the bottom of that file
- (9b) Declare (but do not define) `class smt_token_object;` in `crea_objects.hpp`
- (9c) Define `typedef oid< smt_token_object > smt_token_id_type` in `crea_objects.hpp`
...
- (9e) All SMT objects are consensus, and therefore should exist in `crea::chain` namespace
...
defined by an `enum` in `crea_object_types.hpp`, any new objects must be added here.  In SQL
...
alias `typedef oid< smt_token_object > smt_id_type` is added to `crea_object_types.hpp`.
...
is designed as a reusable library, not tightly coupled to Crea, it contains no reference
to any `crea` namespaces.  So the name `by_id` must refer to `chainbase::by_id`.  If you
define a `struct by_id;` in the `crea::chain` namespace, the result will be that every
...
- (9m) All indexes used in Crea must be `ordered_unique`.  In theory, hashed or non-unique
