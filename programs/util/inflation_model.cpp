...
rvec shows total number of CREA satoshis created since genesis for:
...
*/
   ...
   {
      uint32_t vesting_factor = (block_num < CREA_START_VESTING_BLOCK) ? 0 : 9;

      share_type curate_reward   = calc_percent_reward_per_block< CREA_CURATE_APR_PERCENT >( current_supply );
      reward_delta[ CURATE_OFF ] = std::max( curate_reward, CREA_MIN_CURATE_REWARD.amount );
      reward_delta[ VCURATE_OFF ] = reward_delta[ CURATE_OFF ] * vesting_factor;

      share_type content_reward  = calc_percent_reward_per_block< CREA_CONTENT_APR_PERCENT >( current_supply );
      reward_delta[ CONTENT_OFF ] = std::max( content_reward, CREA_MIN_CONTENT_REWARD.amount );
      reward_delta[ VCONTENT_OFF ] = reward_delta[ CONTENT_OFF ] * vesting_factor;

      share_type producer_reward = calc_percent_reward_per_block< CREA_PRODUCER_APR_PERCENT >( current_supply );
      ...
      if( pow_deficit > 0 )
      {
         pow_reward = calc_percent_reward_per_round< CREA_POW_APR_PERCENT >( current_supply );
         pow_reward = std::max( pow_reward, CREA_MIN_POW_REWARD.amount );
         if( block_num < CREA_START_MINER_VOTING_BLOCK )
            pow_reward *= CREA_MAX_WITNESSES;
         --pow_deficit;
      }
      ...
      {
         liquidity_reward = calc_percent_reward_per_hour< CREA_LIQUIDITY_APR_PERCENT >( current_supply );
         liquidity_reward = std::max( liquidity_reward, CREA_MIN_LIQUIDITY_REWARD.amount );
      }
