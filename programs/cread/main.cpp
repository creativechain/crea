...
{
  static string v_str =
    "  \"version\" : { \"crea_blockchain_hard_fork\" : \""   + fc::string( CREA_BLOCKCHAIN_VERSION )            + "\", " +
    "\"crea_git_revision\" : \""                             + fc::string( crea::utilities::git_revision_sha )  + "\" }";

  return v_str;
}

void info(const crea::protocol::chain_id_type& chainId)
  {
#ifdef IS_TEST_NET
  std::cerr << "------------------------------------------------------\n\n";
  ...
  std::cerr << "cread git_revision: \"" + fc::string(crea::utilities::git_revision_sha) + "\"\n\n";
  }
...
int main( int argc, char** argv )
{
  try
  {
    ...
    const auto& chainPlugin = theApp.get_plugin<crea::plugins::chain::chain_plugin>();
