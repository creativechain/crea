...
#include <crea/plugins/state_snapshot/state_snapshot_plugin.hpp>
...
class snapshot_dump_supplement_helper final : public crea::plugins::chain::snapshot_dump_helper
{
public:
  ...
  virtual void store_external_data_info(const crea::chain::abstract_plugin& plugin, const fc::path& storage_path) override
  {
...
class snapshot_load_supplement_helper final : public crea::plugins::chain::snapshot_load_helper
{
public:
  explicit snapshot_load_supplement_helper(const plugin_external_data_index& idx) : ext_data_idx(idx) {}

  virtual bool load_external_data_info(const crea::chain::abstract_plugin& plugin, fc::path* storage_path) override
  {
...
namespace crea { namespace plugins { namespace state_snapshot {
...
class state_snapshot_plugin::impl final : protected chain::state_snapshot_provider
  {
  using database = crea::chain::database;
  ...
  public:
    impl(state_snapshot_plugin& self, const bpo::variables_map& options) :
      ...
      appbase::app().get_plugin<crea::plugins::chain::chain_plugin>().register_snapshot_provider(*this);
    ...
    void load_snapshot(const std::string& snapshotName, const crea::chain::open_args& openArgs);

  protected:
    /// chain::state_snapshot_provider implementation:
    virtual void process_explicit_snapshot_requests(const crea::chain::open_args& openArgs) override;
...
void state_snapshot_plugin::impl::collectOptions(const bpo::variables_map& options)
  ...
  appbase::app().get_plugin< crea::plugins::chain::chain_plugin >().report_state_options(_self.name(), state_opts);
  }
...
void state_snapshot_plugin::impl::load_snapshot_external_data(const plugin_external_data_index& idx)
  ...
  crea::chain::load_snapshot_supplement_notification notification(load_helper);
...
void state_snapshot_plugin::impl::prepare_snapshot(const std::string& snapshotName)
  ...
  crea::chain::prepare_snapshot_supplement_notification notification(external_data_storage_base_path, dump_helper);
...
void state_snapshot_plugin::impl::load_snapshot(const std::string& snapshotName, const crea::chain::open_args& openArgs)
  {
  bfs::path actualStoragePath = _storagePath / snapshotName;
  actualStoragePath = actualStoragePath.normalize();
...
void state_snapshot_plugin::impl::process_explicit_snapshot_requests(const crea::chain::open_args& openArgs)
  {
  if(_do_immediate_load)
