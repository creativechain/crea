...
#define BLOCK_PRODUCING_LAG_TIME (10000)
      ...
      case operation::tag< custom_binary_operation >::value:
        if( _db.is_producing() )
        {
          ...
          for( const account_name_type& account : impacted )
          {
            ...
            {
              CREA_ASSERT( coo->count < WITNESS_CUSTOM_OP_BLOCK_LIMIT, plugin_exception,
...
{ try {
  ...
  CREA_ADD_PLUGIN_INDEX(my->_db, witness_custom_op_index);
