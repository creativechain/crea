   ...
   rc_plugin_skip_flags skip )
{
   ...
   db.modify( rc_account, [&]( rc_account_object& rca )
   {
      ...
      if( (!skip.skip_reject_not_enough_rc) && db.has_hardfork( CREA_HARDFORK_0_20 ) && db.is_producing() )
      {
         CREA_ASSERT( has_mana, plugin_exception,
            "Account: ${account} needs ${rc_needed} RC. Please wait to transact, or power up CREA.",
            ("account", account_name)
            ("rc_needed", rc)
            );
      }

      if( (!has_mana) && ( skip.skip_negative_rc_balance || !db.has_hardfork( CREA_HARDFORK_0_20 ) ) )
         return;
...
struct post_apply_operation_visitor
{
   ...
   void operator()( const hardfork_operation& op )const
   {
      ...
      if( op.hardfork_id == CREA_HARDFORK_0_20 )
      {
         ...
         _db.modify( _db.get< rc_pool_object, by_id >( rc_pool_object::id_type() ), [&]( rc_pool_object& p )
         {
            ...
            {
               p.pool_array[ i ] =
                  ( ( uint128_t( params.resource_param_array[ i ].resource_dynamics_params.pool_eq ) * 90 * CREA_1_PERCENT )
                  / CREA_100_PERCENT ).to_uint64();
            }
   ...
   void operator()( const clear_null_account_balance_operation& op )const
   {
      _mod_accounts.push_back( CREA_NULL_ACCOUNT );
   }
