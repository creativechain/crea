...
struct pre_operation_visitor
{
  ...
  void operator()( const delete_comment_operation& op )const
  {
    try
    {
      ...
      if( comment == nullptr || !comment->is_root() )
        return;
...
struct post_operation_visitor
{
  ...
  void operator()( const comment_operation& op )const
  {
    try
    {
      ...
      auto& db = _plugin._db;
      const auto& c = db.get_comment( op.author, op.permlink );
      const auto* cc = db.find_comment_cashout( c );

      if( cc == nullptr || cc->get_creation_time() != db.head_block_time() )
        return;
...
void follow_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
  try
  {
    ...
    CREA_ADD_PLUGIN_INDEX(my->_db, follow_index);
    CREA_ADD_PLUGIN_INDEX(my->_db, feed_index);
    CREA_ADD_PLUGIN_INDEX(my->_db, blog_index);
    CREA_ADD_PLUGIN_INDEX(my->_db, reputation_index);
    CREA_ADD_PLUGIN_INDEX(my->_db, follow_count_index);
    CREA_ADD_PLUGIN_INDEX(my->_db, blog_author_stats_index);
