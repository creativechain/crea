#include <crea/plugins/comment_cashout_logging/comment_cashout_logging_plugin.hpp>
...
{
   public:
      comment_cashout_logging_plugin_impl(const std::string &path) : _db(appbase::app().get_plugin<crea::plugins::chain::chain_plugin>().db()),
        _log_dir_path(path) 
      {
      }
...
struct operation_visitor
{
  ...
  void operator()(const claim_reward_balance_operation& op) const
  {
    _log_file << "claim_reward_balance_operation" << ";"
       ...
       << asset_to_string(op.reward_crea) << ";"
       << asset_to_string(op.reward_cbd) << ";"
  ...
  void operator()(const author_reward_operation &op) const
  {
    _log_file << "author_reward_operation" << ";" 
      ...
      << op.permlink << ";" 
      << asset_to_string(op.cbd_payout) << ";"
      << asset_to_string(op.crea_payout) << ";"
