...
struct debug_generate_blocks_args
{
   ...
   uint32_t                                  count = 0;
   uint32_t                                  skip = crea::chain::database::skip_nothing;
...
class debug_node_plugin : public plugin< debug_node_plugin >
{
   public:
      ...
      void debug_update( Lambda&& callback, uint32_t skip = crea::chain::database::skip_nothing )
      {
   ...
   private:
      void on_post_apply_block( const crea::chain::block_notification& note );
...
FC_REFLECT( crea::plugins::debug_node::debug_generate_blocks_args,
            (debug_key)
            (count)
            (skip)
            (miss_blocks)
...
FC_REFLECT( crea::plugins::debug_node::debug_generate_blocks_return,
            (blocks)
          )
