...
void smt_setup_emissions_operation::validate()const
{
  smt_admin_operation_validate( *this );
   ...
   FC_ASSERT( schedule_time > CREA_GENESIS_TIME );
...
void smt_setup_operation::validate()const
{
  ...
  FC_ASSERT( max_supply <= CREA_MAX_SHARE_SUPPLY, "Max supply must be less than ${n}", ("n", CREA_MAX_SHARE_SUPPLY) );
  FC_ASSERT( contribution_begin_time > CREA_GENESIS_TIME, "Contribution begin time must be greater than ${t}", ("t", CREA_GENESIS_TIME) );
  ...
  FC_ASSERT( crea_units_soft_cap <= crea_units_hard_cap, "Hive units soft cap must less than or equal to crea units hard cap" );
  FC_ASSERT( crea_units_soft_cap >= SMT_MIN_SOFT_CAP_CREA_UNITS, "Hive units soft cap must be greater than or equal to ${n}", ("n", SMT_MIN_SOFT_CAP_CREA_UNITS) );
  FC_ASSERT( crea_units_hard_cap >= SMT_MIN_HARD_CAP_CREA_UNITS, "Hive units hard cap must be greater than or equal to ${n}", ("n", SMT_MIN_HARD_CAP_CREA_UNITS) );
  FC_ASSERT( crea_units_hard_cap <= CREA_MAX_SHARE_SUPPLY, "Hive units hard cap must be less than or equal to ${n}", ("n", CREA_MAX_SHARE_SUPPLY) );
...
struct smt_set_runtime_parameters_operation_visitor
{
  ...
  void operator()( const smt_param_vote_regeneration_period_seconds_v1& vote_regeneration )const
  {
   ...
   {
      FC_ASSERT( param_rewards.percent_curation_rewards <= CREA_100_PERCENT,
...
void smt_contribute_operation::validate() const
{
  ...
  FC_ASSERT( contribution.symbol == CREA_SYMBOL, "Contributions must be made in CREA" );
