...
asset_symbol_type asset_symbol_type::from_nai_string( const char* p, uint8_t decimal_places )
{
   try
   {
      ...
      FC_ASSERT( std::strlen( p ) == CREA_ASSET_SYMBOL_NAI_STRING_LENGTH - 1, "Incorrect NAI string length" );
...
uint32_t asset_symbol_type::asset_num_from_nai( uint32_t nai, uint8_t decimal_places )
{
   ...
   switch( nai_data_digits )
   {
      ...
      default:
         ...
         return (nai_data_digits << CREA_NAI_SHIFT) | SMT_ASSET_NUM_CONTROL_MASK | decimal_places;
   }
}
...
void asset_symbol_type::validate()const
{
   switch( asset_num )
   {
      ...
      default:
      {
         uint32_t nai_data_digits = (asset_num >> CREA_NAI_SHIFT);
...
namespace fc {
   ...
   void from_variant( const fc::variant& var, crea::protocol::asset& vo )
   {
      try
      {
         ...
         vo.symbol = crea::protocol::asset_symbol_type::from_nai_string( v_object[ ASSET_NAI_KEY ].as< std::string >().c_str(), v_object[ ASSET_PRECISION_KEY ].as< uint8_t >() );
      } FC_CAPTURE_AND_RETHROW()
   }
}
