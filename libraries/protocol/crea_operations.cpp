...
namespace crea { namespace protocol {
  ...
  void validate_auth_size( const authority& a )
  {
    ...
    FC_ASSERT( size <= CREA_MAX_AUTHORITY_MEMBERSHIP,
      "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", CREA_MAX_AUTHORITY_MEMBERSHIP)("n", size) );
  }
  ...
  {
    FC_ASSERT( title.size() < CREA_COMMENT_TITLE_LIMIT,
      "Title size limit exceeded. Max: ${max} Current: ${n}", ("max", CREA_COMMENT_TITLE_LIMIT - 1)("n", title.size()) );
  ...
  void comment_payout_beneficiaries::validate()const
  {
    ...
    FC_ASSERT( beneficiaries.size() < CREA_BENEFICIARY_LIMIT,
      "Cannot specify more than ${max} beneficiaries.", ("max", CREA_BENEFICIARY_LIMIT - 1) ); // Require size serializtion fits in one byte.

    validate_account_name( beneficiaries[0].account );
    FC_ASSERT( beneficiaries[0].weight <= CREA_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
    sum += beneficiaries[0].weight;
    FC_ASSERT( sum <= CREA_100_PERCENT, "Cannot allocate more than 100% of rewards to a comment" ); // Have to check incrementally to avoid overflow

    for( size_t i = 1; i < beneficiaries.size(); i++ )
    {
      validate_account_name( beneficiaries[i].account );
      FC_ASSERT( beneficiaries[i].weight <= CREA_100_PERCENT, "Cannot allocate more than 100% of rewards to one account" );
  ...
  {
    validate_account_name( author );
    FC_ASSERT( percent_cbd <= CREA_100_PERCENT, "Percent cannot exceed 100%" );
  ...
  void vote_operation::validate() const
  {
    ...
    validate_account_name( author );\
    FC_ASSERT( abs(weight) <= CREA_100_PERCENT, "Weight is not a CREA percentage" );
    validate_permlink( permlink );
  }
  ...
  void set_withdraw_vesting_route_operation::validate() const
  {
    ...
    FC_ASSERT( 0 <= percent && percent <= CREA_100_PERCENT, "Percent must be valid CREA percent" );
  }
  ...
  void witness_update_operation::validate() const
  {
    ...
    FC_ASSERT( url.size() <= CREA_MAX_WITNESS_URL_LENGTH, "URL is too long" );

    FC_ASSERT( url.size() > 0, "URL size must be greater than 0" );
    FC_ASSERT( fc::is_utf8( url ), "URL is not valid UTF8" );
    FC_ASSERT( fee >= asset( 0, CREA_SYMBOL ), "Fee cannot be negative" );
  ...
  void custom_json_operation::validate() const {
    ...
    FC_ASSERT( id.size() <= CREA_CUSTOM_OP_ID_MAX_LENGTH,
      "Operation ID length exceeded. Max: ${max} Current: ${n}", ("max", CREA_CUSTOM_OP_ID_MAX_LENGTH)("n", id.size()) );
  ...
  void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
  {
    ...
    proof = fc::equihash::proof::hash( CREA_EQUIHASH_N, CREA_EQUIHASH_K, seed );
  ...
  void equihash_pow::validate() const
  {
    validate_account_name( input.worker_account );
    auto seed = fc::sha256::hash( input );
    FC_ASSERT( proof.n == CREA_EQUIHASH_N, "proof of work 'n' value is incorrect" );
    FC_ASSERT( proof.k == CREA_EQUIHASH_K, "proof of work 'k' value is incorrect" );
  ...
  void escrow_transfer_operation::validate()const
  {
    ...
    FC_ASSERT( cbd_amount.symbol == CBD_SYMBOL, "CBD amount must contain CBD asset" );
    FC_ASSERT( crea_amount.symbol == CREA_SYMBOL, "CREA amount must contain CREA asset" );
  ...
  {
    validate_account_name( account );
    FC_ASSERT( is_asset_type( reward_crea, CREA_SYMBOL ), "Reward CREA must be expressed in CREA" );
    FC_ASSERT( is_asset_type( reward_cbd, CBD_SYMBOL ), "Reward CBD must be expressed in CBD" );
