...
#define CREA_ASSET_NUM_CBD    (uint32_t(((SMT_MAX_NAI + CREA_NAI_CBD)   << CREA_NAI_SHIFT) | CREA_PRECISION_CBD))
#define CREA_ASSET_NUM_CREA   (uint32_t(((SMT_MAX_NAI + CREA_NAI_CREA)  << CREA_NAI_SHIFT) | CREA_PRECISION_CREA))
#define CREA_ASSET_NUM_VESTS  (uint32_t(((SMT_MAX_NAI + CREA_NAI_VESTS) << CREA_NAI_SHIFT) | CREA_PRECISION_VESTS))
...
#define CREA_SYMBOL_U64     OBSOLETE_SYMBOL_U64
#define OBD_SYMBOL_U64      (uint64_t('T') | (uint64_t('B') << 8) | (uint64_t('D') << 16))
#define CBD_SYMBOL_U64      OBD_SYMBOL_U64
#else

#define VESTS_SYMBOL_U64    (uint64_t('V') | (uint64_t('E') << 8) | (uint64_t('S') << 16) | (uint64_t('T') << 24) | (uint64_t('S') << 32))
#define OBSOLETE_SYMBOL_U64 (uint64_t('S') | (uint64_t('T') << 8) | (uint64_t('E') << 16) | (uint64_t('E') << 24) | (uint64_t('M') << 32))
#define CREA_SYMBOL_U64     (uint64_t('H') | (uint64_t('I') << 8) | (uint64_t('V') << 16) | (uint64_t('E') << 24))
#define OBD_SYMBOL_U64      (uint64_t('S') | (uint64_t('B') << 8) | (uint64_t('D') << 16))
#define CBD_SYMBOL_U64      (uint64_t('H') | (uint64_t('B') << 8) | (uint64_t('D') << 16))
...
class asset_symbol_type
{
  public:
    ...
    // buf must have space for CREA_ASSET_SYMBOL_NAI_STRING_LENGTH
