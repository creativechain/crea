...
void verify_authority( const vector<AuthContainerType>& auth_containers, const flat_set<public_key_type>& sigs,
                       ...
                       uint32_t max_recursion_depth = CREA_MAX_SIG_CHECK_DEPTH,
                       uint32_t max_membership = CREA_MAX_AUTHORITY_MEMBERSHIP,
                       uint32_t max_account_auths = CREA_MAX_SIG_CHECK_ACCOUNTS,
...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         CREA_ASSERT( s.check_authority(id) ||
   ...
   {
      CREA_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      CREA_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
