...
namespace crea { namespace protocol {
  ...
  /** Allows to store all SMT tokens being allowed to use during voting process.
    ...
    *  @see SMT spec for details: https://gitlab.syncad.com/crea/smt-whitepaper/blob/master/smt-manual/manual.md
    */
  struct allowed_vote_assets
  {
    ...
    void validate() const
    {
      ...
      FC_ASSERT(is_allowed(CREA_SYMBOL) == false,
        "CREA can not be explicitly specified as one of allowed_vote_assets");
    }
  ...
  /**
    ...
    *  The percent_cbd may be decreased, but never increased
    */
  struct comment_options_operation : public base_operation
  {
    ...
    uint16_t          percent_cbd            = CREA_100_PERCENT; /// the percent of CBD to key, unkept amounts will be received in form of VESTS
  ...
  /**
    *  This operation converts liquid token (CREA or liquid SMT) into VFS (Vesting Fund Shares,
  ...
  struct transfer_to_vesting_operation : public base_operation
  {
    ...
    asset             amount;  ///< must be CREA or liquid variant of SMT
  ...
  {
    /**
      *  This fee, paid in CREA, is converted into VESTS for the new account. Accounts
  ...
  struct witness_set_properties_operation : public base_operation
  {
    ...
    void get_required_authorities( vector< authority >& a )const
    {
      ...
      if( key_itr != props.end() )
      {
        ...
        a.push_back( authority( 1, CREA_NULL_ACCOUNT, 1 ) ); // The null account auth is impossible to satisfy
    }
  };
  ...
  /**
    ...
    *  used to define the exchange rate between CREA and the dollar (represented by CBD).
    */
  ...
  /**
    ...
    * CREA_MAX_WITNESSES blocks of the head block (1 round) and the
...
FC_REFLECT( crea::protocol::account_update2_operation,
        (account)
        (owner)
        (active)
        (posting)
...
FC_REFLECT( crea::protocol::comment_options_operation, (author)(permlink)(max_accepted_payout)(percent_cbd)(allow_votes)(allow_curation_rewards)(extensions) )
