...
{
  FC_ASSERT( permlink.size() > CREA_MIN_PERMLINK_LENGTH && permlink.size() < CREA_MAX_PERMLINK_LENGTH, "Permlink is not a valid size." );
...
void witness_update_evaluator::do_apply( const witness_update_operation& o )
{
  ...
  if ( _db.has_hardfork( CREA_HARDFORK_0_14__410 ) )
  {
    FC_ASSERT( o.props.account_creation_fee.symbol.is_canon() );
    if( _db.has_hardfork( CREA_HARDFORK_0_20__2651 ) || _db.is_producing() )
    {
      FC_TODO( "Move to validate() after HF20" );
      FC_ASSERT( o.props.account_creation_fee.amount <= CREA_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
    }
  }
  ...
  if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_20__2642 ))
  {
    FC_ASSERT( o.props.maximum_block_size <= CREA_SOFT_MAX_BLOCK_SIZE, "Max block size cannot be more than 2MiB" );
  }
...
{
  FC_ASSERT( _db.has_hardfork( CREA_HARDFORK_0_20__1620 ), "witness_set_properties_evaluator not enabled until HF 20" );
  ...
  if( flags.account_creation_changed )
  {
    ...
    {
      FC_TODO( "Move to validate() after HF20" );
      FC_ASSERT( props.account_creation_fee.amount <= CREA_MAX_ACCOUNT_CREATION_FEE, "account_creation_fee greater than maximum account creation fee" );
    }
  }
  ...
  if(itr == o.props.end() && _db.has_hardfork(CREA_HARDFORK_1_24))
  ...
  const time_point_sec& time, bool mined, const account_name_type& recovery_account, asset initial_delegation = asset( 0, VESTS_SYMBOL ) )
{
  ...
  if( db.has_hardfork( CREA_HARDFORK_0_11 ) )
  {
    FC_TODO( "If after HF 20, there are no temp account creations, the HF check can be removed." )
    if( !db.has_hardfork( CREA_HARDFORK_0_20__1782 ) || ( recovery_account != CREA_TEMP_ACCOUNT ) )
      actual_recovery_account = recovery_account;
  }
  else
  {
    actual_recovery_account = "crea";
  }

  return db.create< account_object >( name, key, time, mined, actual_recovery_account,
    !db.has_hardfork( CREA_HARDFORK_0_20__2539 ) /*voting flow 100%*/, initial_delegation );
}
...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
  ...
  {
    FC_TODO( "Move to validate() after HF20" );
    FC_ASSERT( o.fee <= asset( CREA_MAX_ACCOUNT_CREATION_FEE, CREA_SYMBOL ), "Account creation fee cannot be too large" );
  }

  if( _db.has_hardfork( CREA_HARDFORK_0_20__1771 ) )
  {
    FC_ASSERT( o.fee == wso.median_props.account_creation_fee, "Must pay the exact account creation fee. paid: ${p} fee: ${f}",
            ("p", o.fee)
            ("f", wso.median_props.account_creation_fee) );
  }
  else if( !_db.has_hardfork( CREA_HARDFORK_0_20__1761 ) && _db.has_hardfork( CREA_HARDFORK_0_19__987 ) )
  {
    FC_ASSERT( o.fee >= asset( wso.median_props.account_creation_fee.amount * CREA_CREATE_ACCOUNT_WITH_CREA_MODIFIER, CREA_SYMBOL ), "Insufficient Fee: ${f} required, ${p} provided.",
            ("f", wso.median_props.account_creation_fee * asset( CREA_CREATE_ACCOUNT_WITH_CREA_MODIFIER, CREA_SYMBOL ) )
            ("p", o.fee) );
  }
  else if( _db.has_hardfork( CREA_HARDFORK_0_1 ) )
  {
  ...
  if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_20 ) )
  {
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_15__465 ) )
  {
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_20__1762 ) )
  {
    _db.adjust_balance( _db.get< account_object, by_name >( CREA_NULL_ACCOUNT ), o.fee );
  }
  ...
  if( !_db.has_hardfork( CREA_HARDFORK_0_20__1762 ) && o.fee.amount > 0 )
  {
...
{
  FC_ASSERT( !_db.has_hardfork( CREA_HARDFORK_0_20__1760 ), "Account creation with delegation is deprecated as of Hardfork 20" );
  ...
  auto target_delegation = asset( wso.median_props.account_creation_fee.amount * CREA_CREATE_ACCOUNT_WITH_CREA_MODIFIER * CREA_CREATE_ACCOUNT_DELEGATION_RATIO, CREA_SYMBOL ) * props.get_vesting_share_price();

  auto current_delegation = asset( o.fee.amount * CREA_CREATE_ACCOUNT_DELEGATION_RATIO, CREA_SYMBOL ) * props.get_vesting_share_price() + o.delegation;
  ...
  if( o.delegation.amount > 0 || !_db.has_hardfork( CREA_HARDFORK_0_19__997 ) )
  {
    _db.create< vesting_delegation_object >( [&]( vesting_delegation_object& vdo )
    {
      ...
      vdo.min_delegation_time = _db.head_block_time() + CREA_CREATE_ACCOUNT_DELEGATION_TIME;
    });
  }
...
{
  if( _db.has_hardfork( CREA_HARDFORK_0_1 ) ) FC_ASSERT( o.account != CREA_TEMP_ACCOUNT, "Cannot update temp account." );

  if( ( _db.has_hardfork( CREA_HARDFORK_0_15__465 ) ) && o.posting )
    o.posting->validate();
  ...
  if( o.owner )
  {
    ...
    if( _db.has_hardfork( CREA_HARDFORK_0_11 ) )
      FC_ASSERT( _db.head_block_time() - account_auth.last_owner_update > CREA_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
#endif
  ...
  if( o.active && ( _db.has_hardfork( CREA_HARDFORK_0_15__465 ) ) )
    verify_authority_accounts_exist( _db, *o.active, o.account, authority::active );
  if( o.posting && ( _db.has_hardfork( CREA_HARDFORK_0_15__465 ) ) )
  ...
  if( o.json_metadata.size() > 0 )
  {
    ...
    {
      from_string( meta.json_metadata, o.json_metadata );
      if ( !_db.has_hardfork( CREA_HARDFORK_0_21__3274 ) )
      {
...
{
  FC_ASSERT( _db.has_hardfork( CREA_HARDFORK_0_21__3274 ), "Operation 'account_update2' is not enabled until HF 21" );
  FC_ASSERT( o.account != CREA_TEMP_ACCOUNT, "Cannot update temp account." );
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
  ...
  //if( _db.has_hardfork( CREA_HARDFORK_0_19__876 ) )
  /*
  ...
  */
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_19__977 ) )
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_6__80 ) && !comment.is_root() )
  {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
  ...
  */
  if( !comment_cashout )
  {
    FC_ASSERT( !_db.has_hardfork( CREA_HARDFORK_1_24 ), "Updating parameters for comment that is paid out is forbidden." );
    return;
  }
  ...
  FC_ASSERT( comment_cashout->percent_cbd >= o.percent_cbd, "A comment cannot accept a greater percent CBD." );

  _db.modify( *comment_cashout, [&]( comment_cashout_object& c ) {
      c.max_accepted_payout    = o.max_accepted_payout;
      c.percent_cbd            = o.percent_cbd;
...
{ try {
  if( _db.has_hardfork( CREA_HARDFORK_0_5__55 ) )
  ...
  if( o.parent_author != CREA_ROOT_POST_PARENT )
  {
    ...
    if( !_db.has_hardfork( CREA_HARDFORK_0_17__767 ) )
      FC_ASSERT( parent->get_depth() < CREA_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->get_depth())("y",CREA_MAX_COMMENT_DEPTH_PRE_HF17) );
    else
      FC_ASSERT( parent->get_depth() < CREA_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->get_depth())("y",CREA_MAX_COMMENT_DEPTH) );
  }
  ...
  if ( itr == by_permlink_idx.end() )
  {
    if( o.parent_author != CREA_ROOT_POST_PARENT )
    {
      if( _db.has_hardfork( CREA_HARDFORK_0_12__177 ) && !_db.has_hardfork( CREA_HARDFORK_0_17__869 ) )
    ...
    if( _db.has_hardfork( CREA_HARDFORK_0_20__2019 ) )
    {
      if( o.parent_author == CREA_ROOT_POST_PARENT )
          FC_ASSERT( ( _now - auth.last_root_post ) > CREA_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",_now)("last_root_post", auth.last_root_post) );
      else
          FC_ASSERT( ( _now - auth.last_post ) >= CREA_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",_now)("auth.last_post",auth.last_post) );
    }
    else if( _db.has_hardfork( CREA_HARDFORK_0_12__176 ) )
    {
      if( o.parent_author == CREA_ROOT_POST_PARENT )
          FC_ASSERT( ( _now - auth.last_root_post ) > CREA_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",_now)("last_root_post", auth.last_root_post) );
      else
          FC_ASSERT( ( _now - auth.last_post ) > CREA_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",_now)("auth.last_post",auth.last_post) );
    }
    else if( _db.has_hardfork( CREA_HARDFORK_0_6__113 ) )
    {
      if( o.parent_author == CREA_ROOT_POST_PARENT )
          FC_ASSERT( ( _now - auth.last_post ) > CREA_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",_now)("auth.last_post",auth.last_post) );
      else
          FC_ASSERT( ( _now - auth.last_post ) > CREA_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",_now)("auth.last_post",auth.last_post) );
    }
    else
    ...
    uint16_t reward_weight = CREA_100_PERCENT;
    uint64_t post_bandwidth = auth.post_bandwidth;

    if( _db.has_hardfork( CREA_HARDFORK_0_12__176 ) && !_db.has_hardfork( CREA_HARDFORK_0_17__733 ) && o.parent_author == CREA_ROOT_POST_PARENT )
    {
      uint64_t post_delta_time = std::min( _now.sec_since_epoch() - auth.last_root_post.sec_since_epoch(), CREA_POST_AVERAGE_WINDOW );
      uint32_t old_weight = uint32_t( ( post_bandwidth * ( CREA_POST_AVERAGE_WINDOW - post_delta_time ) ) / CREA_POST_AVERAGE_WINDOW );
      post_bandwidth = ( old_weight + CREA_100_PERCENT );
      reward_weight = uint16_t( std::min( ( CREA_POST_WEIGHT_CONSTANT * CREA_100_PERCENT ) / ( post_bandwidth * post_bandwidth ), uint64_t( CREA_100_PERCENT ) ) );
    }
    ...
    if( _db.has_hardfork( CREA_HARDFORK_0_1 ) )
    {
    ...
    if( _db.has_hardfork( CREA_HARDFORK_0_17__769 ) )
      cashout_time = _now + CREA_CASHOUT_WINDOW_SECONDS;
    else if( ( o.parent_author == CREA_ROOT_POST_PARENT ) && _db.has_hardfork( CREA_HARDFORK_0_12__177 ) )
      cashout_time = _now + CREA_CASHOUT_WINDOW_SECONDS_PRE_HF17;
    else
...
/// this loop can be skiped for validate-only nodes as it is merely gathering stats for indicies
  ...
  else // start edit case
  {
    ...
    if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_21__3313 ) )
    {
      FC_ASSERT( _now - auth.last_post_edit >= CREA_MIN_COMMENT_EDIT_INTERVAL, "Can only perform one comment edit per block." );
    }

    if( !_db.has_hardfork( CREA_HARDFORK_0_17__772 ) )
    {
      FC_ASSERT( comment_cashout, "Comment cashout object must exist" );
      if( _db.has_hardfork( CREA_HARDFORK_0_14__306 ) )
        FC_ASSERT( _db.calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum(), "The comment is archived." );
      else if( _db.has_hardfork( CREA_HARDFORK_0_10 ) )
    ...
    else if ( _db.has_hardfork( CREA_HARDFORK_0_21__2203 ) )
    {
...
void escrow_transfer_evaluator::do_apply( const escrow_transfer_operation& o )
{
  try
  {
    ...
    FC_ASSERT( from_account.pending_transfers < CREA_MAX_PENDING_TRANSFERS, "Account already has the maximum number of open escrow transfers." );
    ...
    _db.create<escrow_object>( o.from, o.to, o.agent, o.crea_amount, o.cbd_amount, o.fee, o.ratification_deadline, o.escrow_expiration, o.escrow_id );
...
void escrow_approve_evaluator::do_apply( const escrow_approve_operation& o )
{
  try
  {
    ...
    if( reject_escrow )
    {
      _db.adjust_balance( o.from, escrow.get_crea_balance() );
      _db.adjust_balance( o.from, escrow.get_cbd_balance() );
...
void escrow_release_evaluator::do_apply( const escrow_release_operation& o )
{
  try
  {
    ...
    const auto& e = _db.get_escrow( o.from, o.escrow_id );
    FC_ASSERT( e.get_crea_balance() >= o.crea_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.crea_amount)("b", e.get_crea_balance()) );
    FC_ASSERT( e.get_cbd_balance() >= o.cbd_amount, "Release amount exceeds escrow balance. Amount: ${a}, Balance: ${b}", ("a", o.cbd_amount)("b", e.get_cbd_balance()) );
    FC_ASSERT( e.to == o.to, "Operation 'to' (${o}) does not match escrow 'to' (${e}).", ("o", o.to)("e", e.to) );
    FC_ASSERT( e.agent == o.agent, "Operation 'agent' (${a}) does not match escrow 'agent' (${e}).", ("o", o.agent)("e", e.agent) );
    ...
    if( e.get_crea_balance().amount == 0 && e.get_cbd_balance().amount == 0 )
    {
      _db.modify( from_account, []( account_object& a )
      {
        a.pending_transfers--;
...
{
  if ( _db.has_hardfork(CREA_HARDFORK_1_24) && o.amount.symbol == CREA_SYMBOL && _db.is_treasury( o.to ) ) {
    const auto &fhistory = _db.get_feed_history();

    FC_ASSERT(!fhistory.current_median_history.is_null(), "Cannot send CREA to ${s} because there is no price feed.", ("s", o.to ));
    ...
    return;
  } else if( _db.has_hardfork( CREA_HARDFORK_0_21__3343 ) )
  {
    FC_ASSERT( o.amount.symbol == CBD_SYMBOL || !_db.is_treasury( o.to ), "Can only transfer CBD or CREA to ${s}", ("s", o.to ) );
  }
...
void transfer_to_vesting_evaluator::do_apply( const transfer_to_vesting_operation& o )
{
  ...
  if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_21__3343 ) )
  {
    FC_ASSERT( o.amount.symbol == CBD_SYMBOL || !_db.is_treasury( o.to ),
      "Can only transfer CBD to ${s}", ("s", o.to ) );
  }
  ...
  /*
    ...
    because malicious accounts would take over whole CREA network.
  ...
  */
  if( _db.has_hardfork( CREA_HARDFORK_1_24 ) )
  {
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
  ...
  FC_ASSERT( account.vesting_shares >= asset( 0, VESTS_SYMBOL ), "Account does not have sufficient Crea Energy for withdraw." );
  FC_ASSERT( static_cast<asset>(account.vesting_shares) - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Crea Energy for withdraw." );

  FC_TODO( "Remove this entire block after HF 20" )
  if( !_db.has_hardfork( CREA_HARDFORK_0_20__1860 ) && !account.mined && _db.has_hardfork( CREA_HARDFORK_0_1 ) )
  {
    const auto& props = _db.get_dynamic_global_properties();
    const witness_schedule_object& wso = _db.get_witness_schedule_object();
    ...
    FC_ASSERT( account.vesting_shares > min_vests || ( _db.has_hardfork( CREA_HARDFORK_0_16__562 ) && o.vesting_shares.amount == 0 ),
            "Account registered by another account requires 10x account creation fee worth of Crea Energy before it can be powered down." );
  }

  if( o.vesting_shares.amount == 0 )
  {
    if( _db.has_hardfork( CREA_HARDFORK_0_5__57 ) )
  ...
  {
    int vesting_withdraw_intervals = CREA_VESTING_WITHDRAW_INTERVALS_PRE_HF_16;
    if( _db.has_hardfork( CREA_HARDFORK_0_16__551 ) )
      vesting_withdraw_intervals = CREA_VESTING_WITHDRAW_INTERVALS; /// 13 weeks = 1 quarter of a year
    ...
    _db.modify( account, [&]( account_object& a )
    {
      ...
      if( _db.has_hardfork( CREA_HARDFORK_0_21 ) && new_vesting_withdraw_rate.amount * vesting_withdraw_intervals < o.vesting_shares.amount )
      {
        new_vesting_withdraw_rate.amount += 1;
      }
      ...
      a.next_vesting_withdrawal = _db.head_block_time() + fc::seconds(CREA_VESTING_WITHDRAW_INTERVAL_SECONDS);
...
void set_withdraw_vesting_route_evaluator::do_apply( const set_withdraw_vesting_route_operation& o )
{
  ...
  if( itr == wd_idx.end() )
  {
    ...
    FC_ASSERT( from_account.withdraw_routes < CREA_MAX_WITHDRAW_ROUTES, "Account already has the maximum number of routes." );
  ...
  FC_ASSERT( total_percent <= CREA_100_PERCENT, "More than 100% of vesting withdrawals allocated to destinations." );
  }
...
void account_witness_proxy_evaluator::do_apply( const account_witness_proxy_operation& o )
{
  ...
  if( o.proxy.size() ) {
    ...
    proxy_chain.reserve( CREA_MAX_PROXY_RECURSION_DEPTH + 1 );
    ...
    while( cprox->proxy.size() != 0 )
    {
      ...
      cprox = &next_proxy;
      FC_ASSERT( proxy_chain.size() <= CREA_MAX_PROXY_RECURSION_DEPTH, "Proxy chain is too long." );
    }
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
  ...
  if( itr == by_account_witness_idx.end() ) {
    ...
    if ( _db.has_hardfork( CREA_HARDFORK_0_2 ) )
    {
      FC_ASSERT( voter.witnesses_voted_for < CREA_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." ); // TODO: Remove after hardfork 2
      ...
      if( _db.has_hardfork( CREA_HARDFORK_0_3 ) ) {
...
void pre_hf20_vote_evaluator( const vote_operation& o, database& _db )
{
  ...
  if( !comment_cashout || ( _db.has_hardfork( CREA_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( *comment_cashout ) == fc::time_point_sec::maximum() ) )
  {
#ifndef CLEAR_VOTES
  ...
  int64_t  used_power  = ((current_power * abs_weight) / CREA_100_PERCENT) * (60*60*24);
  ...
  {
    if( _db.has_hardfork( CREA_HARDFORK_0_12__177 ) )
  ...
  if( itr == comment_vote_idx.end() )
  {
    ...
    if( rshares > 0 )
    {
      if( _db.has_hardfork( CREA_HARDFORK_0_17__900 ) )
        FC_ASSERT( _db.head_block_time() < comment_cashout->cashout_time - CREA_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
      else if( _db.has_hardfork( CREA_HARDFORK_0_7 ) )
        FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( *comment_cashout ) - CREA_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
    }
    ...
    if( !_db.has_hardfork( CREA_HARDFORK_0_17__769 ) )
    {
      ...
      if( _db.has_hardfork( CREA_HARDFORK_0_12__177 ) && !_db.has_hardfork( CREA_HARDFORK_0_13__257)  )
        new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + CREA_CASHOUT_WINDOW_SECONDS_PRE_HF17;
      else
        new_cashout_time_sec = _db.head_block_time().sec_since_epoch() + CREA_CASHOUT_WINDOW_SECONDS_PRE_HF12;
    ...
    _db.modify( *comment_cashout, [&]( comment_cashout_object& c ){
      ...
      if( rshares > 0 )
        ...
        c.net_votes--;
      if( !_db.has_hardfork( CREA_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares )
    ...
    if( root_cashout )
    {
      _db.modify( *root_cashout, [&]( comment_cashout_object& c )
      {
        ...
        if( !_db.has_hardfork( CREA_HARDFORK_0_17__769 ) )
        {
          if( _db.has_hardfork( CREA_HARDFORK_0_12__177 ) && c.last_payout > fc::time_point_sec::min() )
          ...
          if( c.max_cashout_time == fc::time_point_sec::maximum() )
            c.max_cashout_time = _db.head_block_time() + fc::seconds( CREA_MAX_CASHOUT_WINDOW_SECONDS );
        }
      });
    }
    ...
    **/
    ...
    _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
      ...
      if( curation_reward_eligible && _db.has_hardfork( CREA_HARDFORK_0_17__774 ) )
        curation_reward_eligible = _db.get_curation_rewards_percent() > 0;

      if( curation_reward_eligible )
      {
        if( comment_cashout->get_creation_time() < fc::time_point_sec(CREA_HARDFORK_0_6_REVERSE_AUCTION_TIME) )
        {
          ...
          if( _db.has_hardfork( CREA_HARDFORK_0_17__774 ) )
          {
            ...
            auto curve = !_db.has_hardfork( CREA_HARDFORK_0_19__1052 ) && comment_cashout->get_creation_time() > CREA_HF_19_SQRT_PRE_CALC
        ...
        if( _db.head_block_time() > fc::time_point_sec(CREA_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
        {
  ...
  {
    FC_ASSERT( itr->num_changes < CREA_MAX_VOTE_CHANGES, "Voter has used the maximum number of vote changes on this comment." );

    if( _db.has_hardfork( CREA_HARDFORK_0_6__112 ) )
    ...
    if( !_db.has_hardfork( CREA_HARDFORK_0_17__769 ) )
    {
      ...
      if( _db.has_hardfork( CREA_HARDFORK_0_14__259 ) && abs_rshares == 0 )
...
void hf20_vote_evaluator( const vote_operation& o, database& _db )
{
  ...
  {
    FC_ASSERT( !_db.has_hardfork( CREA_HARDFORK_1_24 ), "Votes evaluating for comment that is paid out is forbidden." );
  }
  ...
  auto now = _db.head_block_time();
  FC_ASSERT( ( now - voter.last_vote_time ).to_seconds() >= CREA_MIN_VOTE_INTERVAL_SEC, "Can only vote once every 3 seconds." );

  _db.modify( voter, [&]( account_object& a )
  {
    util::update_flowbar( _db.get_dynamic_global_properties(), a, _db.has_hardfork( CREA_HARDFORK_0_21__3336 ), _db.head_block_num() > CREA_HF_21_STALL_BLOCK );
  });

  if ( _db.has_hardfork( CREA_HARDFORK_0_21__3004 ) )
  {
  ...
  uint128_t used_flow = 0;

  FC_TODO( "This hardfork check should not be needed. Remove after HF21 if that is the case." );
  if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent && o.weight < 0 )
  {
    if( _db.has_hardfork( CREA_HARDFORK_0_22__3485 ) )
    {
      used_flow = ( std::max( ( ( uint128_t( voter.downvote_flowbar.current_flow ) * CREA_100_PERCENT ) / dgpo.downvote_pool_percent ),
                      uint128_t( voter.voting_flowbar.current_flow ) )
            * abs_weight * 60 * 60 * 24 ) / CREA_100_PERCENT;
    }
    else
    {
      used_flow = ( std::max( ( uint128_t( voter.downvote_flowbar.current_flow * CREA_100_PERCENT ) / dgpo.downvote_pool_percent ),
  ...
  {
    used_flow = ( uint128_t( voter.voting_flowbar.current_flow ) * abs_weight * 60 * 60 * 24 ) / CREA_100_PERCENT;
  }
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent && o.weight < 0 )
  {
    FC_ASSERT( voter.voting_flowbar.current_flow + voter.downvote_flowbar.current_flow > used_flow.to_int64(),
      "Account does not have enough flow to downvote. voting_flow: ${v} downvote_flow: ${d} required_flow: ${r}",
      ("v", voter.voting_flowbar.current_flow)("d", voter.downvote_flowbar.current_flow)("r", used_flow.to_int64()) );
  }
  else
  {
    FC_ASSERT( voter.voting_flowbar.has_flow( used_flow.to_int64() ), "Account does not have enough flow to vote." );
  }

  int64_t abs_rshares = used_flow.to_int64();
  ...
  if( itr == comment_vote_idx.end() )
  {
    ...
    _db.modify( voter, [&]( account_object& a )
    {
      if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) && dgpo.downvote_pool_percent > 0 && o.weight < 0 )
      {
        if( used_flow.to_int64() > a.downvote_flowbar.current_flow )
        {
          /* used flow is always less than downvote_flow + voting_flow because the amount used
            * is a fraction of max( downvote_flow, voting_flow ). If more flow is consumed than
            * there is downvote_flow, then it is because voting_flow is greater, and used_flow
            * is strictly smaller than voting_flow. This is the same reason why a check is not
            * required when using voting flow on its own as an upvote.
            */
          auto remainder = used_flow.to_int64() - a.downvote_flowbar.current_flow;
          a.downvote_flowbar.use_flow( a.downvote_flowbar.current_flow );
          a.voting_flowbar.use_flow( remainder );
        }
        else
        {
          a.downvote_flowbar.use_flow( used_flow.to_int64() );
        }
      }
      else
      {
        a.voting_flowbar.use_flow( used_flow.to_int64() );
      }
...
void custom_evaluator::do_apply( const custom_operation& o )
{
  database& d = db();
  if( d.is_producing() )
    FC_ASSERT( o.data.size() <= CREA_CUSTOM_OP_DATA_MAX_LENGTH,
      "Operation data must be less than ${bytes} bytes.", ("bytes", CREA_CUSTOM_OP_DATA_MAX_LENGTH) );

  if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_20 ) )
  {
    FC_ASSERT( o.required_auths.size() <= CREA_MAX_AUTHORITY_MEMBERSHIP,
      "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", CREA_MAX_AUTHORITY_MEMBERSHIP)("n", o.required_auths.size()) );
  }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
  ...
  if( d.is_producing() )
    FC_ASSERT( o.json.length() <= CREA_CUSTOM_OP_DATA_MAX_LENGTH,
      "Operation JSON must be less than ${bytes} bytes.", ("bytes", CREA_CUSTOM_OP_DATA_MAX_LENGTH) );

  if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_20 ) )
  {
    size_t num_auths = o.required_auths.size() + o.required_posting_auths.size();
    FC_ASSERT( num_auths <= CREA_MAX_AUTHORITY_MEMBERSHIP,
      "Authority membership exceeded. Max: ${max} Current: ${n}", ("max", CREA_MAX_AUTHORITY_MEMBERSHIP)("n", num_auths) );
  }
...
void custom_binary_evaluator::do_apply( const custom_binary_operation& o )
{
  ...
  FC_ASSERT( d.has_hardfork( CREA_HARDFORK_0_14__317 ) );
...
void pow_apply( database& db, Operation o )
{
  ...
  if( db.has_hardfork( CREA_HARDFORK_0_5__59 ) )
  {
  ...
  if( db.has_hardfork( CREA_HARDFORK_0_13__256 ) )
  ...
  if( db.head_block_num() < CREA_START_MINER_VOTING_BLOCK )
    pow_reward.amount *= CREA_MAX_WITNESSES;
...
void pow_evaluator::do_apply( const pow_operation& o ) {
  FC_ASSERT( !db().has_hardfork( CREA_HARDFORK_0_13__256 ), "pow is deprecated. Use pow2 instead" );
  pow_apply( db(), o );
}
...
{
  database& db = this->db();
  FC_ASSERT( !db.has_hardfork( CREA_HARDFORK_0_17__770 ), "mining is now disabled" );

  const auto& dgp = db.get_dynamic_global_properties();
  uint32_t target_pow = db.get_pow_summary_target();
  account_name_type worker_account;

  if( db.has_hardfork( CREA_HARDFORK_0_16__551 ) )
  {
  ...
  FC_ASSERT( o.props.maximum_block_size >= CREA_MIN_BLOCK_SIZE_LIMIT * 2, "Voted maximum block size is too small." );
...
{
  if( _db.has_hardfork( CREA_HARDFORK_0_20__409 ) )
    FC_ASSERT( is_asset_type( o.exchange_rate.base, CBD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, CREA_SYMBOL ),
        "Price feed must be a CBD/CREA price" );
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  _db.create<convert_request_object>( o.owner, o.amount, _db.head_block_time() + crea_conversion_delay, o.requestid );
}
...
void limit_order_create_evaluator::do_apply( const limit_order_create_operation& o )
{
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_20__1449 ) )
  {
    FC_ASSERT( o.expiration <= _db.head_block_time() + CREA_MAX_LIMIT_ORDER_EXPIRATION, "Limit Order Expiration must not be more than 28 days in the future" );
  }
  else
  {
    uint32_t rand_offset = _db.head_block_id()._hash[ 4 ] % 86400;
    expiration = std::min( o.expiration, fc::time_point_sec( CREA_HARDFORK_0_20_TIME + CREA_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
  }
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
  ...
  {
    expiration = std::min( o.expiration, fc::time_point_sec( CREA_HARDFORK_0_20_TIME + CREA_MAX_LIMIT_ORDER_EXPIRATION ) );
  }
...
{
  FC_ASSERT( !_db.has_hardfork( CREA_HARDFORK_0_4 ), "report_over_production_operation is disabled." );
}

void claim_account_evaluator::do_apply( const claim_account_operation& o )
{
  FC_ASSERT( _db.has_hardfork( CREA_HARDFORK_0_20__1771 ), "claim_account_operation is not enabled until hardfork 20." );
  ...
  if( o.fee.amount == 0 )
  {
    ...
    if( !_db.is_pending_tx() )
    {
      ...
      FC_ASSERT( current_witness.available_witness_account_subsidies >= CREA_ACCOUNT_SUBSIDY_PRECISION, "Witness ${w} does not have enough subsidized accounts to claim",
        ("w", current_witness.owner) );

      _db.modify( current_witness, [&]( witness_object& w )
      {
        w.available_witness_account_subsidies -= CREA_ACCOUNT_SUBSIDY_PRECISION;
      });
    }

    FC_ASSERT( gpo.available_account_subsidies >= CREA_ACCOUNT_SUBSIDY_PRECISION, "There are not enough subsidized accounts to claim" );

    _db.modify( gpo, [&]( dynamic_global_property_object& gpo )
    {
      gpo.available_account_subsidies -= CREA_ACCOUNT_SUBSIDY_PRECISION;
    });
  }
  else
  {
  ...
  _db.adjust_balance( _db.get_account( CREA_NULL_ACCOUNT ), o.fee );
...
{
  FC_ASSERT( _db.has_hardfork( CREA_HARDFORK_0_20__1771 ), "create_claimed_account_operation is not enabled until hardfork 20." );
...
void request_account_recovery_evaluator::do_apply( const request_account_recovery_operation& o )
{
  ...
  if( request == recovery_request_idx.end() ) // New Request
  {
    ...
    _db.create< account_recovery_request_object >( o.account_to_recover, o.new_owner_authority,
      _db.head_block_time() + CREA_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
  }
  ...
  else // Change Request
  {
    ...
    _db.modify( *request, [&]( account_recovery_request_object& req )
    {
      ...
      req.expires = _db.head_block_time() + CREA_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD;
    });
  }
}
...
void recover_account_evaluator::do_apply( const recover_account_operation& o )
{
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_12 ) )
    FC_ASSERT( _db.head_block_time() - account.last_account_recovery > CREA_OWNER_UPDATE_LIMIT, "Owner authority can only be updated once an hour." );
...
void change_recovery_account_evaluator::do_apply( const change_recovery_account_operation& o )
{
  ...
  if( request == change_recovery_idx.end() ) // New request
  {
    _db.create< change_recovery_account_request_object >( [&]( change_recovery_account_request_object& req )
    {
      ...
      req.effective_on = _db.head_block_time() + CREA_OWNER_AUTH_RECOVERY_PERIOD;
    });
  }
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
  ...
  FC_ASSERT( from.savings_withdraw_requests < CREA_SAVINGS_WITHDRAW_REQUEST_LIMIT, "Account has reached limit for pending withdraw requests." );

  FC_TODO( "Remove is producing after HF 21" );
  if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_21__3343 ) )
  {
    FC_ASSERT( op.amount.symbol == CBD_SYMBOL || !_db.is_treasury( op.to ), "Can only transfer CBD to ${s}", ("s", op.to ) );
  }

  FC_ASSERT( _db.get_savings_balance( from, op.amount.symbol ) >= op.amount );
  _db.adjust_savings_balance( from, -op.amount );
  _db.create<savings_withdraw_object>( op.from, op.to, op.amount, op.memo, _db.head_block_time() + CREA_SAVINGS_WITHDRAW_TIME, op.request_id );
...
{
  FC_ASSERT( _db.has_hardfork( CREA_HARDFORK_0_14__324 ) );
  ...
  if( o.decline )
  {
    ...
    _db.create< decline_voting_rights_request_object >( [&]( decline_voting_rights_request_object& req )
    {
      ...
      req.effective_date = _db.head_block_time() + CREA_OWNER_AUTH_RECOVERY_PERIOD;
    });
  }
  else
  {
...
void claim_reward_balance_evaluator::do_apply( const claim_reward_balance_operation& op )
{
  ...
  FC_ASSERT( op.reward_crea <= acnt.get_rewards(), "Cannot claim that much CREA. Claim: ${c} Actual: ${a}",
    ("c", op.reward_crea)("a", acnt.get_rewards() ) );
  FC_ASSERT( op.reward_cbd <= acnt.get_cbd_rewards(), "Cannot claim that much CBD. Claim: ${c} Actual: ${a}",
    ("c", op.reward_cbd)("a", acnt.get_cbd_rewards()) );
  ...
  if( op.reward_vests == acnt.get_vest_rewards() )
    reward_vesting_crea_to_move = acnt.get_vest_rewards_as_crea();
  else
    reward_vesting_crea_to_move = asset( ( ( uint128_t( op.reward_vests.amount.value ) * uint128_t( acnt.get_vest_rewards_as_crea().amount.value ) )
      / uint128_t( acnt.get_vest_rewards().amount.value ) ).to_uint64(), CREA_SYMBOL );
  ...
  _db.modify( acnt, [&]( account_object& a )
  {
    if( _db.has_hardfork( CREA_HARDFORK_0_20__2539 ) )
    {
      util::update_flowbar( _db.get_dynamic_global_properties(), a, _db.has_hardfork( CREA_HARDFORK_0_21__3336 ), _db.head_block_num() > CREA_HF_21_STALL_BLOCK, op.reward_vests.amount.value );
    }
    ...
    else
    {
      ...
      if( token.symbol == VESTS_SYMBOL)
      {
        ...
        if( token == a->get_vest_rewards() )
          reward_vesting_crea_to_move = a->get_vest_rewards_as_crea();
        else
          reward_vesting_crea_to_move = asset( ( ( uint128_t( token.amount.value ) * uint128_t( a->get_vest_rewards_as_crea().amount.value ) )
            / uint128_t( a->get_vest_rewards().amount.value ) ).to_uint64(), CREA_SYMBOL );
      ...
      {
        FC_ASSERT( is_asset_type( token, CREA_SYMBOL ) == false || token <= a->get_rewards(),
                "Cannot claim that much CREA. Claim: ${c} Actual: ${a}", ("c", token)("a", a->get_rewards() ) );
        FC_ASSERT( is_asset_type( token, CBD_SYMBOL ) == false || token <= a->get_cbd_rewards(),
                "Cannot claim that much CBD. Claim: ${c} Actual: ${a}", ("c", token)("a", a->get_cbd_rewards()) );
...
#pragma message( "TODO: Update get_effective_vesting_shares when modifying this operation to support SMTs." )
  ...
  if( _db.has_hardfork( CREA_HARDFORK_0_20__2539 ) )
  {
    ...
    {
      util::update_flowbar( gpo, a, _db.has_hardfork( CREA_HARDFORK_0_21__3336 ), _db.head_block_num() > CREA_HF_21_STALL_BLOCK );
    });
    ...
    if( gpo.downvote_pool_percent )
    {
      if( _db.has_hardfork( CREA_HARDFORK_0_22__3485 ) )
      {
        available_downvote_shares = asset(
          ( ( uint128_t( delegator.downvote_flowbar.current_flow ) * CREA_100_PERCENT ) / gpo.downvote_pool_percent
          + ( CREA_100_PERCENT / gpo.downvote_pool_percent ) - 1 ).to_int64(), VESTS_SYMBOL );
      }
      else
      {
        available_downvote_shares = asset(
          ( delegator.downvote_flowbar.current_flow * CREA_100_PERCENT ) / gpo.downvote_pool_percent
          + ( CREA_100_PERCENT / gpo.downvote_pool_percent ) - 1, VESTS_SYMBOL );
      }
    }
    else
    {
    ...
    available_downvote_shares.amount = std::min( available_downvote_shares.amount, max_flow - delegator.received_vesting_shares.amount );
  ...
  auto min_delegation = _db.has_hardfork( CREA_HARDFORK_0_20__1761 ) ?
    asset( wso.median_props.account_creation_fee.amount / 3, CREA_SYMBOL ) * gpo.get_vesting_share_price() :
    asset( wso.median_props.account_creation_fee.amount * 10, CREA_SYMBOL ) * gpo.get_vesting_share_price();
  auto min_update = _db.has_hardfork( CREA_HARDFORK_0_20__1761 ) ?
    asset( wso.median_props.account_creation_fee.amount / 30, CREA_SYMBOL ) * gpo.get_vesting_share_price() :
  ...
  if( delegation == nullptr )
  {
    ...
    FC_TODO( "This hardfork check should not be needed. Remove after HF21 if that is the case." );
    if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) )
      FC_ASSERT( available_downvote_shares >= op.vesting_shares, "Account ${acc} does not have enough downvote flow to delegate. required: ${r} available: ${a}",
    ...
    _db.modify( delegator, [&]( account_object& a )
    {
      ...
      if( _db.has_hardfork( CREA_HARDFORK_0_20__2539 ) )
      {
        ...
        if( _db.has_hardfork( CREA_HARDFORK_0_22__3485 ) )
        {
          a.downvote_flowbar.use_flow( ( ( uint128_t( op.vesting_shares.amount.value ) * gpo.downvote_pool_percent ) / CREA_100_PERCENT ).to_int64() );
        }
        else if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) )
        {
          a.downvote_flowbar.use_flow( op.vesting_shares.amount.value );
        }
      }
    });
    ...
    _db.modify( delegatee, [&]( account_object& a )
    {
      if( _db.has_hardfork( CREA_HARDFORK_0_20__2539 ) )
      {
        util::update_flowbar( gpo, a, _db.has_hardfork( CREA_HARDFORK_0_21__3336 ), _db.head_block_num() > CREA_HF_21_STALL_BLOCK, op.vesting_shares.amount.value );
      }
  ...
  else if( op.vesting_shares >= delegation->vesting_shares )
  {
    ...
    FC_TODO( "This hardfork check should not be needed. Remove after HF21 if that is the case." );
    if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) )
      FC_ASSERT( available_downvote_shares >= delta, "Account ${acc} does not have enough downvote flow to delegate. required: ${r} available: ${a}",
    ...
    _db.modify( delegator, [&]( account_object& a )
    {
      ...
      if( _db.has_hardfork( CREA_HARDFORK_0_20__2539 ) )
      {
        ...
        if( _db.has_hardfork( CREA_HARDFORK_0_22__3485 ) )
        {
          a.downvote_flowbar.use_flow( ( ( uint128_t( delta.amount.value ) * gpo.downvote_pool_percent ) / CREA_100_PERCENT ).to_int64() );
        }
        else if( _db.has_hardfork( CREA_HARDFORK_0_21__3336 ) )
        {
          a.downvote_flowbar.use_flow( delta.amount.value );
        }
      }
    });
    ...
    _db.modify( delegatee, [&]( account_object& a )
    {
      if( _db.has_hardfork( CREA_HARDFORK_0_20__2539 ) )
      {
        util::update_flowbar( gpo, a, _db.has_hardfork( CREA_HARDFORK_0_21__3336 ), _db.head_block_num() > CREA_HF_21_STALL_BLOCK, delta.amount.value );
      }
  ...
  else /* delegation->vesting_shares > op.vesting_shares */
  {
    ...
    {
      FC_ASSERT( delta >= min_update, "Crea Energy decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
