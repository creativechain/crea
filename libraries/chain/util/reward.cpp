...
uint64_t get_rshare_reward( const comment_reward_context& ctx )
{
   ...
   u256 rf(ctx.total_reward_fund_creativecoin.amount.value);
   ...
   if( is_comment_payout_dust( ctx.current_creativecoin_price, payout ) )
      payout = 0;

   asset max_creativecoin = to_creativecoin( ctx.current_creativecoin_price, ctx.max_sbd );

   payout = std::min( payout, uint64_t( max_creativecoin.amount.value ) );
