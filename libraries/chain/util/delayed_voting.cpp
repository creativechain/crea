...
void delayed_voting::run( const fc::time_point_sec& head_time )
{
  ...
  while( current != idx.end() &&
        ...
        head_time >= ( current->get_the_earliest_time() + CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS )
      )
  {
    ...
    /*
      ...
      Why `30`? CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS / CREA_DELAYED_VOTING_INTERVAL_SECONDS == 30
