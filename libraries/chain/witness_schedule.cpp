...
using crea::chain::util::rd_system_params;
using crea::chain::util::rd_user_params;
using crea::chain::util::rd_validate_user_params;
  ...
  db.modify( wso, [&](witness_schedule_object& o )
  {
   ...
   for( const auto& witness : idx )
   {
      db.modify( witness, [&]( witness_object& wobj )
      {
         ...
         wobj.virtual_scheduled_time = CREA_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wobj.votes.value+1);
      } );
   }
}
  ...
  rd_validate_user_params( account_subsidy_user_params );
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
    ...
    rd_setup_dynamics_params( account_subsidy_per_witness_user_params, account_subsidy_system_params, _wso.account_subsidy_witness_rd );
      ...
      int64_t min_decay = (fc::uint128( median_decay ) * CREA_DECAY_BACKSTOP_PERCENT / CREA_100_PERCENT).to_int64();
...
void update_witness_schedule4( database& db )
{
   ...
   active_witnesses.reserve( CREA_MAX_WITNESSES );
  ...
  selected_voted.reserve( wso.max_voted_witnesses );
   ...
   for( auto itr = widx.begin();
        ...
        ++itr )
   {
      if( db.has_hardfork( CREA_HARDFORK_0_14__278 ) && (itr->signing_key == public_key_type()) )
         continue;
  ...
  auto num_elected = active_witnesses.size();
   ...
   while( mitr != pow_idx.end() && selected_miners.size() < wso.max_miner_witnesses )
   {
      ...
      if( selected_voted.find(mitr->id) == selected_voted.end() )
      {
         ...
         if( !( db.has_hardfork( CREA_HARDFORK_0_14__278 ) && db.get_witness( mitr->owner ).signing_key == public_key_type() ) )
         {
  ...
  auto num_miners = selected_miners.size();
   ...
   for( auto witness_count = selected_voted.size() + selected_miners.size();
        sitr != schedule_idx.end() && witness_count < CREA_MAX_WITNESSES;
        ++sitr )
   {
      new_virtual_time = sitr->virtual_scheduled_time; /// everyone advances to at least this time
      processed_witnesses.push_back(sitr);

      if( db.has_hardfork( CREA_HARDFORK_0_14__278 ) && sitr->signing_key == public_key_type() )
  ...
  auto num_timeshare = active_witnesses.size() - num_miners - num_elected;
   ...
   {
      auto new_virtual_scheduled_time = new_virtual_time + CREA_VIRTUAL_SCHEDULE_LAP_LENGTH2 / ((*itr)->votes.value+1);
   ...
   size_t expected_active_witnesses = std::min( size_t(CREA_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("CREA_MAX_WITNESSES",CREA_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );

  auto majority_version = wso.majority_version;

   if( db.has_hardfork( CREA_HARDFORK_0_5__54 ) )
   {
    ...
    if( hpo.current_hardfork_version == CREA_HARDFORK_0_22_VERSION )
    {
      if( hpo.next_hardfork != CREA_HARDFORK_0_23_VERSION )
      {
        ...
        {
          _hpo.next_hardfork = CREA_HARDFORK_0_23_VERSION;
          _hpo.next_hardfork_time = fc::time_point_sec(CREA_HARDFORK_0_23_TIME);
        });
      }
    }
    else
  ...
  db.modify( wso, [&]( witness_schedule_object& _wso )
  {
    for( size_t i = 0; i < active_witnesses.size(); i++ )
    {
      ...
      for( size_t i = active_witnesses.size(); i < CREA_MAX_WITNESSES; i++ )
      {
...
{
   if( (db.head_block_num() % CREA_MAX_WITNESSES) == 0 ) //wso.next_shuffle_block_num )
   {
      if( db.has_hardfork(CREA_HARDFORK_0_4) )
      {
    ...
    fc::uint128 new_virtual_time;
      ...
      /// only use vote based scheduling after the first 1M CREA is created or if there is no POW queued
      if( props.num_pow_witnesses == 0 || db.head_block_num() > CREA_START_MINER_VOTING_BLOCK )
      {
         const auto& widx = db.get_index<witness_index>().indices().get<by_vote_name>();

         for( auto itr = widx.begin(); itr != widx.end() && (active_witnesses.size() < (CREA_MAX_WITNESSES-2)); ++itr )
         {
            if( itr->pow_worker )
               continue;
        ...
        db.modify( *sitr, [&]( witness_object& wo )
        {
          ...
          if( new_virtual_time == fc::uint128::max_value() )
              new_virtual_time = fc::uint128();
               ...
               if( db.has_hardfork( CREA_HARDFORK_0_2 ) )
                  wo.virtual_scheduled_time += CREA_VIRTUAL_SCHEDULE_LAP_LENGTH2 / (wo.votes.value+1);
               else
                  wo.virtual_scheduled_time += CREA_VIRTUAL_SCHEDULE_LAP_LENGTH / (wo.votes.value+1);
            } );
         }
      }
    ...
    const auto& pow_idx = db.get_index<witness_index>().indices().get<by_pow>();
      ...
      if( props.num_pow_witnesses > CREA_MAX_WITNESSES )
      {
         if( itr != pow_idx.end() )
         {
            db.modify( *itr, [&](witness_object& wit )
    ...
    while( itr != pow_idx.end() )
    {
      active_witnesses.push_back( itr->owner );
         ...
         if( db.head_block_num() > CREA_START_MINER_VOTING_BLOCK || active_witnesses.size() >= CREA_MAX_WITNESSES )
            break;
         ++itr;
      }
    ...
    /*
      ...
      _wso.current_shuffled_witnesses.reserve( active_witnesses.size() );
         ...
         for( const string& w : active_witnesses )
            ...
            */
         // active witnesses has exactly CREA_MAX_WITNESSES elements, asserted above
         for( size_t i = 0; i < active_witnesses.size(); i++ )
         {
            _wso.current_shuffled_witnesses[i] = active_witnesses[i];
         }
