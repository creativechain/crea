...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   if( !_db.has_hardfork( CREA_HARDFORK_0_20__1761 ) && _db.has_hardfork( CREA_HARDFORK_0_19__987 ) )
   {
...
void comment_options_evaluator::do_apply( const comment_options_operation& o )
{
   ...
   FC_ASSERT( comment.percent_creativecoin_dollars >= o.percent_creativecoin_dollars, "A comment cannot accept a greater percent SBD." );
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   FC_ASSERT( account.vesting_shares >= asset( 0, VESTS_SYMBOL ), "Account does not have sufficient Creativecoin Power for withdraw." );
   FC_ASSERT( account.vesting_shares - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Creativecoin Power for withdraw." );
   ...
   if( !_db.has_hardfork( CREA_HARDFORK_0_20__1860 ) && !account.mined && _db.has_hardfork( CREA_HARDFORK_0_1 ) )
   {
      ...
      FC_ASSERT( account.vesting_shares > min_vests || ( _db.has_hardfork( CREA_HARDFORK_0_16__562 ) && o.vesting_shares.amount == 0 ),
                 "Account registered by another account requires 10x account creation fee worth of Creativecoin Power before it can be powered down." );
   }
...
{ try {
   ...
   int64_t regenerated_power = (CREA_100_PERCENT * elapsed_seconds) / CREA_VOTE_REGENERATION_SECONDS;
   int64_t current_power     = std::min( int64_t(voter.voting_power + regenerated_power), int64_t(CREA_100_PERCENT) );
   ...
   int64_t max_vote_denom = dgpo.vote_power_reserve_rate * CREA_VOTE_REGENERATION_SECONDS;
      ...
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         if( curation_reward_eligible )
         {
            ...
            if( _db.head_block_time() > fc::time_point_sec(CREA_HARDFORK_0_6_REVERSE_AUCTION_TIME) )  /// start enforcing this prior to the hardfork
            {
               ...
               uint64_t delta_t = std::min( uint64_t((cv.last_update - comment.created).to_seconds()), uint64_t(CREA_REVERSE_AUCTION_WINDOW_SECONDS) );

               w *= delta_t;
               w /= CREA_REVERSE_AUCTION_WINDOW_SECONDS;
...
void feed_publish_evaluator::do_apply( const feed_publish_operation& o )
{
   if( _db.has_hardfork( CREA_HARDFORK_0_20__409 ) )
      FC_ASSERT( is_asset_type( o.exchange_rate.base, SBD_SYMBOL ) && is_asset_type( o.exchange_rate.quote, CREA_SYMBOL ),
            "Price feed must be a SBD/CREA price" );
...
void claim_reward_balance2_evaluator::do_apply( const claim_reward_balance2_operation& op )
{
   ...
   for( const asset& token : op.reward_tokens )
   {
      ...
      else
      {
         ...
         else if( token.symbol == CREA_SYMBOL || token.symbol == SBD_SYMBOL )
         {
...
#pragma message( "TODO: Update get_effective_vesting_shares when modifying this operation to support SMTs." )
   ...
   else if( op.vesting_shares >= delegation->vesting_shares )
   {
      ...
      FC_ASSERT( delta >= min_update, "Creativecoin Power increase is not enough of a difference. min_update: ${min}", ("min", min_update) );
   ...
   else /* delegation->vesting_shares > op.vesting_shares */
   {
      ...
      {
         FC_ASSERT( delta >= min_update, "Creativecoin Power decrease is not enough of a difference. min_update: ${min}", ("min", min_update) );
