...
#include <crea/chain/util/nai_generator.hpp>
#include <crea/chain/util/sps_processor.hpp>
#include <crea/chain/util/delayed_voting.hpp>
...
#endif /// IS_TEST_NET
  const char* value = getenv("CREA_HF24_TIME");
  if(value != nullptr)
  {
    hf24Time = atol(value);
    ilog("CREA_HF24_TIME has been specified through environment variable as ${v}, long value: ${l}", ("v", value)("l", hf24Time));
  }
...
long hf23_time()
{
  ...
  const char* value = getenv("CREA_HF23_TIME");
  if(value != nullptr)
  {
    hf23Time = atol(value);
    ilog("CREA_HF23_TIME has been specified through environment variable as ${v}, long value: ${l}", ("v", value)("l", hf23Time));
  }
...
uint32_t database::reindex( const open_args& args )
{
  ...
  BOOST_SCOPE_EXIT(this_,&note) {
    CREA_TRY_NOTIFY(this_->_post_reindex_signal, note);
  ...
  try
  {
    ...
    CREA_TRY_NOTIFY(_pre_reindex_signal, note);
    ...
    CREA_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{
  if( hardfork >= CREA_TREASURY_RENAME_HARDFORK )
    return NEW_CREA_TREASURY_ACCOUNT;
  else
...
{
  return ( name == NEW_CREA_TREASURY_ACCOUNT ) || ( name == OBSOLETE_TREASURY_ACCOUNT );
}
...
{ try {
  if( !has_hardfork( CREA_HARDFORK_0_6__127 ) )
...
{
  if( has_hardfork( CREA_HARDFORK_0_17__769 ) || comment.is_root() )
    return comment;
  else
...
{
  return get< reward_fund_object, by_name >( CREA_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
  ...
  return uint64_t(CREA_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
...
void database::pop_block()
{
  try
  {
    ...
    CREA_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
  ...
  auto interval = CREA_BLOCK_INTERVAL;
  const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
  ...
  if( when < first_slot_time )
    return 0;
  return (when - first_slot_time).to_seconds() / CREA_BLOCK_INTERVAL + 1;
}
      ...
      if( has_hardfork( CREA_HARDFORK_0_20__2539 ) )
      {
        modify( to_account, [&]( account_object& a )
        {
          util::update_flowbar(
            cprops,
            a,
            has_hardfork( CREA_HARDFORK_0_21__3336 ),
            head_block_num() > CREA_HF_21_STALL_BLOCK,
...
void database::adjust_proxied_witness_votes( const account_object& a,
                        const std::array< share_type, CREA_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                        int depth )
{
  if( a.proxy != CREA_PROXY_TO_SELF_ACCOUNT )
  {
    /// nested proxies are not supported, vote will not propagate
    if( depth >= CREA_MAX_PROXY_RECURSION_DEPTH )
      return;
    ...
    {
      for( int i = CREA_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
      {
  ...
  else
  {
    ...
    for( int i = CREA_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
      total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
  ...
  modify( witness, [&]( witness_object& w )
  {
    ...
    if( has_hardfork( CREA_HARDFORK_0_2 ) )
      w.virtual_scheduled_time = w.virtual_last_update + (CREA_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
    else
      w.virtual_scheduled_time = w.virtual_last_update + (CREA_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

    /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
    if( has_hardfork( CREA_HARDFORK_0_4 ) )
    {
...
void database::clear_witness_votes( const account_object& a )
{
  ...
  if( has_hardfork( CREA_HARDFORK_0_6__104 ) )
...
bool database::collect_account_total_balance( const account_object& account, asset* total_crea, asset* total_cbd,
  asset* total_vests, asset* vesting_shares_crea_value )
{
  const auto& gpo = get_dynamic_global_properties();

  *vesting_shares_crea_value = account.vesting_shares * gpo.get_vesting_share_price();
  *total_crea = *vesting_shares_crea_value;
  *total_vests = account.vesting_shares;
  *total_crea += account.get_vest_rewards_as_crea();
  *total_vests += account.get_vest_rewards();

  *total_crea += account.get_balance();
  *total_crea += account.get_savings();
  *total_crea += account.get_rewards();

  *total_cbd = account.get_cbd_balance();
  *total_cbd += account.get_cbd_savings();
  *total_cbd += account.get_cbd_rewards();

  return ( total_crea->amount.value != 0 ) || ( total_cbd->amount.value != 0 ) || ( total_vests->amount.value != 0 );
}
...
void database::clear_null_account_balance()
{
  ...
  asset total_crea, total_cbd, total_vests, vesting_shares_crea_value;

  if( !( collect_account_total_balance( null_account, &total_crea, &total_cbd, &total_vests, &vesting_shares_crea_value ) ) )
    return;
  ...
  if( null_account.get_cbd_balance().amount > 0 )
  {
    adjust_balance( null_account, -null_account.get_cbd_balance() );
  }

  if( null_account.get_cbd_savings().amount > 0 )
  {
    adjust_savings_balance( null_account, -null_account.get_cbd_savings() );
  }
  ...
  if( null_account.get_cbd_rewards().amount > 0 )
  {
    adjust_reward_balance( null_account, -null_account.get_cbd_rewards() );
  }
  ...
  if( null_account.get_vest_rewards().amount > 0 )
  {
    ...
    modify( gpo, [&]( dynamic_global_property_object& g )
    {
      ...
      g.pending_rewarded_vesting_crea -= null_account.get_vest_rewards_as_crea();
    });
...
{
  if( !has_hardfork( CREA_TREASURY_RENAME_HARDFORK ) )
    return;

  auto treasury_name = get_treasury_name();
  auto old_treasury_name = get_treasury_name( CREA_TREASURY_RENAME_HARDFORK - 1 );
  ...
  if( treasury_name == old_treasury_name || //ABW: once we actually include change of treasury in HF24 that part of condition can be removed
    !( collect_account_total_balance( old_treasury_account, &total_crea, &total_cbd, &total_vests, &vesting_shares_crea_value ) ) )
    return;
  ...
  if( total_crea.amount.value > 0 )
    vop.total_moved.push_back( total_crea );
  if( total_vests.amount.value > 0 )
    vop.total_moved.push_back( total_vests );
  if( total_cbd.amount.value > 0 )
    vop.total_moved.push_back( total_cbd );
  pre_push_virtual_operation( vop_op );
  ...
  if( old_treasury_account.get_cbd_balance().amount > 0 )
  {
    adjust_balance( treasury_account, old_treasury_account.get_cbd_balance() );
    adjust_balance( old_treasury_account, -old_treasury_account.get_cbd_balance() );
  }

  if( old_treasury_account.get_cbd_savings().amount > 0 )
  {
    adjust_savings_balance( treasury_account, old_treasury_account.get_cbd_savings() );
    adjust_savings_balance( old_treasury_account, -old_treasury_account.get_cbd_savings() );
  }
  ...
  if( old_treasury_account.vesting_shares.amount > 0 )
  {
    ...
    adjust_balance( treasury_account, vesting_shares_crea_value );
  ...
  if( old_treasury_account.get_cbd_rewards().amount > 0 )
  {
    adjust_reward_balance( treasury_account, old_treasury_account.get_cbd_rewards() );
    adjust_reward_balance( old_treasury_account, -old_treasury_account.get_cbd_rewards() );
  }

  if( old_treasury_account.get_vest_rewards().amount > 0 )
  {
    //see above handling of regular vests
    adjust_balance( treasury_account, old_treasury_account.get_vest_rewards_as_crea() );

    const auto& gpo = get_dynamic_global_properties();
    modify( gpo, [ & ]( dynamic_global_property_object& g )
    {
      g.pending_rewarded_vesting_shares -= old_treasury_account.get_vest_rewards();
      g.pending_rewarded_vesting_crea -= old_treasury_account.get_vest_rewards_as_crea();
    } );
...
{
  auto zero_crea = asset( 0 , CREA_SYMBOL );
  auto zero_cbd = asset( 0 , CBD_SYMBOL );
  ...
  {
    auto found = balances.find( hf23_helper::hf23_item{ name, zero_crea, zero_cbd } );
    ...
    adjust_balance( treasury_account, -found->cbd_balance );
    adjust_balance( treasury_account, -found->balance );

    adjust_balance( *account_ptr, found->cbd_balance );
    adjust_balance( *account_ptr, found->balance );

    operation vop = hardfork_crea_restore_operation( name, treasury_name, found->cbd_balance, found->balance );
    push_virtual_operation( vop );

    ilog( "Balances ${cbd} and ${crea} for the account ${acc} were restored", ( "cbd", found->cbd_balance )( "crea", found->balance )( "acc", name ) );
  }
}
...
void database::clear_accounts( hf23_helper::hf23_items& balances, const std::set< std::string >& cleared_accounts )
{
  ...
  for( auto account_name : cleared_accounts )
  {
    ...
    asset total_transferred_cbd, total_transferred_crea, total_converted_vests, total_crea_from_vests;
    clear_account( *account_ptr, &total_transferred_cbd, &total_transferred_crea, &total_converted_vests, &total_crea_from_vests );

    hf23_helper::gather_balance( balances, account_name, total_transferred_crea, total_transferred_cbd );

    operation vop = hardfork_crea_operation( account_name, treasury_name,
      total_transferred_cbd, total_transferred_crea, total_converted_vests, total_crea_from_vests );
    push_virtual_operation( vop );
  }
}

void database::clear_account( const account_object& account,
  asset* transferred_cbd_ptr, asset* transferred_crea_ptr,
  asset* converted_vests_ptr, asset* crea_from_vests_ptr )
{
  ...
  asset total_transferred_crea = asset( 0, CREA_SYMBOL );
  asset total_transferred_cbd = asset( 0, CBD_SYMBOL );
  asset total_converted_vests = asset( 0, VESTS_SYMBOL );
  asset total_crea_from_vests = asset( 0, CREA_SYMBOL );
  ...
  if( account.vesting_shares.amount > 0 )
  {
    ...
    auto converted_crea = vests_to_convert * cprops.get_vesting_share_price();
    total_converted_vests += account.vesting_shares;
    total_crea_from_vests += asset( converted_crea, CREA_SYMBOL );
    ...
    modify( account, [&]( account_object& a )
    {
      ...
      if( has_hardfork( CREA_HARDFORK_1_24 ) )
      {
        a.delayed_votes.clear();
        a.sum_delayed_votes = 0;
      }
    ...
    adjust_balance( treasury_account, asset( converted_crea, CREA_SYMBOL ) );
  ...
  while( escrow_itr != escrow_idx.end() && escrow_itr->from == account_name )
  {
    ...
    adjust_balance( account, escrow.get_crea_balance() );
    adjust_balance( account, escrow.get_cbd_balance() );
  ...
  if( has_hardfork( CREA_HARDFORK_1_24 ) )
  {
    adjust_balance( account, asset( 0, CBD_SYMBOL ) );
    adjust_savings_balance( account, asset( 0, CBD_SYMBOL ) );
    adjust_reward_balance( account, asset( 0, CBD_SYMBOL ) );
  }

  // Remove remaining savings balances
  total_transferred_crea += account.get_savings();
  total_transferred_cbd += account.get_cbd_savings();
  adjust_balance( treasury_account, account.get_savings() );
  adjust_savings_balance( account, -account.get_savings() );
  adjust_balance( treasury_account, account.get_cbd_savings() );
  adjust_savings_balance( account, -account.get_cbd_savings() );

  // Remove CBD and CREA balances
  total_transferred_crea += account.get_balance();
  total_transferred_cbd += account.get_cbd_balance();
  adjust_balance( treasury_account, account.get_balance() );
  adjust_balance( account, -account.get_balance() );
  adjust_balance( treasury_account, account.get_cbd_balance() );
  adjust_balance( account, -account.get_cbd_balance() );

  // Transfer reward balances
  total_transferred_crea += account.get_rewards();
  total_transferred_cbd += account.get_cbd_rewards();
  adjust_balance( treasury_account, account.get_rewards() );
  adjust_reward_balance( account, -account.get_rewards() );
  adjust_balance( treasury_account, account.get_cbd_rewards() );
  adjust_reward_balance( account, -account.get_cbd_rewards() );

  // Convert and transfer vesting rewards
  adjust_balance( treasury_account, account.get_vest_rewards_as_crea() );
  total_converted_vests += account.get_vest_rewards();
  total_crea_from_vests += account.get_vest_rewards_as_crea();

  modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
  {
    gpo.pending_rewarded_vesting_shares -= account.get_vest_rewards();
    gpo.pending_rewarded_vesting_crea -= account.get_vest_rewards_as_crea();
  } );

  modify( account, []( account_object &a )
  {
    a.reward_vesting_balance = asset( 0, VESTS_SYMBOL );
    a.reward_vesting_crea = asset( 0, CREA_SYMBOL );
  } );

  if( transferred_cbd_ptr != nullptr )
    *transferred_cbd_ptr = total_transferred_cbd;
  if( transferred_crea_ptr != nullptr )
    *transferred_crea_ptr = total_transferred_crea;
  if( converted_vests_ptr != nullptr )
    *converted_vests_ptr = total_converted_vests;
  if( crea_from_vests_ptr != nullptr )
    *crea_from_vests_ptr = total_crea_from_vests;
}

void database::process_proposals( const block_notification& note )
{
  if( has_hardfork( CREA_PROPOSALS_HARDFORK ) )
  {
...
{
  if( head_block_num() >= CREA_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
  {
  ...
  while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
  {
    ...
    */
        ...
        if( !( auto_vest_mode ^ itr->auto_vest ) )
        {
          ...
          if( to_deposit > 0 )
          {
            ...
            else
            {
              modify( cprops, [&]( dynamic_global_property_object& o )
              {
                o.total_vesting_fund_crea     -= routed;
    ...
    modify( from_account, [&]( account_object& a )
    {
      ...
      {
        a.next_vesting_withdrawal += fc::seconds( CREA_VESTING_WITHDRAW_INTERVAL_SECONDS );
      }
    });
...
void database::adjust_total_payout( const comment_cashout_object& cur, const asset& cbd_created, const asset& curator_cbd_value, const asset& beneficiary_value )
{
...
void fill_comment_reward_context_local_state( util::comment_reward_context& ctx, const comment_cashout_object& comment_cashout )
{
  ...
  ctx.max_cbd = comment_cashout.max_accepted_payout;
}
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment, const comment_cashout_object& comment_cashout, bool forward_curation_remainder )
{
  try
  {
    ...
    if( comment_cashout.net_rshares > 0 )
    {
      ...
      if( reward_tokens > 0 )
      {
        share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent() ) / CREA_100_PERCENT ).to_uint64();
        ...
        for( auto& b : comment_cashout.beneficiaries )
        {
          ...
          auto vop = comment_benefactor_reward_operation( b.account, comment_author, to_string( comment_cashout.permlink ), asset( 0, CBD_SYMBOL ), asset( 0, CREA_SYMBOL ), asset( 0, VESTS_SYMBOL ) );

          if( has_hardfork( CREA_HARDFORK_0_21__3343 ) && is_treasury( b.account ) )
          {
            benefactor_vesting_crea = 0;
            vop.cbd_payout = asset( benefactor_tokens, CREA_SYMBOL ) * get_feed_history().current_median_history;
            adjust_balance( get_treasury(), vop.cbd_payout );
            adjust_supply( asset( -benefactor_tokens, CREA_SYMBOL ) );
            adjust_supply( vop.cbd_payout );
          }
          else if( has_hardfork( CREA_HARDFORK_0_20__2022 ) )
          {
            auto benefactor_cbd_crea = ( benefactor_tokens * comment_cashout.percent_cbd ) / ( 2 * CREA_100_PERCENT ) ;
        ...
        auto cbd_crea     = ( author_tokens * comment_cashout.percent_cbd ) / ( 2 * CREA_100_PERCENT ) ;
        ...
        /*
          Total payout for curators is calculated due to the performance in third party softwares(f.e. `creamind`).
        ...
        */
        auto curators_vesting_payout = calculate_vesting( *this, asset( curation_tokens, CREA_SYMBOL ), has_hardfork( CREA_HARDFORK_0_17__659 ) );

        operation vop = author_reward_operation( comment_author, to_string( comment_cashout.permlink ), cbd_payout.first, cbd_payout.second, asset( 0, VESTS_SYMBOL ),
        ...
        adjust_total_payout( comment_cashout, cbd_payout.first + to_cbd( cbd_payout.second + asset( vesting_crea, CREA_SYMBOL ) ), to_cbd( asset( curation_tokens, CREA_SYMBOL ) ), to_cbd( asset( total_beneficiary, CREA_SYMBOL ) ) );

        post_push_virtual_operation( vop );
        vop = comment_reward_operation( comment_author, to_string( comment_cashout.permlink ), to_cbd( asset( claimed_reward, CREA_SYMBOL ) ), author_tokens,
    ...
    modify( comment_cashout, [&]( comment_cashout_object& c )
    {
      ...
      */
      ...
      if( has_hardfork( CREA_HARDFORK_0_17__769 ) )
      {
        c.cashout_time = fc::time_point_sec::maximum();
      }
      else if( comment.is_root() )
      {
        if( has_hardfork( CREA_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
          c.cashout_time = head_block_time() + CREA_SECOND_CASHOUT_WINDOW;
        else
          c.cashout_time = fc::time_point_sec::maximum();
      }
    ...
    while( vote_itr != vote_idx.end() && vote_itr->comment == comment.get_id() )
    {
      ...
      ++vote_itr;
      if( !has_hardfork( CREA_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment_cashout ) != fc::time_point_sec::maximum() )
      {
...
void database::process_comment_cashout()
{
  ...
  if( !has_hardfork( CREA_FIRST_CASHOUT_TIME ) )
    return;
  ...
  for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
  {
    ...
    modify( *itr, [&]( reward_fund_object& rfo )
    {
      ...
      if( has_hardfork( CREA_HARDFORK_0_19__1051 ) )
        decay_time = CREA_RECENT_RSHARES_DECAY_TIME_HF19;
      else
        decay_time = CREA_RECENT_RSHARES_DECAY_TIME_HF17;
  ...
  if( has_hardfork( CREA_HARDFORK_0_17__771 ) )
  {
  ...
  while( current != cidx.end() && current->cashout_time <= head_block_time() )
  {
    ...
    if( has_hardfork( CREA_HARDFORK_0_17__771 ) )
    {
      ...
      funds[ fund_id ].crea_awarded += cashout_comment_helper( ctx, _comment, *current, forward_curation_remainder );
    }
    else
    {
      ...
      while( itr != com_by_root.end() && itr->get_root_id() == _comment.get_root_id() )
      {
        ...
        ctx.total_reward_fund_crea = gpo.get_total_reward_fund_crea();
    ...
    if( has_hardfork( CREA_HARDFORK_0_19__876 ) )
    {
...
void database::process_funds()
{
  ...
  if( has_hardfork( CREA_HARDFORK_0_16__551) )
  {
    /**
      ...
      */
    int64_t start_inflation_rate = int64_t( CREA_INFLATION_RATE_START_PERCENT );
    int64_t inflation_rate_adjustment = int64_t( head_block_num() / CREA_INFLATION_NARROWING_PERIOD );
    int64_t inflation_rate_floor = int64_t( CREA_INFLATION_RATE_STOP_PERCENT );
    ...
    auto content_reward = ( new_crea * props.content_reward_percent ) / CREA_100_PERCENT;
    if( has_hardfork( CREA_HARDFORK_0_17__774 ) )
      content_reward = pay_reward_funds( content_reward );
    auto vesting_reward = ( new_crea * props.vesting_reward_percent ) / CREA_100_PERCENT;
    auto sps_fund = ( new_crea * props.sps_fund_percent ) / CREA_100_PERCENT;
    auto witness_reward = new_crea - content_reward - vesting_reward - sps_fund;
    ...
    if( sps_fund.value )
    {
      new_cbd = asset( sps_fund, CREA_SYMBOL ) * feed.current_median_history;
      adjust_balance( get_treasury_name(), new_cbd );
    }
    ...
    modify( props, [&]( dynamic_global_property_object& p )
    {
      ...
      p.virtual_supply      += asset( new_crea + sps_fund, CREA_SYMBOL );
  ...
  else
  {
    ...
    if( props.head_block_number < CREA_START_VESTING_BLOCK )
...
{
  if( has_hardfork( CREA_HARDFORK_0_12__178 ) )
    return asset( 0, CREA_SYMBOL );

  const auto& props = get_dynamic_global_properties();
  static_assert( CREA_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
  asset percent( protocol::calc_percent_reward_per_hour< CREA_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), CREA_SYMBOL );
  return std::max( percent, CREA_MIN_LIQUIDITY_REWARD );
}
...
asset database::get_content_reward()const
{
  ...
  asset percent( protocol::calc_percent_reward_per_block< CREA_CONTENT_APR_PERCENT >( props.virtual_supply.amount ), CREA_SYMBOL );
  return std::max( percent, CREA_MIN_CONTENT_REWARD );
}
...
asset database::get_curation_reward()const
{
  const auto& props = get_dynamic_global_properties();
  static_assert( CREA_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
  asset percent( protocol::calc_percent_reward_per_block< CREA_CURATE_APR_PERCENT >( props.virtual_supply.amount ), CREA_SYMBOL);
  return std::max( percent, CREA_MIN_CURATE_REWARD );
}
...
asset database::get_producer_reward()
{
  const auto& props = get_dynamic_global_properties();
  static_assert( CREA_BLOCK_INTERVAL == 3, "this code assumes a 3-second time interval" );
  asset percent( protocol::calc_percent_reward_per_block< CREA_PRODUCER_APR_PERCENT >( props.virtual_supply.amount ), CREA_SYMBOL);
  auto pay = std::max( percent, CREA_MIN_PRODUCER_REWARD );
...
#ifndef IS_TEST_NET
  /// 0 block rewards until at least CREA_MAX_WITNESSES have produced a POW
  if( props.num_pow_witnesses < CREA_MAX_WITNESSES && props.head_block_number < CREA_START_VESTING_BLOCK )
    return asset( 0, CREA_SYMBOL );
#endif
...
void database::pay_liquidity_reward()
{
  ...
  if( (head_block_num() % CREA_LIQUIDITY_REWARD_BLOCKS) == 0 )
  {
...
uint16_t database::get_curation_rewards_percent() const
{
  ...
  else if( has_hardfork( CREA_HARDFORK_0_8__116 ) )
    return CREA_1_PERCENT * 25;
  else
    return CREA_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
  ...
  for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
  {
    ...
    auto r = ( reward * itr->percent_content_rewards ) / CREA_100_PERCENT;

    modify( *itr, [&]( reward_fund_object& rfo )
    {
      rfo.reward_balance += asset( r, CREA_SYMBOL );
    });

    used_rewards += r;

    // Sanity check to ensure we aren't printing more CREA than has been allocated through inflation
...
void database::account_recovery_processing()
{
  ...
  while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + CREA_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
  {
    remove( *hist );
    hist = hist_idx.begin();
  }
...
void database::expire_escrow_ratification()
{
  ...
  while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
  {
    ...
    adjust_balance( old_escrow.from, old_escrow.get_crea_balance() );
    adjust_balance( old_escrow.from, old_escrow.get_cbd_balance() );
...
void database::process_decline_voting_rights()
{
  ...
  while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
  {
    ...
    std::array<share_type, CREA_MAX_PROXY_RECURSION_DEPTH+1> delta;
    delta[0] = -account.get_real_vesting_shares();
    for( int i = 0; i < CREA_MAX_PROXY_RECURSION_DEPTH; ++i )
    ...
    {
      a.can_vote = false;
      a.proxy = CREA_PROXY_TO_SELF_ACCOUNT;
    });
...
void database::init_genesis( uint64_t init_supply, uint64_t cbd_init_supply )
{
  try
  {
    ...
    public_key_type      init_public_key(CREA_INIT_PUBLIC_KEY);

    create< account_object >( CREA_MINER_ACCOUNT );
    create< account_authority_object >( [&]( account_authority_object& auth )
    {
      auth.account = CREA_MINER_ACCOUNT;
      auth.owner.weight_threshold = 1;
      auth.active.weight_threshold = 1;
    });

    create< account_object >( CREA_NULL_ACCOUNT );
    create< account_authority_object >( [&]( account_authority_object& auth )
    {
      auth.account = CREA_NULL_ACCOUNT;
      auth.owner.weight_threshold = 1;
      auth.active.weight_threshold = 1;
    });
    ...
    create< account_object >( NEW_CREA_TREASURY_ACCOUNT );
#endif

    create< account_object >( CREA_TEMP_ACCOUNT );
    create< account_authority_object >( [&]( account_authority_object& auth )
    {
      auth.account = CREA_TEMP_ACCOUNT;
      auth.owner.weight_threshold = 0;
      auth.active.weight_threshold = 0;
    });

    for( int i = 0; i < CREA_NUM_INIT_MINERS; ++i )
    {
      create< account_object >( CREA_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() ), init_public_key );

      create< account_authority_object >( [&]( account_authority_object& auth )
      {
        auth.account = CREA_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
      ...
      {
        w.owner        = CREA_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
        w.signing_key  = init_public_key;
        w.schedule = witness_object::miner;
      } );
    }

    create< dynamic_global_property_object >( CREA_INIT_MINER_NAME, asset( init_supply, CREA_SYMBOL ), asset( cbd_init_supply, CBD_SYMBOL ) );
    // feed initial token supply to first miner
    modify( get_account( CREA_INIT_MINER_NAME ), [&]( account_object& a )
    {
      a.balance = asset( init_supply, CREA_SYMBOL );
      a.cbd_balance = asset( cbd_init_supply, CBD_SYMBOL );
    } );

#ifdef IS_TEST_NET
    create< feed_history_object >( [&]( feed_history_object& o )
    {
      o.current_median_history = price( asset( 1, CBD_SYMBOL ), asset( 1, CREA_SYMBOL ) );
    });
#else
    // Nothing to do
    create< feed_history_object >( [&]( feed_history_object& o ) {});
    ...
    {
      hpo.processed_hardforks.push_back( CREA_GENESIS_TIME );
    } );
...
void database::notify_changed_objects()
{
  try
  {
    ...
    get_changed_ids( ids );
    CREA_TRY_NOTIFY( changed_objects, ids )*/
    /*
    if( _undo_db.enabled() )
    {
      ...
      CREA_TRY_NOTIFY( changed_objects, changed_ids )
    }
    */
  }
  FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
  ...
  if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( CREA_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / CREA_100_PERCENT ).to_uint64() ) )
  {
    uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / CREA_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
  ...
  if( BOOST_UNLIKELY( next_block_num == 1 ) )
  {
    ...
    uint32_t n;
    for( n=0; n<CREA_NUM_HARDFORKS; n++ )
    {
  ...
  if( has_hardfork( CREA_HARDFORK_0_12 ) )
  {
  ...
  if( block_size < CREA_MIN_BLOCK_SIZE )
  {
    elog( "Block size is too small",
      ("next_block_num",next_block_num)("block_size", block_size)("min",CREA_MIN_BLOCK_SIZE)
    );
  }
  ...
  if( has_hardfork( CREA_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
  {
...
try {
  if( (head_block_num() % CREA_FEED_INTERVAL_BLOCKS) != 0 )
    return;
  ...
  for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
  {
    ...
    {
      if( now < wit.get_last_cbd_exchange_update() + CREA_MAX_FEED_AGE_SECONDS
        && !wit.get_cbd_exchange_rate().is_null() )
      {
        feeds.push_back( wit.get_cbd_exchange_rate() );
      }
    }
    else if( wit.get_last_cbd_exchange_update() < now + CREA_MAX_FEED_AGE_SECONDS &&
        !wit.get_cbd_exchange_rate().is_null() )
    {
      feeds.push_back( wit.get_cbd_exchange_rate() );
    }
    ...
    modify( get_feed_history(), [&]( feed_history_object& fho )
    {
      ...
      if( fho.price_history.size() )
      {
        ...
        if( has_hardfork( CREA_HARDFORK_0_14__230 ) )
        {
          ...
          if( gpo.get_current_cbd_supply().amount > 0 )
          {
            price min_price( asset( 9 * gpo.get_current_cbd_supply().amount, CBD_SYMBOL ), gpo.current_supply );
...
{ try {
  ...
  if( BOOST_LIKELY(head_block_num() > 0) )
  {
    if( !(skip & skip_tapos_check) )
    {
      ...
      CREA_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
    ...
    CREA_ASSERT( trx.expiration <= now + fc::seconds(CREA_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
            "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",CREA_MAX_TIME_UNTIL_EXPIRATION));
    if( has_hardfork( CREA_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
      CREA_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
    CREA_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
  }
  ...
  while( true )
  {
    ...
    if( actions_itr == actions.end() )
    {
      ...
      if( pending_itr != pending_action_idx.end() && pending_itr->execution_time <= head_block_time() )
      {
        ...
        const auto& gpo = get_dynamic_global_properties();
        uint64_t required_actions_partition_size = ( gpo.maximum_block_size * gpo.required_actions_partition_percent ) / CREA_100_PERCENT;
...
{
  if( !has_hardfork( CREA_SMT_HARDFORK ) ) return;
...
FC_TODO( "#ifndef not needed after HF 20 is live" );
  ...
  {
    CREA_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < CREA_MAX_UNDO_HISTORY, undo_database_exception,
            "The database does not have enough undo history to support a blockchain with so many missed blocks. "
            "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
            ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
            ("max_undo",CREA_MAX_UNDO_HISTORY) );
  }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
  modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
  {
    dgp.virtual_supply = dgp.current_supply
      + ( get_feed_history().current_median_history.is_null() ? asset( 0, CREA_SYMBOL ) : dgp.get_current_cbd_supply() * get_feed_history().current_median_history );
    ...
    if( !median_price.is_null() && has_hardfork( CREA_HARDFORK_0_14__230 ) )
    {
      ...
      if( has_hardfork( CREA_HARDFORK_1_24 ) )
      {
        // Removing the cbd in the treasury from the debt ratio calculations
        const auto &treasury_account = get_treasury();
        const auto hdb_supply_without_treasury = (dgp.get_current_cbd_supply() - treasury_account.cbd_balance).amount < 0 ? asset(0, CBD_SYMBOL) : (dgp.get_current_cbd_supply() - treasury_account.cbd_balance) ;
        ...
        percent_cbd = uint16_t( ( ( fc::uint128_t( ( hdb_supply_without_treasury * get_feed_history().current_median_history ).amount.value ) * CREA_100_PERCENT + virtual_supply_without_treasury.amount.value/2 )
                                  / virtual_supply_without_treasury.amount.value ).to_uint64() );
      } else if( has_hardfork( CREA_HARDFORK_0_21 ) )
      {
        percent_cbd = uint16_t( ( ( fc::uint128_t( ( dgp.get_current_cbd_supply() * get_feed_history().current_median_history ).amount.value ) * CREA_100_PERCENT + dgp.virtual_supply.amount.value/2 )
          / dgp.virtual_supply.amount.value ).to_uint64() );
      }
      else
      {
        percent_cbd = uint16_t( ( ( fc::uint128_t( ( dgp.get_current_cbd_supply() * get_feed_history().current_median_history ).amount.value ) * CREA_100_PERCENT )
        / dgp.virtual_supply.amount.value ).to_uint64() );
      }
...
{ try {
  ...
  /**
    ...
    *
    */
  if( head_block_num() < CREA_START_MINER_VOTING_BLOCK )
  {
    modify( dpo, [&]( dynamic_global_property_object& _dpo )
    {
      if ( head_block_num() > CREA_MAX_WITNESSES )
        _dpo.last_irreversible_block_num = head_block_num() - CREA_MAX_WITNESSES;
    } );
  }
  else
  {
    ...
    static_assert( CREA_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

    // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
    // 1 1 1 1 1 1 1 2 2 2 -> 1
    // 3 3 3 3 3 3 3 3 3 3 -> 3

    size_t offset = ((CREA_100_PERCENT - CREA_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / CREA_100_PERCENT);
...
{
  bool has_hf_20__1815 = has_hardfork( CREA_HARDFORK_0_20__1815 );

#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
  if( has_hf_20__1815 )
  {
    CREA_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
    ...
    CREA_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
    CREA_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
    CREA_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
    CREA_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
      order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
      ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
  }
...
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
  if( has_hf_20__1815 )
  {
    CREA_ASSERT( new_order_pays == new_order.amount_for_sale() ||
  ...
  if( !has_hardfork( CREA_HARDFORK_0_12__178 ) &&
      ( (age >= CREA_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( CREA_HARDFORK_0_10__149)) ||
      (age >= CREA_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( CREA_HARDFORK_0_10__149) ) ) )
  {
    if( old_order_receives.symbol == CREA_SYMBOL )
    {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_cbd )
{
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
  try
  {
    ...
    {
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
      if( has_hardfork( CREA_HARDFORK_0_20__1815 ) )
      {
        CREA_ASSERT( pays < order.amount_for_sale(),
          order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
          ("order", order)("pays", pays)("receives", receives) );
      }
  ...
  modify( a, [&]( account_object& acnt )
  {
    switch( delta.symbol.asset_num )
    {
      case CREA_ASSET_NUM_CREA:
        ...
        if( check_balance )
        {
          FC_ASSERT( acnt.get_balance().amount.value >= 0, "Insufficient CREA funds" );
        }
        break;
      case CREA_ASSET_NUM_CBD:
        if( a.cbd_seconds_last_update != head_block_time() )
        {
          acnt.cbd_seconds += fc::uint128_t(a.get_cbd_balance().amount.value) * (head_block_time() - a.cbd_seconds_last_update).to_seconds();
              ...
              (acnt.cbd_seconds_last_update - acnt.cbd_last_interest_payment).to_seconds() > CREA_CBD_INTEREST_COMPOUND_INTERVAL_SEC )
          {
            ...
            interest *= get_dynamic_global_properties().get_cbd_interest_rate();
        ...
        if( check_balance )
        {
          FC_ASSERT( acnt.get_cbd_balance().amount.value >= 0, "Insufficient CBD funds" );
        }
        break;
    ...
    switch( value_delta.symbol.asset_num )
    {
      case CREA_ASSET_NUM_CREA:
        if( share_delta.amount.value == 0 )
        {
          ...
          if( check_balance )
          {
            FC_ASSERT( acnt.get_rewards().amount.value >= 0, "Insufficient reward CREA funds" );
          }
        }
        else
        {
      ...
      case CREA_ASSET_NUM_CBD:
        ...
        if( check_balance )
        {
          FC_ASSERT( acnt.get_cbd_rewards().amount.value >= 0, "Insufficient reward CBD funds" );
        }
        break;
      default:
        FC_ASSERT( false, "invalid symbol" );
...
void database::adjust_balance( const account_object& a, const asset& delta )
{
  ...
  bool check_balance = has_hardfork( CREA_HARDFORK_0_20__1811 );
  ...
  modify( a, [&]( account_object& acnt )
  {
    switch( delta.symbol.asset_num )
    {
      case CREA_ASSET_NUM_CREA:
        ...
        if( check_balance )
        {
          FC_ASSERT( acnt.get_savings().amount.value >= 0, "Insufficient savings CREA funds" );
        }
        break;
      case CREA_ASSET_NUM_CBD:
        if( a.savings_cbd_seconds_last_update != head_block_time() )
        {
          acnt.savings_cbd_seconds += fc::uint128_t(a.get_cbd_savings().amount.value) * (head_block_time() - a.savings_cbd_seconds_last_update).to_seconds();
        ...
        if( check_balance )
        {
          FC_ASSERT( acnt.get_cbd_savings().amount.value >= 0, "Insufficient savings CBD funds" );
        }
        break;
      default:
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
  ...
  bool check_supply = has_hardfork( CREA_HARDFORK_0_20__1811 );

  const auto& props = get_dynamic_global_properties();
  if( props.head_block_number < CREA_BLOCKS_PER_DAY*7 )
  ...
  modify( props, [&]( dynamic_global_property_object& props )
  {
    switch( delta.symbol.asset_num )
    {
      ...
      case CREA_ASSET_NUM_CBD:
        ...
        props.virtual_supply = props.get_current_cbd_supply() * get_feed_history().current_median_history + props.current_supply;
        if( check_supply )
        {
          FC_ASSERT( props.get_current_cbd_supply().amount.value >= 0 );
        }
        break;
      default:
        FC_ASSERT( false, "invalid symbol" );
...
void database::generate_optional_actions()
{
  ...
  CREA_TRY_NOTIFY( _generate_optional_actions_signal, note );
}

void database::init_hardforks()
{
  _hardfork_versions.times[ 0 ] = fc::time_point_sec( CREA_GENESIS_TIME );
  _hardfork_versions.versions[ 0 ] = hardfork_version( 0, 0 );
  FC_ASSERT( CREA_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_1 ] = fc::time_point_sec( CREA_HARDFORK_0_1_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_1 ] = CREA_HARDFORK_0_1_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_2 ] = fc::time_point_sec( CREA_HARDFORK_0_2_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_2 ] = CREA_HARDFORK_0_2_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_3 ] = fc::time_point_sec( CREA_HARDFORK_0_3_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_3 ] = CREA_HARDFORK_0_3_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_4 ] = fc::time_point_sec( CREA_HARDFORK_0_4_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_4 ] = CREA_HARDFORK_0_4_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_5 ] = fc::time_point_sec( CREA_HARDFORK_0_5_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_5 ] = CREA_HARDFORK_0_5_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_6 ] = fc::time_point_sec( CREA_HARDFORK_0_6_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_6 ] = CREA_HARDFORK_0_6_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_7 ] = fc::time_point_sec( CREA_HARDFORK_0_7_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_7 ] = CREA_HARDFORK_0_7_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_8 ] = fc::time_point_sec( CREA_HARDFORK_0_8_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_8 ] = CREA_HARDFORK_0_8_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_9 ] = fc::time_point_sec( CREA_HARDFORK_0_9_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_9 ] = CREA_HARDFORK_0_9_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_10 ] = fc::time_point_sec( CREA_HARDFORK_0_10_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_10 ] = CREA_HARDFORK_0_10_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_11 ] = fc::time_point_sec( CREA_HARDFORK_0_11_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_11 ] = CREA_HARDFORK_0_11_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_12 ] = fc::time_point_sec( CREA_HARDFORK_0_12_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_12 ] = CREA_HARDFORK_0_12_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_13 ] = fc::time_point_sec( CREA_HARDFORK_0_13_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_13 ] = CREA_HARDFORK_0_13_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_14 ] = fc::time_point_sec( CREA_HARDFORK_0_14_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_14 ] = CREA_HARDFORK_0_14_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_15 ] = fc::time_point_sec( CREA_HARDFORK_0_15_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_15 ] = CREA_HARDFORK_0_15_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_16 ] = fc::time_point_sec( CREA_HARDFORK_0_16_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_16 ] = CREA_HARDFORK_0_16_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_17 ] = fc::time_point_sec( CREA_HARDFORK_0_17_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_17 ] = CREA_HARDFORK_0_17_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_18 ] = fc::time_point_sec( CREA_HARDFORK_0_18_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_18 ] = CREA_HARDFORK_0_18_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_19 ] = fc::time_point_sec( CREA_HARDFORK_0_19_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_19 ] = CREA_HARDFORK_0_19_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_20 ] = fc::time_point_sec( CREA_HARDFORK_0_20_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_20 ] = CREA_HARDFORK_0_20_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_21 ] = fc::time_point_sec( CREA_HARDFORK_0_21_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_21 ] = CREA_HARDFORK_0_21_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_22 == 22, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_22 ] = fc::time_point_sec( CREA_HARDFORK_0_22_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_22 ] = CREA_HARDFORK_0_22_VERSION;
  FC_ASSERT( CREA_HARDFORK_0_23 == 23, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_0_23 ] = fc::time_point_sec( CREA_HARDFORK_0_23_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_0_23 ] = CREA_HARDFORK_0_23_VERSION;
  FC_ASSERT( CREA_HARDFORK_1_24 == 24, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_1_24 ] = fc::time_point_sec( CREA_HARDFORK_1_24_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_1_24 ] = CREA_HARDFORK_1_24_VERSION;
#ifdef IS_TEST_NET
  FC_ASSERT( CREA_HARDFORK_1_25 == 25, "Invalid hardfork configuration" );
  _hardfork_versions.times[ CREA_HARDFORK_1_25 ] = fc::time_point_sec( CREA_HARDFORK_1_25_TIME );
  _hardfork_versions.versions[ CREA_HARDFORK_1_25 ] = CREA_HARDFORK_1_25_VERSION;
#endif

  const auto& hardforks = get_hardfork_property_object();
  FC_ASSERT( hardforks.last_hardfork <= CREA_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("CREA_NUM_HARDFORKS",CREA_NUM_HARDFORKS) );
  FC_ASSERT( _hardfork_versions.versions[ hardforks.last_hardfork ] <= CREA_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
  FC_ASSERT( CREA_BLOCKCHAIN_HARDFORK_VERSION >= CREA_BLOCKCHAIN_VERSION );
  FC_ASSERT( CREA_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions.versions[ CREA_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
  try
  {
    ...
    if( has_hardfork( CREA_HARDFORK_0_5__54 ) )
    {
      ...
      {
        if( hardforks.last_hardfork < CREA_NUM_HARDFORKS ) {
    ...
    {
      while( hardforks.last_hardfork < CREA_NUM_HARDFORKS
          && _hardfork_versions.times[ hardforks.last_hardfork + 1 ] <= head_block_time()
          && hardforks.last_hardfork < CREA_HARDFORK_0_5__54 )
      {
        apply_hardfork( hardforks.last_hardfork + 1 );
      }
    }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
  ...
  for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= CREA_NUM_HARDFORKS; i++ )
  {
    if( i <= CREA_HARDFORK_0_5__54 )
...
void database::apply_hardfork( uint32_t hardfork )
{
  ...
  switch( hardfork )
  {
    ...
    case CREA_HARDFORK_0_2:
      ...
      break;
    case CREA_HARDFORK_0_3:
      retally_witness_votes();
      break;
    case CREA_HARDFORK_0_4:
      reset_virtual_schedule_time(*this);
      break;
    case CREA_HARDFORK_0_5:
      break;
    case CREA_HARDFORK_0_6:
      retally_witness_vote_counts();
      retally_comment_children();
      break;
    case CREA_HARDFORK_0_7:
      break;
    case CREA_HARDFORK_0_8:
      retally_witness_vote_counts(true);
      break;
    case CREA_HARDFORK_0_9:
      {
      ...
      break;
    case CREA_HARDFORK_0_10:
      retally_liquidity_weight();
      break;
    case CREA_HARDFORK_0_11:
      break;
    case CREA_HARDFORK_0_12:
      {
        ...
        for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
        {
          ...
          if( comment.is_root() )
          {
            ...
            if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
            {
              ...
              {
                c.cashout_time = head_block_time() + CREA_CASHOUT_WINDOW_SECONDS_PRE_HF17;
              });
            }
            ...
            else if( itr->last_payout > fc::time_point_sec() )
            {
              modify( *itr, [&]( comment_cashout_object& c )
              {
                c.cashout_time = c.last_payout + CREA_SECOND_CASHOUT_WINDOW;
              });
            }
          }
        }

        modify( get< account_authority_object, by_account >( CREA_MINER_ACCOUNT ), [&]( account_authority_object& auth )
        {
          auth.posting = authority();
          auth.posting.weight_threshold = 1;
        });

        modify( get< account_authority_object, by_account >( CREA_NULL_ACCOUNT ), [&]( account_authority_object& auth )
        {
          auth.posting = authority();
          auth.posting.weight_threshold = 1;
        });

        modify( get< account_authority_object, by_account >( CREA_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
        {
          auth.posting = authority();
          auth.posting.weight_threshold = 1;
        });
      ...
      break;
    case CREA_HARDFORK_0_13:
      break;
    case CREA_HARDFORK_0_14:
      break;
    case CREA_HARDFORK_0_15:
      break;
    case CREA_HARDFORK_0_16:
      {
        modify( get_feed_history(), [&]( feed_history_object& fho )
        {
          while( fho.price_history.size() > CREA_FEED_HISTORY_WINDOW )
            fho.price_history.pop_front();
        });
      }
      break;
    case CREA_HARDFORK_0_17:
      {
        static_assert(
          CREA_MAX_VOTED_WITNESSES_HF0 + CREA_MAX_MINER_WITNESSES_HF0 + CREA_MAX_RUNNER_WITNESSES_HF0 == CREA_MAX_WITNESSES,
          "HF0 witness counts must add up to CREA_MAX_WITNESSES" );
        static_assert(
          CREA_MAX_VOTED_WITNESSES_HF17 + CREA_MAX_MINER_WITNESSES_HF17 + CREA_MAX_RUNNER_WITNESSES_HF17 == CREA_MAX_WITNESSES,
          "HF17 witness counts must add up to CREA_MAX_WITNESSES" );

        modify( get_witness_schedule_object(), [&]( witness_schedule_object& wso )
        {
          wso.max_voted_witnesses = CREA_MAX_VOTED_WITNESSES_HF17;
          wso.max_miner_witnesses = CREA_MAX_MINER_WITNESSES_HF17;
          wso.max_runner_witnesses = CREA_MAX_RUNNER_WITNESSES_HF17;
        });

        const auto& gpo = get_dynamic_global_properties();

        auto& post_rf = create< reward_fund_object >( CREA_POST_REWARD_FUND_NAME, gpo.get_total_reward_fund_crea(), head_block_time()
#ifndef IS_TEST_NET
          , CREA_HF_17_RECENT_CLAIMS
#endif
          );
        ...
        */
        ...
        root_posts.reserve( CREA_HF_17_NUM_POSTS );
        vector< const comment_cashout_object* > replies;
        replies.reserve( CREA_HF_17_NUM_REPLIES );
        ...
        for( const auto& itr : root_posts )
        {
          modify( *itr, [&]( comment_cashout_object& c )
          {
            c.cashout_time = std::max( c.get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS, c.cashout_time );
          });
        }
        ...
        for( const auto& itr : replies )
        {
          modify( *itr, [&]( comment_cashout_object& c )
          {
            c.cashout_time = std::max( calculate_discussion_payout_time( c ), itr->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
          });
        }
      }
      break;
    case CREA_HARDFORK_0_18:
      break;
    case CREA_HARDFORK_0_19:
      {
        modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
        {
          gpo.vote_power_reserve_rate = CREA_REDUCED_VOTE_POWER_RATE;
        });

        modify( get< reward_fund_object, by_name >( CREA_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
        {
#ifndef IS_TEST_NET
          rfo.recent_claims = CREA_HF_19_RECENT_CLAIMS;
#endif
    ...
    case CREA_HARDFORK_0_20:
      {
        ...
        for( const auto& witness : wso.current_shuffled_witnesses )
        {
          ...
          if( witness != account_name_type() )
          {
            ...
            {
              w.props.account_creation_fee = asset( w.props.account_creation_fee.amount * CREA_CREATE_ACCOUNT_WITH_CREA_MODIFIER, CREA_SYMBOL );
            });
          }
        }
        ...
        {
          wso.median_props.account_creation_fee = asset( wso.median_props.account_creation_fee.amount * CREA_CREATE_ACCOUNT_WITH_CREA_MODIFIER, CREA_SYMBOL );
        });
      }
      break;
    case CREA_HARDFORK_0_21:
    {
      modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
      {
        gpo.sps_fund_percent = CREA_PROPOSAL_FUND_PERCENT_HF21;
        gpo.content_reward_percent = CREA_CONTENT_REWARD_PERCENT_HF21;
        gpo.downvote_pool_percent = CREA_DOWNVOTE_POOL_PERCENT_HF21;
        gpo.reverse_auction_seconds = CREA_REVERSE_AUCTION_WINDOW_SECONDS_HF21;
      });
      ...
      modify( get< reward_fund_object, by_name >( CREA_POST_REWARD_FUND_NAME ), [&]( reward_fund_object& rfo )
      {
        rfo.percent_curation_rewards = 50 * CREA_1_PERCENT;
        rfo.author_reward_curve = convergent_linear;
        rfo.curation_reward_curve = convergent_square_root;
        rfo.content_constant = CREA_CONTENT_CONSTANT_HF21;
#ifndef  IS_TEST_NET
        rfo.recent_claims = CREA_HF21_CONVERGENT_LINEAR_RECENT_CLAIMS;
#endif
      });
    }
    break;
    case CREA_HARDFORK_0_22:
      break;
    case CREA_HARDFORK_0_23:
    {
    ...
    case CREA_SMT_HARDFORK:
    {
      ...
      modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
      {
        gpo.required_actions_partition_percent = 25 * CREA_1_PERCENT;
      });
...
void database::validate_invariants()const
{
  try
  {
    ...
    for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
    {
      ...
      total_cbd += itr->get_cbd_balance();
      total_cbd += itr->get_cbd_savings();
      total_cbd += itr->get_cbd_rewards();
      total_vesting += itr->get_vesting();
      total_vesting += itr->get_vest_rewards();
      pending_vesting_crea += itr->get_vest_rewards_as_crea();
      total_vsf_votes += ( itr->proxy == CREA_PROXY_TO_SELF_ACCOUNT ?
                      itr->witness_vote_weight() :
                      ( CREA_MAX_PROXY_RECURSION_DEPTH > 0 ?
                          itr->proxied_vsf_votes[CREA_MAX_PROXY_RECURSION_DEPTH - 1] :
    ...
    {
      if( itr->sell_price.base.symbol == CREA_SYMBOL )
      {
    ...
    {
      total_supply += itr->get_crea_balance();
      total_cbd += itr->get_cbd_balance();

      if( itr->get_fee().symbol == CREA_SYMBOL )
        total_supply += itr->get_fee();
      else if( itr->get_fee().symbol == CBD_SYMBOL )
        total_cbd += itr->get_fee();
      else
    ...
    total_supply += gpo.get_total_vesting_fund_crea() + gpo.get_total_reward_fund_crea() + gpo.get_pending_rewarded_vesting_crea();

    FC_ASSERT( gpo.current_supply == total_supply, "", ("gpo.current_supply",gpo.current_supply)("total_supply",total_supply) );
    FC_ASSERT( gpo.get_current_cbd_supply() + gpo.get_init_cbd_supply() == total_cbd, "", ("gpo.current_cbd_supply",gpo.get_current_cbd_supply())("total_cbd",total_cbd) );
    ...
    FC_ASSERT( gpo.get_pending_rewarded_vesting_crea() == pending_vesting_crea, "", ("pending_rewarded_vesting_crea",gpo.get_pending_rewarded_vesting_crea())("pending_vesting_crea", pending_vesting_crea));

    FC_ASSERT( gpo.virtual_supply >= gpo.current_supply );
    if ( !get_feed_history().current_median_history.is_null() )
    {
      FC_ASSERT( gpo.get_current_cbd_supply()* get_feed_history().current_median_history + gpo.current_supply
        == gpo.virtual_supply, "", ("gpo.current_cbd_supply",gpo.get_current_cbd_supply())("get_feed_history().current_median_history",get_feed_history().current_median_history)("gpo.current_supply",gpo.current_supply)("gpo.virtual_supply",gpo.virtual_supply) );
    }
    ...
    ilog( "CREA supply: ${h}", ( "h", gpo.current_supply.amount.value ) );
    ilog( "CBD supply: ${s} ( + ${i} initial )", ( "s", gpo.get_current_cbd_supply().amount.value )( "i", gpo.get_init_cbd_supply().amount.value ) );
    ilog( "virtual supply (CREA): ${w}", ( "w", gpo.virtual_supply.amount.value ) );
    ilog( "VESTS: ${v} ( + ${p} pending ) worth (CREA): ${x} ( + ${y} )", ( "v", gpo.total_vesting_shares.amount.value )( "p", gpo.pending_rewarded_vesting_shares.amount.value )( "x", gpo.get_total_vesting_fund_crea().amount.value )( "y", gpo.get_pending_rewarded_vesting_crea().amount.value ) );
  }
...
#pragma message( "TODO: Add reward_fund_object iteration here once they support SMTs." )
    ...
    for( ; itr != end; ++itr )
    {
      ...
      total_liquid_supply += asset( smt.total_vesting_fund_smt, smt.liquid_symbol )
                    /*+ gpo.get_total_reward_fund_crea() */
  ...
  try
  {
    ...
    for( const auto& account : get_index< account_index, by_id >() )
    {
      modify( account, [&]( account_object& a )
      {
        ...
        a.vesting_withdraw_rate  = asset( a.to_withdraw / CREA_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
        if( a.vesting_withdraw_rate.amount == 0 )
          a.vesting_withdraw_rate.amount = 1;

        for( uint32_t i = 0; i < CREA_MAX_PROXY_RECURSION_DEPTH; ++i )
...
void database::retally_witness_votes()
{
  ...
  for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
  {
    if( itr->proxy != CREA_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
  ...
  for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
  {
    ...
    if( force || (a.proxy != CREA_PROXY_TO_SELF_ACCOUNT  ) )
    {
      const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
      auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
      while( wit_itr != vidx.end() && wit_itr->account == a.name )
