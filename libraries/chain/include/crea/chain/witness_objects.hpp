...
namespace crea { namespace chain {
  ...
  {
    /**
      *  This fee, paid in CREA, is converted into VESTING SHARES for the new account. Accounts
      *  without vesting shares cannot earn usage rations and therefore are powerless. This minimum
      *  fee requires all accounts to have some kind of commitment to the network that includes the
      *  ability to vote and make transactions.
      */
    asset             account_creation_fee =
      asset( CREA_MIN_ACCOUNT_CREATION_FEE, CREA_SYMBOL );
  ...
  class witness_object : public object< witness_object_type, witness_object >
  {
    ...
    public:
      ...
      //CBD to CREA ratio proposed by the witness
      const price& get_cbd_exchange_rate() const { return cbd_exchange_rate; }
      //time when CBD/CREA price ratio was last confirmed (TODO: add routine to check if price feed is valid)
      const time_point_sec& get_last_cbd_exchange_update() const { return last_cbd_exchange_update; }
...
template<> struct is_static_length< crea::chain::witness_vote_object > : public boost::true_type {};
template<> struct is_static_length< crea::chain::witness_schedule_object > : public boost::true_type {};
