...
namespace chain {
  ...
  {
    fc::time_point_sec         times[ CREA_NUM_HARDFORKS + 1 ];
    protocol::hardfork_version versions[ CREA_NUM_HARDFORKS + 1 ];
  };
  ...
  struct open_args
    ...
    uint64_t initial_supply = CREA_INIT_SUPPLY;
    uint64_t cbd_initial_supply = CREA_CBD_INIT_SUPPLY;
  ...
  class database : public chainbase::database
  {
    ...
    public:
      ...
      chain_id_type crea_chain_id = CREA_CHAIN_ID;
    ...
    public:
      ...
      std::pair< asset, asset > create_cbd( const account_object& to_account, asset hive, bool to_reward_balance=false );
      ...
      void adjust_total_payout( const comment_cashout_object& a, const asset& hbd, const asset& curator_cbd_value, const asset& beneficiary_value );

      void adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_cbd );
      ...
      /**
        ...
        */
      asset to_cbd( const asset& hive )const;
      asset to_crea( const asset& hbd )const;
      ...
      void init_schema();
      void init_genesis(uint64_t initial_supply = CREA_INIT_SUPPLY, uint64_t cbd_initial_supply = CREA_CBD_INIT_SUPPLY );
      ...
      void clear_account( const account_object& account,
        asset* transferred_cbd_ptr = nullptr, asset* transferred_crea_ptr = nullptr,
        asset* converted_vests_ptr = nullptr, asset* crea_from_vests_ptr = nullptr );
  ...
  protected:
    ...
    private:
      ...
      bool collect_account_total_balance( const account_object& account, asset* total_crea, asset* total_cbd,
  ...
  struct prepare_snapshot_supplement_notification
  {
    prepare_snapshot_supplement_notification(const fc::path& _external_data_storage_base_path, crea::plugins::chain::snapshot_dump_helper& _dump_helper) :
    ...
    crea::plugins::chain::snapshot_dump_helper& dump_helper;
  };

  struct load_snapshot_supplement_notification
  {
    explicit load_snapshot_supplement_notification(crea::plugins::chain::snapshot_load_helper& _load_helper) :
      load_helper(_load_helper) {}

    crea::plugins::chain::snapshot_load_helper& load_helper;
  };
