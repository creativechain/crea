   ...
   const dynamic_global_property_object& props, bool mined, const account_name_type& recovery_account, uint32_t hardfork )
{
   ...
   if( hardfork >= CREA_HARDFORK_0_11 )
   {
      ...
      if( ( hardfork < CREA_HARDFORK_0_20__1782 ) || ( recovery_account != CREA_TEMP_ACCOUNT ) )
      {
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( _db.has_hardfork( CREA_HARDFORK_0_6__80 ) && comment.parent_author != CREA_ROOT_POST_PARENT )
   {
...
{ try {
   ...
   if( o.parent_author != CREA_ROOT_POST_PARENT )
   {
      ...
      if( !_db.has_hardfork( CREA_HARDFORK_0_17__767 ) )
         FC_ASSERT( parent->depth < CREA_MAX_COMMENT_DEPTH_PRE_HF17, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CREA_MAX_COMMENT_DEPTH_PRE_HF17) );
      else
         FC_ASSERT( parent->depth < CREA_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}.", ("x",parent->depth)("y",CREA_MAX_COMMENT_DEPTH) );
   }
   ...
   if ( itr == by_permlink_idx.end() )
   {
      ...
      {
         if( o.parent_author == CREA_ROOT_POST_PARENT )
             FC_ASSERT( ( now - auth.last_root_post ) > CREA_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("last_root_post", auth.last_root_post) );
         else
             FC_ASSERT( (now - auth.last_post) >= CREA_MIN_REPLY_INTERVAL_HF20, "You may only comment once every 3 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( CREA_HARDFORK_0_12__176 ) )
      {
         if( o.parent_author == CREA_ROOT_POST_PARENT )
             ...
             FC_ASSERT( (now - auth.last_post) > CREA_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else if( _db.has_hardfork( CREA_HARDFORK_0_6__113 ) )
      {
         if( o.parent_author == CREA_ROOT_POST_PARENT )
             FC_ASSERT( (now - auth.last_post) > CREA_MIN_ROOT_COMMENT_INTERVAL, "You may only post once every 5 minutes.", ("now",now)("auth.last_post",auth.last_post) );
         else
             FC_ASSERT( (now - auth.last_post) > CREA_MIN_REPLY_INTERVAL, "You may only comment once every 20 seconds.", ("now",now)("auth.last_post",auth.last_post) );
      }
      else
      ...
      {
         uint64_t post_delta_time = std::min( _db.head_block_time().sec_since_epoch() - auth.last_root_post.sec_since_epoch(), CREA_POST_AVERAGE_WINDOW );
      ...
      const auto& new_comment = _db.create< comment_object >( [&]( comment_object& com )
      {
         ...
         if ( o.parent_author == CREA_ROOT_POST_PARENT )
         {
            ...
            com.cashout_time = _db.has_hardfork( CREA_HARDFORK_0_12__177 ) ?
               _db.head_block_time() + CREA_CASHOUT_WINDOW_SECONDS_PRE_HF17 :
...
void withdraw_vesting_evaluator::do_apply( const withdraw_vesting_operation& o )
{
   ...
   FC_ASSERT( account.vesting_shares - account.delegated_vesting_shares >= o.vesting_shares, "Account does not have sufficient Crea Energy for withdraw." );
...
void pre_hf20_vote_evaluator( const vote_operation& o, database& _db )
{
   ...
   if( _db.has_hardfork( CREA_HARDFORK_0_12__177 ) && _db.calculate_discussion_payout_time( comment ) == fc::time_point_sec::maximum() )
   {
#ifndef CLEAR_VOTES
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         if( _db.has_hardfork( CREA_HARDFORK_0_17__900 ) )
            FC_ASSERT( _db.head_block_time() < comment.cashout_time - CREA_UPVOTE_LOCKOUT_HF17, "Cannot increase payout within last twelve hours before payout." );
         else if( _db.has_hardfork( CREA_HARDFORK_0_7 ) )
            FC_ASSERT( _db.head_block_time() < _db.calculate_discussion_payout_time( comment ) - CREA_UPVOTE_LOCKOUT_HF7, "Cannot increase payout within last minute before payout." );
      }
      ...
      _db.modify( comment, [&]( comment_object& c ){
         ...
         if( rshares > 0 )
            ...
            c.net_votes--;
         if( !_db.has_hardfork( CREA_HARDFORK_0_6__114 ) && c.net_rshares == -c.abs_rshares) FC_ASSERT( c.net_votes < 0, "Comment has negative net votes?" );
      });
      ...
      **/
      _db.create<comment_vote_object>( [&]( comment_vote_object& cv ){
         ...
         {
            if( comment.created < fc::time_point_sec(CREA_HARDFORK_0_6_REVERSE_AUCTION_TIME) ) {
               ...
               if( _db.has_hardfork( CREA_HARDFORK_0_17__774 ) )
               {
                  ...
                  auto curve = !_db.has_hardfork( CREA_HARDFORK_0_19__1052 ) && comment.created > CREA_HF_19_SQRT_PRE_CALC
...
void custom_evaluator::do_apply( const custom_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_20 ) )
   {
      FC_ASSERT( o.required_auths.size() <= CREA_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", o.required_auths.size()) );
   }
}
...
void custom_json_evaluator::do_apply( const custom_json_operation& o )
{
   ...
   if( _db.is_producing() || _db.has_hardfork( CREA_HARDFORK_0_20 ) )
   {
      ...
      FC_ASSERT( num_auths <= CREA_MAX_AUTHORITY_MEMBERSHIP, "Too many auths specified. Max: 10, Current: ${n}", ("n", num_auths) );
   }
...
void limit_order_create_evaluator::do_apply( const limit_order_create_operation& o )
{
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       else
       {
          ...
          obj.expiration = std::min( o.expiration, fc::time_point_sec( CREA_HARDFORK_0_20_TIME + CREA_MAX_LIMIT_ORDER_EXPIRATION + rand_offset ) );
       }
   });
...
void limit_order_create2_evaluator::do_apply( const limit_order_create2_operation& o )
{
   ...
   const auto& order = _db.create<limit_order_object>( [&]( limit_order_object& obj )
   {
       ...
       {
          obj.expiration = std::min( o.expiration, fc::time_point_sec( CREA_HARDFORK_0_20_TIME + CREA_MAX_LIMIT_ORDER_EXPIRATION ) );
       }
   });
...
void transfer_from_savings_evaluator::do_apply( const transfer_from_savings_operation& op )
{
   ...
   _db.create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
      ...
      s.complete = _db.head_block_time() + CREA_SAVINGS_WITHDRAW_TIME;
   });
