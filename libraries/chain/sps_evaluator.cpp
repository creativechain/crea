...
using crea::chain::create_proposal_evaluator;

void create_proposal_evaluator::do_apply( const create_proposal_operation& o )
{
  try
  {
    FC_ASSERT( _db.has_hardfork( CREA_PROPOSALS_HARDFORK ), "Proposals functionality not enabled until hardfork ${hf}", ("hf", CREA_PROPOSALS_HARDFORK) );
    ...
    */
    ...
    asset fee_cbd( CREA_TREASURY_FEE, CBD_SYMBOL );

    if(_db.has_hardfork(CREA_HARDFORK_1_24))
    {
      uint32_t proposal_run_time = o.end_date.sec_since_epoch() - o.start_date.sec_since_epoch();

      if(proposal_run_time > CREA_PROPOSAL_FEE_INCREASE_DAYS_SEC)
      {
        uint32_t extra_days = (proposal_run_time / CREA_ONE_DAY_SECONDS) - CREA_PROPOSAL_FEE_INCREASE_DAYS;
        fee_cbd += asset(CREA_PROPOSAL_FEE_INCREASE_AMOUNT * extra_days, CBD_SYMBOL);
      }
    }
...
void update_proposal_evaluator::do_apply( const update_proposal_operation& o )
{
  try
  {
    FC_ASSERT( _db.has_hardfork( CREA_HARDFORK_1_24 ), "The update proposal functionality not enabled until hardfork ${hf}", ("hf", CREA_HARDFORK_1_24) );
...
void remove_proposal_evaluator::do_apply(const remove_proposal_operation& op)
{
  try
  {
    ...
    /*
      ...
      Therefore all proposals are marked with flag `removed` and `end_date` is moved beyond 'head_time + CREA_PROPOSAL_MAINTENANCE_CLEANUP`
    ...
    */
    for( const auto pid : op.proposal_ids )
    {
      ...
      _db.modify( *found_id, [&]( proposal_object& proposal )
      {
        ...
        auto new_end_date = head_date - fc::seconds( CREA_PROPOSAL_MAINTENANCE_CLEANUP );
