...
/*#define CREA_REQUIRE_THROW( expr, exc_type )            \
{                                                         \
  ...
  if( fc::enable_record_assert_trip )                    \
    std::cout << "CREA_REQUIRE_THROW begin "            \
      << req_throw_info << std::endl;                  \
  BOOST_REQUIRE_THROW( expr, exc_type );                 \
  if( fc::enable_record_assert_trip )                    \
    std::cout << "CREA_REQUIRE_THROW end "              \
      << req_throw_info << std::endl;                  \
}*/
...
#define CREA_CHECK_THROW( expr, exc_type )                \
{                                                         \
  std::string req_throw_info = fc::json::to_string(      \
    fc::mutable_variant_object()                        \
    ("source_file", __FILE__)                           \
  ...
  if( fc::enable_record_assert_trip )                    \
    std::cout << "CREA_CHECK_THROW begin "              \
      << req_throw_info << std::endl;                  \
  BOOST_CHECK_THROW( expr, exc_type );                   \
  if( fc::enable_record_assert_trip )                    \
    std::cout << "CREA_CHECK_THROW end "                \
      << req_throw_info << std::endl;                  \
}
...
{ \
  const auto temp = op.field; \
  op.field = value; \
  CREA_REQUIRE_THROW( op.validate(), exc_type ); \
  op.field = temp; \
}
...
{ \
  ...
  trx.operations.back() = op; \
  op.field = bak; \
  CREA_REQUIRE_THROW(db.push_transaction(trx, ~0), exc_type); \
}
...
struct database_fixture {
  ...
  asset get_cbd_balance( const string& account_name )const;
  asset get_savings( const string& account_name )const;
  asset get_cbd_savings( const string& account_name )const;
  asset get_rewards( const string& account_name )const;
  asset get_cbd_rewards( const string& account_name )const;
  asset get_vesting( const string& account_name )const;
  asset get_vest_rewards( const string& account_name )const;
  asset get_vest_rewards_as_crea( const string& account_name )const;
