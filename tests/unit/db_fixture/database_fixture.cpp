...
#include <crea/utilities/database_configuration.hpp>
...
#include <crea/chain/smt_objects/nai_pool_object.hpp>
...
typedef crea::plugins::account_history::account_history_plugin ah_plugin;
//typedef crea::plugins::account_history_rocksdb::account_history_rocksdb_plugin ah_plugin;
...
clean_database_fixture::clean_database_fixture( uint16_t shared_file_size_in_mb )
{
  ...
  appbase::app().register_plugin< crea::plugins::rc::rc_plugin >();
  ...
  crea::plugins::rc::rc_plugin_skip_flags rc_skip;
  ...
  appbase::app().get_plugin< crea::plugins::rc::rc_plugin >().set_rc_plugin_skip_flags( rc_skip );
  ...
  generate_block();
  db->set_hardfork( CREA_BLOCKCHAIN_VERSION.minor_v() );
  generate_block();

  vest( "initminer", 10000 );

  // Fill up the rest of the required miners
  for( int i = CREA_NUM_INIT_MINERS; i < CREA_MAX_WITNESSES; i++ )
  {
    account_create( CREA_INIT_MINER_NAME + fc::to_string( i ), init_account_pub_key );
    fund( CREA_INIT_MINER_NAME + fc::to_string( i ), CREA_MIN_PRODUCER_REWARD.amount.value );
    witness_create( CREA_INIT_MINER_NAME + fc::to_string( i ), init_account_priv_key, "foo.bar", init_account_pub_key, CREA_MIN_PRODUCER_REWARD.amount );
  }
...
void clean_database_fixture::validate_database()
{
  ...
  appbase::app().get_plugin< crea::plugins::rc::rc_plugin >().validate_database();
}
...
void clean_database_fixture::resize_shared_mem( uint64_t size )
{
...

  {
    ...
    args.database_cfg = crea::utilities::default_database_configuration();
    db->open( args );
  }
...
void database_fixture::generate_blocks(fc::time_point_sec timestamp, bool miss_intermediate_blocks)
{
  ...
  BOOST_REQUIRE( ( db->head_block_time() - timestamp ).to_seconds() < CREA_BLOCK_INTERVAL );
}
  ...
  )
{
  try
  {
    ...
    op.creator = creator;
    op.fee = asset( actual_fee, CREA_SYMBOL );
    ...
    trx.set_expiration( db->head_block_time() + CREA_MAX_TIME_UNTIL_EXPIRATION );
    ...
    if( fee_remainder > 0 )
    {
      vest( CREA_INIT_MINER_NAME, name, asset( fee_remainder, CREA_SYMBOL ) );
    }
...
)
{
  try
  {
    return account_create(
      ...
      std::max( db->get_witness_schedule_object().median_props.account_creation_fee.amount * CREA_CREATE_ACCOUNT_WITH_CREA_MODIFIER, share_type( 100 ) ),
      key,
      post_key,
      "" );
  }
  ...
  )
{
  try
  {
    transfer( CREA_INIT_MINER_NAME, account_name, asset( amount, CREA_SYMBOL ) );
    ...
    db_plugin->debug_update( [=]( database& db)
    {
      ...
      if( amount.symbol == CBD_SYMBOL )
      {
        ...
        if( median_feed.current_median_history.is_null() )
          ...
          {
            f.current_median_history = price( asset( 1, CBD_SYMBOL ), asset( 1, CREA_SYMBOL ) );
          });
      }
...
void database_fixture::vest( const string& from, const string& to, const asset& amount )
{
  try
  {
    FC_ASSERT( amount.symbol == CREA_SYMBOL, "Can only vest TESTS" );
...
void database_fixture::vest( const string& from, const share_type& amount )
{
  try
  {
    ...
    op.from = from;
    op.to = "";
    op.amount = asset( amount, CREA_SYMBOL );
...
void database_fixture::set_price_feed( const price& new_price )
{
  for( size_t i = 1; i < 8; i++ )
  {
    ...
    op.owner = CREA_INIT_MINER_NAME + fc::to_string( i );
  ...
  generate_blocks( CREA_BLOCKS_PER_HOUR );
...
{
  trx.clear();
  for( size_t i=0; i<CREA_MAX_WITNESSES; i++ )
  {
    witness_set_properties_operation op;
    op.owner = CREA_INIT_MINER_NAME + (i == 0 ? "" : fc::to_string( i ));
    op.props = props;
  ...
  for( size_t i=0; i<2*CREA_MAX_WITNESSES+1; i++ )
  {
    generate_block();
  ...
  FC_ASSERT( false, "Couldn't apply properties in ${n} blocks", ("n", 2*CREA_MAX_WITNESSES+1) );
}
...
asset database_fixture::get_cbd_balance( const string& account_name )const
{
  return db->get_account( account_name ).get_cbd_balance();
}
...
asset database_fixture::get_cbd_savings( const string& account_name )const
{
  return db->get_account( account_name ).get_cbd_savings();
}
...
asset database_fixture::get_cbd_rewards( const string& account_name )const
{
  return db->get_account( account_name ).get_cbd_rewards();
}
...
asset database_fixture::get_vest_rewards_as_crea( const string& account_name )const
{
  return db->get_account( account_name ).get_vest_rewards_as_crea();
}
...
void push_invalid_operation(const operation& invalid_op, const fc::ecc::private_key& key, database* db)
{
  ...
  CREA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::assert_exception );
}
...
void t_smt_database_fixture< T >::create_invalid_smt( const char* control_account_name, const fc::ecc::private_key& key )
{
  ...
  CREA_REQUIRE_THROW( set_create_op( &op_precision, control_account_name, CREA_ASSET_MAX_DECIMALS + 1, *this->db ), fc::assert_exception );
}
...
void sps_proposal_database_fixture::plugin_prepare()
{
  ...
  generate_block();
  db->set_hardfork( CREA_NUM_HARDFORKS );
  generate_block();
...
uint64_t sps_proposal_database_fixture::get_nr_blocks_until_maintenance_block()
{
  ...
  auto ret = ( next_maintenance_time - block_time ).to_seconds() / CREA_BLOCK_INTERVAL;
