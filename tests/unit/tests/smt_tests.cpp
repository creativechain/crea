...
BOOST_AUTO_TEST_CASE( asset_symbol_vesting_methods )
{
  try
  {
    ...
    asset_symbol_type Hive = CREA_SYMBOL;
    ...
    asset_symbol_type Hbd = CBD_SYMBOL;
    FC_ASSERT( Hbd.is_vesting() == false );
    FC_ASSERT( Hbd.get_paired_symbol() == CBD_SYMBOL );
...
BOOST_AUTO_TEST_CASE( setup_validate )
{
  try
  {
    ...
    op.contribution_begin_time = CREA_GENESIS_TIME;
    CREA_REQUIRE_THROW( op.validate(), fc::exception );
    ...
    op.crea_units_soft_cap = SMT_MIN_SOFT_CAP_CREA_UNITS;
    op.crea_units_hard_cap = SMT_MIN_HARD_CAP_CREA_UNITS;
    op.validate();
...
BOOST_AUTO_TEST_CASE( smt_create_apply )
{
  try
  {
    ...
    BOOST_TEST_MESSAGE( " -- SMT create with insufficient CREA balance" );
    ...
    BOOST_TEST_MESSAGE( " -- Check that an SMT cannot be created with decimals greater than CREA_MAX_DECIMALS" );
    ...
    BOOST_TEST_MESSAGE( " -- Check that we cannot create an SMT with an insufficent CREA creation fee" );
