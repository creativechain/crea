...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_authorities )
{
  try
  {
    ...
    op.expiration = db->head_block_time() + fc::seconds( CREA_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( db->get_balance( bob_account, CREA_SYMBOL ).amount.value == bob_balance.amount.value );
    ...
    BOOST_REQUIRE( limit_order->get_market() == std::make_pair( alice_symbol, CREA_SYMBOL ) );
    BOOST_REQUIRE( db->get_balance( alice_account, alice_symbol ).amount.value == alice_smt_balance.amount.value );
    BOOST_REQUIRE( db->get_balance( alice_account, CREA_SYMBOL ).amount.value == alice_balance.amount.value );
    validate_database();
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( get_balance( "alice" ) == alice_crea + ASSET( "0.000 TESTS" ) );
    BOOST_REQUIRE( get_rewards( "alice" ) == ASSET( "10.000 TESTS" ) );
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == alice_cbd + ASSET( "0.000 TBD" ) );
    ...
    // Legacy symbols
    asset full_crea = ASSET( "10.000 TESTS" );
    asset full_cbd = ASSET( "10.000 TBD" );
    op.reward_tokens.push_back( full_cbd );
    op.reward_tokens.push_back( full_crea );
    op.reward_tokens.push_back( partial_vests );
    PUSH_OP(op, alice_private_key);
    BOOST_REQUIRE( get_balance( "alice" ) == alice_crea + full_crea );
    BOOST_REQUIRE( get_rewards( "alice" ) == ASSET( "0.000 TESTS" ) );
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == alice_cbd + full_cbd );
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
  try
  {
    ...
    BOOST_TEST_MESSAGE( " -- Valid maximum SMT creation fee (CREA_MAX_SHARE_SUPPLY)" );
    ...
    BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: CREA cannot be an SMT" );
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate )
{
  try
  {
    ...
    CREA_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
  }
  FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_differing_decimals )
{
  try
  {
    ...
    // Fail on duplicate SMT lookup
    CREA_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), 2 /* Decimals */ ), fc::assert_exception)
  }
  FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_different_users )
{
  try
  {
    ...
    // Fail on duplicate SMT lookup
    CREA_REQUIRE_THROW( create_smt_with_nai( "bob", bob_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
  }
  FC_LOG_AND_RETHROW();
}

BOOST_AUTO_TEST_CASE( smt_create_with_crea_funds )
{
  try
  {
    BOOST_TEST_MESSAGE( "Testing: smt_create_with_crea_funds" );
...
BOOST_AUTO_TEST_CASE( smt_create_with_invalid_nai )
{
  try
  {
    ...
    CREA_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, ast.to_nai(), ast.decimals() ), fc::assert_exception)
  }
  FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_creation_fee_test )
{
  try
  {
    ...
    // This ensures that our actual smt_creation_fee is sane in production (either CREA or CBD)
    ...
    for ( int i = 0; i < 2; i++ )
    {
      ...
      if ( !i ) // First pass
        ...
        {
          dgpo.smt_creation_fee = asset( 2000, CREA_SYMBOL );
        } );
      else // Second pass
        db->modify( dgpo, [&] ( dynamic_global_property_object& dgpo )
        {
      ...
      BOOST_TEST_MESSAGE( " -- Valid creation fee, using CREA" );
      // We should be able to pay with CREA
      smt_create_operation op;
      op.control_account = "alice";
...
BOOST_AUTO_TEST_CASE( smt_create_reset )
{
  try
  {
    ...
    rewards.percent_curation_rewards = 15 * CREA_1_PERCENT;
...

    {
      ...
      BOOST_REQUIRE( token.cashout_window_seconds == CREA_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( token.reverse_auction_window_seconds == CREA_REVERSE_AUCTION_WINDOW_SECONDS_HF20 );
      BOOST_REQUIRE( token.vote_regeneration_period_seconds == CREA_VOTING_MANA_REGENERATION_SECONDS );
      BOOST_REQUIRE( token.votes_per_regeneration_period == SMT_DEFAULT_VOTES_PER_REGEN_PERIOD );
      BOOST_REQUIRE( token.content_constant == CREA_CONTENT_CONSTANT_HF0 );
    ...
    tx.set_expiration( db->head_block_time() + CREA_BLOCK_INTERVAL * 10 );
    tx.signatures.clear();
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
  }
...
BOOST_AUTO_TEST_CASE( smt_nai_pool_count )
{
  try
  {
    ...
    for ( unsigned int i = 1; i <= SMT_MAX_NAI_POOL_COUNT; i++ )
    {
      ...
      tx.operations.push_back( op );
      tx.set_expiration( this->db->head_block_time() + CREA_MAX_TIME_UNTIL_EXPIRATION );
    ...
    CREA_REQUIRE_THROW( get_new_smt_symbol( 0, this->db ), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( smt_setup_emissions_validate )
{
  try
  {
    ...
    op.symbol = CREA_SYMBOL;
    op.lep_abs_amount = asset( 0, CREA_SYMBOL );
    op.rep_abs_amount = asset( 0, CREA_SYMBOL );
    ...
    op.schedule_time = CREA_GENESIS_TIME;
    CREA_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_validate )
{
  try
  {
    ...
    op.symbol = CREA_SYMBOL;
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
    op.symbol = VESTS_SYMBOL;
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
    op.symbol = CBD_SYMBOL;
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
    ...
    op.control_account = "####";
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid account name
    op.control_account = "alice";

    op.setup_parameters.clear();
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Empty setup parameters
    op.setup_parameters.emplace( smt_param_allow_voting { .value = true } );
...
BOOST_AUTO_TEST_CASE( smt_set_runtime_parameters_validate )
{
  try
  {
    ...
    smt_param_rewards_v1 rewards;
    rewards.content_constant = CREA_CONTENT_CONSTANT_HF0;
    rewards.percent_curation_rewards = CREA_100_PERCENT + 1;
    ...
    rewards.percent_curation_rewards = CREA_100_PERCENT;
    op.runtime_parameters.clear();
    op.runtime_parameters.insert( rewards );
    op.validate();
...
BOOST_AUTO_TEST_CASE( smt_contribute_validate )
{
  try
  {
    ...
    op.contribution = asset( 1000, CREA_SYMBOL );
    ...
    op.contribution = asset( -1, CREA_SYMBOL );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
    op.contribution = asset( 1000, CREA_SYMBOL );

    BOOST_TEST_MESSAGE( " -- Failure on no contribution" );
    op.contribution = asset( 0, CREA_SYMBOL );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
    op.contribution = asset( 1000, CREA_SYMBOL );
    ...
    BOOST_TEST_MESSAGE( " -- Failure on contribution to CREA" );
    op.symbol = CREA_SYMBOL;
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
    op.symbol = new_symbol;
...
BOOST_AUTO_TEST_CASE( smt_contribute_apply )
{
  try
  {
    ...
    auto alice_asset_accumulator = asset( 0, CREA_SYMBOL );
    auto bob_asset_accumulator = asset( 0, CREA_SYMBOL );
    auto sam_asset_accumulator = asset( 0, CREA_SYMBOL );
    ...
    db_plugin->debug_update( [=] ( database& db )
    {
      ...
      db.create< smt_ico_object >( [&]( smt_ico_object& o )
      {
        ...
        o.crea_units_soft_cap = SMT_MIN_SOFT_CAP_CREA_UNITS;
        o.crea_units_hard_cap = 99000;
      } );
    } );

    smt_contribute_operation bob_op;
    bob_op.contributor = "bob";
    bob_op.contribution = asset( 1000, CREA_SYMBOL );
    ...
    alice_op.contribution = asset( 2000, CREA_SYMBOL );
    ...
    sam_op.contribution = asset( 3000, CREA_SYMBOL );
    ...
    auto alices_contributions = asset( 0, CREA_SYMBOL );
    auto bobs_contributions = asset( 0, CREA_SYMBOL );
    auto sams_contributions = asset( 0, CREA_SYMBOL );
    ...
    BOOST_REQUIRE( db->get_balance( "alice", CREA_SYMBOL ) == ASSET( "1000.000 TESTS" ) - alice_asset_accumulator );
    BOOST_REQUIRE( db->get_balance( "bob", CREA_SYMBOL ) == ASSET( "1000.000 TESTS" ) - bob_asset_accumulator );
    BOOST_REQUIRE( db->get_balance( "sam", CREA_SYMBOL ) == ASSET( "1000.000 TESTS" ) - sam_asset_accumulator );
...
BOOST_AUTO_TEST_CASE( smt_transfer_apply )
{
  try
  {
    ...
    op.to = NEW_CREA_TREASURY_ACCOUNT;
    tx.operations.push_back( op );
    tx.set_expiration( db->head_block_time() + CREA_MAX_TIME_UNTIL_EXPIRATION );
    sign( tx, alice_private_key );
