...
BOOST_AUTO_TEST_CASE( blocked_operations )
{
  try
  {
    ...
    //check withdraw vesting route
    {
      ...
      op.percent = 50 * CREA_1_PERCENT;
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      BOOST_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );

      tx.clear();
      op.to_account = NEW_CREA_TREASURY_ACCOUNT;
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      BOOST_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
    }
    ...
    //check transfer from savings of non-CBD
    {
...
BOOST_AUTO_TEST_CASE( comment_beneficiary )
{
  try
  {
...

    {
      ...
      b.beneficiaries.push_back( beneficiary_route_type( OBSOLETE_TREASURY_ACCOUNT, CREA_100_PERCENT ) );
      op.author = "alice";
      op.permlink = "test";
    ...
    asset initial_treasury_balance = db->get_treasury().get_cbd_balance();
    generate_blocks( db->find_comment_cashout( db->get_comment( "alice", string( "test" ) ) )->cashout_time );
    BOOST_REQUIRE_EQUAL( get_cbd_balance( OBSOLETE_TREASURY_ACCOUNT ).amount.value, 0 );
    BOOST_REQUIRE_EQUAL( db->get_treasury().get_cbd_balance().amount.value, 1150 + initial_treasury_balance.amount.value );
  ...
  try
  {
    ...
    db_plugin->debug_update( [&]( database& db )
    {
      ...
      db.modify( old_treasury, [&]( account_object& t )
      {
        ...
        t.reward_crea_balance = ASSET( "2.000 TESTS" );
        t.cbd_balance = ASSET( "5.000 TBD" );
        t.savings_cbd_balance = ASSET( "3.000 TBD" );
        t.reward_cbd_balance = ASSET( "2.000 TBD" );
      } );
    } );
    database_fixture::validate_database();
    {
      ...
      BOOST_REQUIRE_EQUAL( old_treasury.get_cbd_balance().amount.value, 5000 );
      BOOST_REQUIRE_EQUAL( old_treasury.get_cbd_savings().amount.value, 3000 );
      BOOST_REQUIRE_EQUAL( old_treasury.get_cbd_rewards().amount.value, 2000 );
...

    {
      ...
      BOOST_REQUIRE_EQUAL( old_treasury.get_cbd_balance().amount.value, 0 );
      BOOST_REQUIRE_EQUAL( old_treasury.get_cbd_savings().amount.value, 0 );
      BOOST_REQUIRE_EQUAL( old_treasury.get_cbd_rewards().amount.value, 0 );
    ...
    {
      auto& treasury = db->get_account( NEW_CREA_TREASURY_ACCOUNT );
      ...
      BOOST_REQUIRE_EQUAL( treasury.get_cbd_balance().amount.value, 5000 + initial_treasury_balance.amount.value );
      BOOST_REQUIRE_EQUAL( treasury.get_cbd_savings().amount.value, 3000 );
      BOOST_REQUIRE_EQUAL( treasury.get_cbd_rewards().amount.value, 2000 );
...
BOOST_AUTO_TEST_CASE( treasury_debt_ratio )
{
  ...
  {
    ACTORS((alice))
    BOOST_TEST_MESSAGE( "After HF24 funds in the treasury don't count towards the CBD debt ratio" );
    set_price_feed( price( ASSET( "1.000 TBD" ), ASSET( "10.000 TESTS" ) ) );
    generate_block();
    auto& dgpo = db->get_dynamic_global_properties();
    const auto before_cbd_print_rate = dgpo.cbd_print_rate;

    FUND("alice", ASSET( "1000000.000 TBD" ));
    const auto during_cbd_print_rate = dgpo.cbd_print_rate;

    transfer( "alice", db->get_treasury_name(), asset( 1000000000, CBD_SYMBOL ) );
    generate_block();
    const auto after_cbd_print_rate = dgpo.cbd_print_rate;

    BOOST_REQUIRE( after_cbd_print_rate == before_cbd_print_rate );
    BOOST_REQUIRE( after_cbd_print_rate != during_cbd_print_rate );
    database_fixture::validate_database();
  }
  FC_LOG_AND_RETHROW()
}
