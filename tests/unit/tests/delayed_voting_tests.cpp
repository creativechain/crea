...
constexpr int DAYS_FOR_DELAYED_VOTING{ (CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS / CREA_DELAYED_VOTING_INTERVAL_SECONDS) };
...
BOOST_AUTO_TEST_CASE( delayed_voting_proxy_02 )
{
  try
  {
    ...
    }
    {
      ...
      witness_create( "witness1", witness1_private_key, "url.witness1", witness1_private_key.get_public_key(), CREA_MIN_PRODUCER_REWARD.amount );
      witness_create( "witness2", witness2_private_key, "url.witness2", witness2_private_key.get_public_key(), CREA_MIN_PRODUCER_REWARD.amount );
      generate_block();
...

    {
      /*
        *****`+ 1 day`*****
        ...
        `celine` makes vests
      */
      start_time += CREA_DELAYED_VOTING_INTERVAL_SECONDS;
    ...
    }
    {
      /*
        *****`+ 15 days`*****
          ...
          `celine` makes vests
      */
      start_time += 13 * CREA_DELAYED_VOTING_INTERVAL_SECONDS;
      generate_blocks( start_time, true );
      day = 15;
    ...
    }
    {
      /*
        *****`30 days - 1 block`*****
      */
      start_time += 15 * CREA_DELAYED_VOTING_INTERVAL_SECONDS - CREA_BLOCK_INTERVAL;
      generate_blocks( start_time, true );
    ...
    */
...

    {
      /*
        *****`30 days`*****
        ...
        `celine` makes vests
      */
      start_time += CREA_BLOCK_INTERVAL;
      generate_blocks( start_time, true );
      day = 30;
...

    {
      /*
        *****`+ 31 days`*****
        `celine` makes vests
      */
      start_time += fc::seconds( CREA_DELAYED_VOTING_INTERVAL_SECONDS ) ;
      generate_blocks( start_time, true );
      day = 31;
    ...
    }
    {
      /*
        *****`lasted 40 days`*****
      */
      start_time += 8 * CREA_DELAYED_VOTING_INTERVAL_SECONDS;
      generate_blocks( start_time, true );
...

    {
      /*
        *****`lasted 80 days`*****
      */
      start_time += 40 * CREA_DELAYED_VOTING_INTERVAL_SECONDS;
      generate_blocks( start_time, true );
...
BOOST_AUTO_TEST_CASE( delayed_voting_proxy_01 )
{
  try
  {
    ...
    }
    {
      ...
      witness_create( "witness", witness_private_key, "url.witness", witness_private_key.get_public_key(), CREA_MIN_PRODUCER_REWARD.amount );
      generate_block();
    }
    ...
    }
    {
      ...
      generate_blocks( start_time + CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS , true );
...
BOOST_AUTO_TEST_CASE( delayed_voting_04 )
{
  try
  {
    ...
    for(int i = 1; i < (CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS / CREA_DELAYED_VOTING_INTERVAL_SECONDS) - 1; i++)
    {
      generate_blocks( start_time + (i * CREA_DELAYED_VOTING_INTERVAL_SECONDS) , true );
...
BOOST_AUTO_TEST_CASE( delayed_voting_06 )
{
  try
  {
    ...
    CREA_REQUIRE_THROW( witness_vote( "bob", "witness", false/*approve*/, bob_private_key ), fc::assert_exception) ;
    generate_block();
    BOOST_REQUIRE( get_votes( "witness" ) == basic_votes );
    ...
    generate_blocks( start_time + CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS );
    BOOST_REQUIRE( get_votes( "witness" ) == basic_votes );
...
BOOST_AUTO_TEST_CASE( delayed_voting_basic_03 )
{
  try
  {
    ...
    generate_blocks( start_time + (2 * CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS) , true );
...
BOOST_AUTO_TEST_CASE( delayed_voting_basic_02 )
{
  ...
  */
  ...
  }
  {
    ...
    CREA_REQUIRE_THROW( dv.update_votes( items, time ), fc::exception );
    items->clear();
...
BOOST_AUTO_TEST_CASE( delayed_voting_basic_01 )
{
  ...
  */
  ...
  }
  {
    ...
    CREA_REQUIRE_THROW( dv.add_votes( items, false/*withdraw_executor*/, 88/*val*/, *accs[0] ), fc::exception );
  }
  {
  ...
  {
    //delayed_voting_messages::incorrect_withdraw_data
    CREA_REQUIRE_THROW( dv.add_votes( items, false/*withdraw_executor*/, 4/*val*/, *accs[1] ), fc::exception );
  }
}
...
BOOST_AUTO_TEST_CASE( delayed_voting_processor_03 )
{
...

  {
    ...
    CREA_REQUIRE_THROW( delayed_voting_processor::erase( dq, sum, 1 ), fc::exception );
  }
...
BOOST_AUTO_TEST_CASE( delayed_voting_processor_02 )
{
  ...
  //delayed_voting_messages::incorrect_erased_votes
  //dq={1,2}
  CREA_REQUIRE_THROW( delayed_voting_processor::erase( dq, sum, 10ul ), fc::exception );
...
BOOST_AUTO_TEST_CASE( delayed_voting_processor_01 )
{
  ...
  }
  {
    ...
    sum = 1;
    CREA_REQUIRE_THROW( delayed_voting_processor::erase_front( dq, sum ), fc::exception );
  ...
  }
  {
    ...
    CREA_REQUIRE_THROW( delayed_voting_processor::add( dq, sum, time + fc::seconds( 29 )/*time*/, 1000/*val*/ ), fc::exception );
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_01 )
{
  try
  {
    ...
    generate_blocks( db->head_block_time() + CREA_OWNER_AUTH_RECOVERY_PERIOD, true );
    BOOST_REQUIRE( CAN_VOTE("bob") == false );
    CREA_REQUIRE_THROW( witness_vote( "bob", "witness", true, bob_private_key ), fc::assert_exception );
    generate_block();
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_02 )
{
  try
  {
    ...
    // bob block
    decline_voting_rights("bob", true, bob_private_key);
    generate_blocks( db->head_block_time() + (CREA_OWNER_AUTH_RECOVERY_PERIOD.to_seconds() / 2), true );
    ...
    witness_vote( "bob", "witness", true, bob_private_key );
    generate_blocks( db->head_block_time() + (CREA_OWNER_AUTH_RECOVERY_PERIOD.to_seconds() / 3), true );
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_04 )
{
  try
  {
    ...
    generate_block();
    BOOST_REQUIRE( get_votes("witness") == basic_votes );
    generate_blocks( db->head_block_time() + ( CREA_DELAYED_VOTING_INTERVAL_SECONDS * 29 ) , true ); // 120s
    ...
    generate_blocks( db->head_block_time() + CREA_DELAYED_VOTING_INTERVAL_SECONDS , true );
...
#define CHECK_ACCOUNT_CREA( account ) \
  BOOST_REQUIRE( get_balance( #account ) == expected_ ## account ## _crea )
...
BOOST_AUTO_TEST_CASE( scenario_01 )
{
  ...
  asset expected_carol_crea;

/* https://gitlab.syncad.com/crea-group/crea/issues/5#note_24084
...
*/
  ...
  witness_create( "alice0bp", alice0bp_private_key, "url.alice.bp", alice0bp_private_key.get_public_key(), CREA_MIN_PRODUCER_REWARD.amount );
  witness_create( "bob0bp", bob0bp_private_key, "url.bob.bp", bob0bp_private_key.get_public_key(), CREA_MIN_PRODUCER_REWARD.amount );
  generate_block();

/*
  For simplicity we are going to assume 1 vest == 1 CREA at all times but in reality all conversions
  are done using rate from the time when action is performed.
  
  Before test starts alice has 1300 CREA in liquid form. She sets up vest route in the following way:
...
/*
  Day 0: alice powers up 1000 CREA; she has 1000 vests, including delayed maturing on day 30 and 300 CREA
*/
  uint32_t today = 0;
...
/*
  Day 5: alice powers up 300 CREA; she has 1300 vests, including delayed 1000 maturing on day 30 and 300 maturing on day 35, 0 CREA
*/
...
*/
  ...
  int64_t new_vests_portion = new_vests / CREA_VESTING_WITHDRAW_INTERVALS;

  int64_t quarter = new_vests / 4;
  int64_t portion = quarter / CREA_VESTING_WITHDRAW_INTERVALS;
...
/*
  Day 17: PD of 100 vests from alice gives 25 CREA to carol, 25 CREA to alice, powers up 25 vests to bob (maturing on day 47)
...
*/
  ...
  initial_carol_crea = get_balance( "carol" );
  ...
  expected_carol_crea = initial_carol_crea;
