...
#define VOTING_FLOWBAR( account ) db->get_account( account ).voting_flowbar
#define DOWNVOTE_FLOWBAR( account ) db->get_account( account ).downvote_flowbar

inline uint16_t get_voting_power( const account_object& a )
{
  return (uint16_t)( a.voting_flowbar.current_mana / chain::util::get_effective_vesting_shares( a ) );
}
...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
  try
  {
    ...
    const account_object& init = db->get_account( CREA_INIT_MINER_NAME );
    asset init_starting_balance = init.get_balance();

    account_create_operation op;

    op.new_account_name = "alice";
    op.creator = CREA_INIT_MINER_NAME;
    ...
    BOOST_REQUIRE( acct.get_cbd_balance().amount.value == ASSET( "0.000 TBD" ).amount.value );
    ...
    tx.signatures.clear();
    tx.operations.clear();
    op.fee = asset( get_balance( CREA_INIT_MINER_NAME ).amount + 1, CREA_SYMBOL );
    op.new_account_name = "bob";
    tx.operations.push_back( op );
    sign( tx, init_account_priv_key );
    ...
    fund( CREA_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );
    vest( CREA_INIT_MINER_NAME, CREA_TEMP_ACCOUNT, ASSET( "10.000 TESTS" ) );

    BOOST_TEST_MESSAGE( "--- Test account creation with temp account does not set recovery account" );
    op.creator = CREA_TEMP_ACCOUNT;
...
BOOST_AUTO_TEST_CASE( account_update_authorities )
{
  try
  {
    ...
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_irrelevant_sig );

    BOOST_TEST_MESSAGE( "--- Test failure when containing duplicate signatures" );
    tx.signatures.clear();
    sign( tx, active_key );
    sign( tx, active_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_duplicate_sig );
    ...
    tx.operations.push_back( op );
    sign( tx, active_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), tx_missing_owner_auth );
...
BOOST_AUTO_TEST_CASE( comment_authorities )
{
  try
  {
    ...
    ACTORS( (alice)(bob) );
    generate_blocks( 60 / CREA_BLOCK_INTERVAL );
    ...
    sign( tx, bob_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );

    BOOST_TEST_MESSAGE( "--- Test failure when signed by a signature not in the creator's authority" );
    tx.signatures.clear();
    sign( tx, bob_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_posting_auth );
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( alice_comment_cashout->cashout_time == fc::time_point_sec( db->head_block_time() + fc::seconds( CREA_CASHOUT_WINDOW_SECONDS ) ) );
    ...
    BOOST_REQUIRE( bob_comment_cashout->cashout_time == bob_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
    ...
    BOOST_REQUIRE( sam_comment_cashout->cashout_time == sam_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
    BOOST_REQUIRE( sam_comment.get_root_id() == alice_comment.get_id() );
    validate_database();

    generate_blocks( 60 * 5 / CREA_BLOCK_INTERVAL + 1 );
    ...
    {
      o.total_reward_shares2 = crea::chain::util::evaluate_reward_curve( 10 );
    });
    ...
    BOOST_REQUIRE( mod_sam_comment_cashout->cashout_time == mod_sam_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
    validate_database();
    ...
    generate_blocks( 60 * 5 / CREA_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( comment_delete_apply )
{
  try
  {
    ...
    vest( CREA_INIT_MINER_NAME, "alice", ASSET( "1000.000 TESTS" ) );
    ...
    tx.set_expiration( db->head_block_time() + CREA_MIN_TRANSACTION_EXPIRATION_LIMIT );
    sign( tx, alice_private_key );
    db->push_transaction( tx, 0 );
    ...
    generate_block();
    vote.weight = -1 * CREA_100_PERCENT;
    tx.clear();
    tx.operations.push_back( vote );
    tx.operations.push_back( op );
    sign( tx, alice_private_key );
    ...
    generate_blocks( CREA_MIN_ROOT_COMMENT_INTERVAL.to_seconds() / CREA_BLOCK_INTERVAL );
    ...
    generate_blocks( CREA_CASHOUT_WINDOW_SECONDS / CREA_BLOCK_INTERVAL );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
  try
  {
...

    {
      ...
      auto old_mana = alice.voting_flowbar.current_mana;
      ...
      int64_t max_vote_denom = ( db->get_dynamic_global_properties().vote_power_reserve_rate * CREA_VOTING_MANA_REGENERATION_SECONDS ) / (60*60*24);

      BOOST_REQUIRE( alice.last_vote_time == db->head_block_time() );
      BOOST_REQUIRE( alice_comment_cashout->net_rshares.value == ( old_mana - alice.voting_flowbar.current_mana ) - CREA_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( alice_comment_cashout->cashout_time == alice_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( itr->rshares == ( old_mana - alice.voting_flowbar.current_mana ) - CREA_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( itr != vote_idx.end() );
      validate_database();

      BOOST_TEST_MESSAGE( "--- Test reduced power for quick voting" );

      generate_blocks( db->head_block_time() + CREA_MIN_VOTE_INTERVAL_SEC );

      util::flowbar old_flowbar = VOTING_FLOWBAR( "alice" );
      util::flowbar_params params( util::get_effective_vesting_shares( db->get_account( "alice" ) ), CREA_VOTING_MANA_REGENERATION_SECONDS );
      old_flowbar.regenerate_mana( params, db->head_block_time() );
      ...
      BOOST_REQUIRE( bob_comment_cashout->net_rshares.value == ( old_flowbar.current_mana - db->get_account( "alice" ).voting_flowbar.current_mana ) - CREA_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( bob_comment_cashout->cashout_time == bob_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( itr != vote_idx.end() );
      validate_database();
      ...
      old_mana = VOTING_FLOWBAR( "bob" ).current_mana;
      auto old_abs_rshares = db->find_comment_cashout( db->get_comment( "alice", string( "foo" ) ) )->abs_rshares.value;

      generate_blocks( db->head_block_time() + fc::seconds( ( CREA_CASHOUT_WINDOW_SECONDS / 2 ) ), true );
      ...
      BOOST_REQUIRE( new_alice_comment_cashout->net_rshares.value == old_abs_rshares + ( old_mana - new_bob.voting_flowbar.current_mana ) - CREA_VOTE_DUST_THRESHOLD );
      BOOST_REQUIRE( new_alice_comment_cashout->cashout_time == new_alice_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( itr != vote_idx.end() );
      validate_database();
      ...
      util::flowbar old_downvote_flowbar;
      util::flowbar_params downvote_params( util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4, CREA_VOTING_MANA_REGENERATION_SECONDS );
      old_downvote_flowbar.regenerate_mana( downvote_params, db->head_block_time() );
      int64_t sam_weight = old_downvote_flowbar.current_mana - DOWNVOTE_FLOWBAR( "sam" ).current_mana - CREA_VOTE_DUST_THRESHOLD;
      ...
      BOOST_REQUIRE( new_bob_comment_cashout->cashout_time == new_bob_comment_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( itr != vote_idx.end() );
      validate_database();
      ...
      int64_t regenerated_power = (CREA_100_PERCENT * ( db->head_block_time() - db->get_account( "alice").last_vote_time ).to_seconds() ) / CREA_VOTING_MANA_REGENERATION_SECONDS;
      ...
      int64_t new_rshares = ( ( fc::uint128_t( get_vesting( "alice" ).amount.value ) * used_power ) / CREA_100_PERCENT ).to_uint64() - CREA_VOTE_DUST_THRESHOLD;

      {
        auto* alice_cashout = db->find_comment_cashout( db->get_comment( "alice", string( "foo" ) ) );
        BOOST_REQUIRE( alice_cashout->cashout_time == alice_cashout->get_creation_time() + CREA_CASHOUT_WINDOW_SECONDS );
      }
      ...
      new_rshares = old_flowbar.current_mana - VOTING_FLOWBAR( "alice" ).current_mana - CREA_VOTE_DUST_THRESHOLD;
      ...
      new_rshares = old_downvote_flowbar.current_mana - DOWNVOTE_FLOWBAR( "alice" ).current_mana - CREA_VOTE_DUST_THRESHOLD;
      ...
      db_plugin->debug_update( [=]( database& db )
      {
        ...
        {
          a.downvote_flowbar.current_mana /= 30;
          a.downvote_flowbar.last_update_time = db.head_block_time().sec_since_epoch();
        });
      });
      ...
      {
        const auto& alice = db->get_account( "alice" );
        old_downvote_flowbar.current_mana = alice.downvote_flowbar.current_mana;
        old_downvote_flowbar.last_update_time = alice.downvote_flowbar.last_update_time;
        old_flowbar.current_mana = alice.voting_flowbar.current_mana;
        old_flowbar.last_update_time = alice.voting_flowbar.last_update_time;
        old_flowbar.regenerate_mana( params, db->head_block_time() );
        old_downvote_flowbar.regenerate_mana( params, db->head_block_time() );
        alice_weight = old_flowbar.current_mana * 60 * 60 * 24;
        auto max_vote_denom = db->get_dynamic_global_properties().vote_power_reserve_rate * CREA_VOTING_MANA_REGENERATION_SECONDS;
...

      {
        ...
        const comment_cashout_object* bob_comment_cashout = db->find_comment_cashout( bob_comment );
        const auto& alice = db->get_account( "alice" );
        BOOST_REQUIRE( bob_comment_cashout->net_rshares == old_net_rshares - alice_weight + CREA_VOTE_DUST_THRESHOLD );
        BOOST_REQUIRE( bob_comment_cashout->abs_rshares == old_abs_rshares + alice_weight - CREA_VOTE_DUST_THRESHOLD );
        BOOST_REQUIRE( alice_bob_vote->rshares == -1 * ( alice_weight - CREA_VOTE_DUST_THRESHOLD ) );
        BOOST_REQUIRE( alice_bob_vote->last_update == db->head_block_time() );
        BOOST_REQUIRE( alice_bob_vote->vote_percent == op.weight );
        BOOST_REQUIRE( alice.downvote_flowbar.current_mana == 0 );
        BOOST_REQUIRE( alice.voting_flowbar.current_mana == old_flowbar.current_mana - alice_weight + old_downvote_flowbar.current_mana );
        validate_database();
      }

      BOOST_TEST_MESSAGE( "--- Test reduced effectiveness when increasing rshares within lockout period" );

      generate_blocks( fc::time_point_sec( ( new_bob_comment_cashout->cashout_time - CREA_UPVOTE_LOCKOUT_HF17 ).sec_since_epoch() + CREA_BLOCK_INTERVAL ), true );

      old_flowbar = VOTING_FLOWBAR( "dave" );
      ...
      new_rshares = old_flowbar.current_mana - VOTING_FLOWBAR( "dave" ).current_mana - CREA_VOTE_DUST_THRESHOLD;
      new_rshares = ( new_rshares * ( CREA_UPVOTE_LOCKOUT_SECONDS - CREA_BLOCK_INTERVAL ) ) / CREA_UPVOTE_LOCKOUT_SECONDS;
      ...
      new_rshares = old_flowbar.current_mana - VOTING_FLOWBAR( "dave" ).current_mana - CREA_VOTE_DUST_THRESHOLD;
      new_rshares = ( new_rshares * ( CREA_UPVOTE_LOCKOUT_SECONDS - CREA_BLOCK_INTERVAL - CREA_BLOCK_INTERVAL ) ) / CREA_UPVOTE_LOCKOUT_SECONDS;
...
BOOST_AUTO_TEST_CASE( transfer_validate )
{
  try
  {
    ...
    op.to = "bob";
    op.memo = "Memo";
    op.amount = asset( 100, CREA_SYMBOL );
    op.validate();
    ...
    std::string memo;
    for ( int i = 0; i < CREA_MAX_MEMO_SIZE + 1; i++ )
      memo += "x";
    op.memo = memo;
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
    op.memo = "Memo";
...
BOOST_AUTO_TEST_CASE( transfer_apply )
{
  try
  {
    ...
    tx.set_expiration( db->head_block_time() + CREA_MAX_TIME_UNTIL_EXPIRATION );
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), fc::exception );

    BOOST_REQUIRE( new_alice.get_balance().amount.value == ASSET( "0.000 TESTS" ).amount.value );
    BOOST_REQUIRE( new_bob.get_balance().amount.value == ASSET( "10.000 TESTS" ).amount.value );
    validate_database();

    BOOST_TEST_MESSAGE( "--- Test successfully transfering CREA to treasury and converting it to CBD" );
    set_price_feed( price( ASSET( "1.000 TBD" ), ASSET( "1.000 TESTS" ) ) );
    generate_block();
    auto treasury_cbd_balance = db->get_treasury().get_cbd_balance();
    op.from = "bob";
    ...
    BOOST_REQUIRE( db->get_treasury().get_cbd_balance() == treasury_cbd_balance + ASSET( "1.000 TBD" ) );
    validate_database();

    BOOST_TEST_MESSAGE( "--- Test transfering CBD to treasury" );
    treasury_cbd_balance = db->get_treasury().get_cbd_balance();
    ...
    BOOST_REQUIRE( get_cbd_balance( "bob" ) == ASSET( "0.000 TBD" ) );
    BOOST_REQUIRE( db->get_treasury().get_cbd_balance() == treasury_cbd_balance + ASSET( "1.000 TBD" ) );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_vesting_apply )
{
  try
  {
    ...
    auto vests = asset( gpo.get_total_vesting_fund_crea().amount, CREA_SYMBOL );
    ...
    BOOST_REQUIRE( gpo.get_total_vesting_fund_crea().amount.value == vests.amount.value );
    BOOST_REQUIRE( gpo.get_total_vesting_shares().amount.value == shares.amount.value );
    validate_database();

    op.to = "bob";
    op.amount = asset( 2000, CREA_SYMBOL );
    tx.operations.clear();
    tx.signatures.clear();
    tx.operations.push_back( op );
    tx.set_expiration( db->head_block_time() + CREA_MAX_TIME_UNTIL_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_authorities )
{
  try
  {
    ...
    BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
    CREA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
    ...
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( CREA_VESTING_WITHDRAW_INTERVALS * 2 ) ).value + 1 );
    BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
    BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + CREA_VESTING_WITHDRAW_INTERVAL_SECONDS );
    validate_database();
    ...
    BOOST_REQUIRE( alice.get_vesting().amount.value == old_vesting_shares.amount.value );
    BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( CREA_VESTING_WITHDRAW_INTERVALS * 3 ) ).value + 1 );
    BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
    BOOST_REQUIRE( alice.next_vesting_withdrawal == db->head_block_time() + CREA_VESTING_WITHDRAW_INTERVAL_SECONDS );
    validate_database();
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      ...
      {
        gpo.current_supply += wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" ) - gpo.get_total_vesting_fund_crea();
        gpo.total_vesting_fund_crea = wso.median_props.account_creation_fee - ASSET( "0.001 TESTS" );
      });
...
BOOST_AUTO_TEST_CASE( witness_update_apply )
{
  try
  {
    ...
    op.fee = ASSET( "1.000 TESTS" );
    op.block_signing_key = signing_key.get_public_key();
    op.props.account_creation_fee = legacy_crea_asset::from_asset( asset(CREA_MIN_ACCOUNT_CREATION_FEE + 10, CREA_SYMBOL) );
    op.props.maximum_block_size = CREA_MIN_BLOCK_SIZE_LIMIT + 100;
...
BOOST_AUTO_TEST_CASE(account_witness_vote_apply_delay)
{
  ...
  try
  {
    ...
    fund("alice", 5000);
    vest("alice", 5000);
    //vests are going to vote after CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS
    ...
    validate_database();
    generate_blocks(db->head_block_time() + fc::seconds(CREA_DELAYED_VOTING_TOTAL_INTERVAL_SECONDS));
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( alice.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    ...
    BOOST_REQUIRE( sam.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    ...
    BOOST_REQUIRE( sam.proxied_vsf_votes_total() == bob.get_real_vesting_shares() );
    BOOST_REQUIRE( dave.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    ...
    tx.operations.clear();
    tx.signatures.clear();
    op.proxy = CREA_PROXY_TO_SELF_ACCOUNT;
    op.account = "bob";
    tx.operations.push_back( op );
    sign( tx, bob_private_key );
    ...
    BOOST_REQUIRE( alice.proxy == "sam" );
    BOOST_REQUIRE( alice.proxied_vsf_votes_total().value == 0 );
    BOOST_REQUIRE( bob.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    BOOST_REQUIRE( bob.proxied_vsf_votes_total().value == 0 );
    BOOST_REQUIRE( sam.proxy == "dave" );
    ...
    vote.account= "bob";
    vote.witness = CREA_INIT_MINER_NAME;
    tx.operations.clear();
    tx.signatures.clear();
    tx.operations.push_back( vote );
    sign( tx, bob_private_key );
    ...
    BOOST_REQUIRE( db->get_witness( CREA_INIT_MINER_NAME ).votes == ( alice.get_real_vesting_shares() + bob.get_real_vesting_shares() ) );
    validate_database();
    ...
    BOOST_REQUIRE( db->get_witness( CREA_INIT_MINER_NAME ).votes == bob.get_real_vesting_shares() );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( account_witness_proxy_apply_delay )
{
  ...
  try
  {
    ...
    BOOST_REQUIRE( _alice.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    ...
    BOOST_REQUIRE( _bob.proxied_vsf_votes_total().value == 0 );
    BOOST_REQUIRE( _alice.proxied_vsf_votes_total().value == 0 );
    BOOST_REQUIRE( _sam.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    ...
    BOOST_REQUIRE( _sam.proxied_vsf_votes_total() == _bob.get_vesting().amount );
    BOOST_REQUIRE( _dave.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    ...
    BOOST_REQUIRE( _alice.proxy == "sam" );
    BOOST_REQUIRE( _alice.proxied_vsf_votes_total().value == 0 );
    BOOST_REQUIRE( _bob.proxy == CREA_PROXY_TO_SELF_ACCOUNT );
    BOOST_REQUIRE( _bob.proxied_vsf_votes_total().value == 0 );
    BOOST_REQUIRE( _sam.proxy == "dave" );
    ...
    BOOST_REQUIRE( db->get_witness( CREA_INIT_MINER_NAME ).votes == ( _alice.get_vesting() + _bob.get_vesting() ).amount );
    validate_database();
    ...
    BOOST_REQUIRE( db->get_witness( CREA_INIT_MINER_NAME ).votes == _bob.get_vesting().amount );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( custom_json_rate_limit )
{
  try
  {
    ...
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), plugin_exception );
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
  try
  {
...

    {
      ...
      BOOST_REQUIRE( alice_witness.get_cbd_exchange_rate() == op.exchange_rate );
      BOOST_REQUIRE( alice_witness.get_last_cbd_exchange_update() == db->head_block_time() );
      validate_database();
    }
    ...
    auto& alice_witness = db->get_witness( "alice" );
    // BOOST_REQUIRE( std::abs( alice_witness.get_cbd_exchange_rate().to_real() - op.exchange_rate.to_real() ) < 0.0000005 );
    BOOST_REQUIRE( alice_witness.get_last_cbd_exchange_update() == db->head_block_time() );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( new_bob.get_cbd_balance().amount.value == ASSET( "7.000 TBD" ).amount.value );
    validate_database();
    ...
    BOOST_REQUIRE( new_alice.get_cbd_balance().amount.value == ASSET( "2.500 TBD" ).amount.value );
    validate_database();
    ...
    BOOST_REQUIRE( new_bob.get_balance().amount.value == ASSET( "3.000 TESTS" ).amount.value );
    BOOST_REQUIRE( new_bob.get_cbd_balance().amount.value == ASSET( "4.000 TBD" ).amount.value );
    ...
    BOOST_REQUIRE( convert_request->conversion_date == db->head_block_time() + CREA_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( fixture_convert_checks_balance )
{
  ...
  try
  {
    ...
    fund( "dany", 5000 );
    CREA_REQUIRE_THROW( convert( "dany", ASSET( "5000.000 TESTS" ) ), fc::exception );
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( bob.get_balance().amount.value == ASSET( "0.000 TESTS" ).amount.value );
    BOOST_REQUIRE( bob.get_cbd_balance().amount.value == ASSET( "1000.000 TBD" ).amount.value );
    validate_database();
    ...
    BOOST_REQUIRE( alice.get_cbd_balance().amount.value == ASSET( "0.000 TBD" ).amount.value );
    validate_database();
    ...
    op.expiration = db->head_block_time() + fc::seconds( CREA_MAX_LIMIT_ORDER_EXPIRATION + 1 );
    tx.operations.clear();
    tx.signatures.clear();
    tx.operations.push_back( op );
    sign( tx, alice_private_key );
    ...
    BOOST_REQUIRE( limit_order_idx.find( boost::make_tuple( "bob", op.orderid ) ) == limit_order_idx.end() );
    BOOST_REQUIRE( alice.get_balance().amount.value == ASSET( "990.000 TESTS" ).amount.value );
    BOOST_REQUIRE( alice.get_cbd_balance().amount.value == ASSET( "7.500 TBD" ).amount.value );
    BOOST_REQUIRE( bob.get_balance().amount.value == ASSET( "5.000 TESTS" ).amount.value );
    BOOST_REQUIRE( bob.get_cbd_balance().amount.value == ASSET( "992.500 TBD" ).amount.value );
    ...
    BOOST_REQUIRE( alice.get_balance().amount.value == ASSET( "990.000 TESTS" ).amount.value );
    BOOST_REQUIRE( alice.get_cbd_balance().amount.value == ASSET( "15.000 TBD" ).amount.value );
    BOOST_REQUIRE( bob.get_balance().amount.value == ASSET( "10.000 TESTS" ).amount.value );
    BOOST_REQUIRE( bob.get_cbd_balance().amount.value == ASSET( "977.500 TBD" ).amount.value );
    validate_database();
    ...
    BOOST_REQUIRE( alice.get_cbd_balance().amount.value == ASSET( "22.500 TBD" ).amount.value );
    ...
    BOOST_REQUIRE( alice.get_cbd_balance().amount.value == ASSET( "33.500 TBD" ).amount.value );
    BOOST_REQUIRE( bob.get_balance().amount.value == ASSET( "25.000 TESTS" ).amount.value );
    BOOST_REQUIRE( bob.get_cbd_balance().amount.value == ASSET( "965.500 TBD" ).amount.value );
    validate_database();
    ...
    //auto start_cbd = gpo.get_current_cbd_supply();
    ...
    BOOST_REQUIRE( alice.get_cbd_balance().amount.value == ASSET( "45.500 TBD" ).amount.value );
    BOOST_REQUIRE( bob.get_balance().amount.value == ASSET( "35.909 TESTS" ).amount.value );
    BOOST_REQUIRE( bob.get_cbd_balance().amount.value == ASSET( "954.500 TBD" ).amount.value );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( limit_order_create2_apply )
{
  try
  {
    ...
    /// First check validation on price constructor level:
    {
      price broken_price;
      /// Invalid base value
      CREA_REQUIRE_THROW(broken_price=price(ASSET("0.000 TESTS"), ASSET("1.000 TBD")),
        fc::exception);
      /// Invalid quote value
      CREA_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TBD")),
        fc::exception);
      /// Invalid symbol (same in base & quote)
      CREA_REQUIRE_THROW(broken_price=price(ASSET("1.000 TESTS"), ASSET("0.000 TESTS")),
        fc::exception);
    }
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_authorities )
{
  try
  {
    ...
    c.expiration = db->head_block_time() + fc::seconds( CREA_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( limit_order_cancel_apply )
{
  try
  {
    ...
    create.expiration = db->head_block_time() + fc::seconds( CREA_MAX_LIMIT_ORDER_EXPIRATION );
    tx.operations.clear();
    tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
  try
  {
    ...
    vest( CREA_INIT_MINER_NAME, "bob", asset( 1000, CREA_SYMBOL ) );
    ...
    generate_blocks( db->head_block_time() + CREA_OWNER_UPDATE_LIMIT );
    ...
    generate_blocks( db->head_block_time() + CREA_OWNER_UPDATE_LIMIT + fc::seconds( CREA_BLOCK_INTERVAL ) );
    ...
    BOOST_REQUIRE( req_itr->expires == db->head_block_time() + CREA_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD );
    auto expires = req_itr->expires;
    ++req_itr;
    BOOST_REQUIRE( req_itr == request_idx.end() );

    generate_blocks( time_point_sec( expires - CREA_BLOCK_INTERVAL ), true );
    ...
    generate_blocks( db->head_block_time() + ( CREA_OWNER_AUTH_RECOVERY_PERIOD - CREA_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
    generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
  try
  {
    ...
    CREA_REQUIRE_THROW( change_recovery_account("alice", "nobody"), fc::exception );
    CREA_REQUIRE_THROW( change_recovery_account("haxer", "sam"   ), fc::exception );
    CREA_REQUIRE_THROW( change_recovery_account("haxer", "nobody"), fc::exception );
    ...
    generate_blocks( db->head_block_time() + CREA_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( CREA_BLOCK_INTERVAL ), true );
    // cannot request account recovery until recovery account is approved
    CREA_REQUIRE_THROW( request_account_recovery( "sam", sam_private_key, "alice", alice_pub1 ), fc::exception );
    generate_blocks(1);
    // cannot finish account recovery until requested
    CREA_REQUIRE_THROW( recover_account( "alice", alice_priv1, alice_private_key ), fc::exception );
    // do the request
...
BOOST_AUTO_TEST_CASE( escrow_transfer_apply )
{
  try
  {
    ...
    auto alice_crea_balance = alice.get_balance() - op.crea_amount - op.fee;
    auto alice_cbd_balance = alice.get_cbd_balance() - op.cbd_amount;
    auto bob_crea_balance = bob.get_balance();
    auto bob_cbd_balance = bob.get_cbd_balance();
    auto sam_crea_balance = sam.get_balance();
    auto sam_cbd_balance = sam.get_cbd_balance();
    ...
    BOOST_REQUIRE( escrow.get_cbd_balance() == op.cbd_amount );
    BOOST_REQUIRE( escrow.get_crea_balance() == op.crea_amount );
    ...
    BOOST_REQUIRE( alice.get_balance() == alice_crea_balance );
    BOOST_REQUIRE( alice.get_cbd_balance() == alice_cbd_balance );
    BOOST_REQUIRE( bob.get_balance() == bob_crea_balance );
    BOOST_REQUIRE( bob.get_cbd_balance() == bob_cbd_balance );
    BOOST_REQUIRE( sam.get_balance() == sam_crea_balance );
    BOOST_REQUIRE( sam.get_cbd_balance() == sam_cbd_balance );
...
BOOST_AUTO_TEST_CASE( escrow_approve_apply )
{
  try
  {
    ...
    et_op.to = "bob";
    et_op.agent = "sam";
    et_op.crea_amount = ASSET( "1.000 TESTS" );
    ...
    BOOST_REQUIRE( escrow.get_cbd_balance() == ASSET( "0.000 TBD" ) );
    BOOST_REQUIRE( escrow.get_crea_balance() == ASSET( "1.000 TESTS" ) );
    ...
    tx.set_expiration( db->head_block_time() + CREA_BLOCK_INTERVAL );
    sign( tx, bob_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
    ...
    CREA_REQUIRE_THROW( db->get_escrow( op.from, op.escrow_id ), fc::exception );
    BOOST_REQUIRE( alice.get_balance() == ASSET( "10.000 TESTS" ) );
    validate_database();
    ...
    generate_blocks( et_op.ratification_deadline + CREA_BLOCK_INTERVAL, true );
...
BOOST_AUTO_TEST_CASE( escrow_dispute_apply )
{
  try
  {
    ...
    et_op.crea_amount = ASSET( "1.000 TESTS" );
    et_op.fee = ASSET( "0.100 TESTS" );
    et_op.ratification_deadline = db->head_block_time() + CREA_BLOCK_INTERVAL;
    et_op.escrow_expiration = db->head_block_time() + 2 * CREA_BLOCK_INTERVAL;
    ...
    BOOST_REQUIRE( escrow.ratification_deadline == et_op.ratification_deadline );
    BOOST_REQUIRE( escrow.escrow_expiration == et_op.escrow_expiration );
    BOOST_REQUIRE( escrow.get_cbd_balance() == et_op.cbd_amount );
    BOOST_REQUIRE( escrow.get_crea_balance() == et_op.crea_amount );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
  try
  {
    ...
    BOOST_TEST_MESSAGE( "--- failure when hbd is not CBD symbol" );
    op.cbd_amount = ASSET( "1.000 TESTS" );
    CREA_REQUIRE_THROW( op.validate(), fc::exception );


    BOOST_TEST_MESSAGE( "--- failure when hive is not CREA symbol" );
    op.cbd_amount.symbol = CBD_SYMBOL;
    op.crea_amount = ASSET( "1.000 TBD" );
    CREA_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( escrow_release_apply )
{
  try
  {
    ...
    op.crea_amount = ASSET( "0.100 TESTS" );
    ...
    BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).get_crea_balance() == ASSET( "0.900 TESTS" ) );
    ...
    BOOST_REQUIRE( db->get_escrow( op.from, op.escrow_id ).get_crea_balance() == ASSET( "0.800 TESTS" ) );
    ...
    op.crea_amount = ASSET( "0.000 TESTS" );
    op.cbd_amount = ASSET( "1.000 TBD" );
    ...
    BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).get_crea_balance() == ASSET( "0.700 TESTS" ) );
    ...
    BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).get_crea_balance() == ASSET( "0.600 TESTS" ) );
    ...
    BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).get_crea_balance() == ASSET( "0.500 TESTS" ) );


    BOOST_TEST_MESSAGE( "--- success deleting escrow when balances are both zero" );
    tx.clear();
    op.crea_amount = ASSET( "0.500 TESTS" );
    tx.operations.push_back( op );
    sign( tx, sam_private_key );
    db->push_transaction( tx, 0 );

    BOOST_REQUIRE( get_balance( "alice" ) == ASSET( "9.700 TESTS" ) );
    CREA_REQUIRE_THROW( db->get_escrow( et_op.from, et_op.escrow_id ), fc::exception );
    ...
    BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).get_crea_balance() == ASSET( "0.900 TESTS" ) );
    ...
    BOOST_REQUIRE( db->get_escrow( et_op.from, et_op.escrow_id ).get_crea_balance() == ASSET( "0.800 TESTS" ) );
    ...
    tx.clear();
    op.crea_amount = ASSET( "0.600 TESTS" );
    tx.operations.push_back( op );
    sign( tx, alice_private_key );
    db->push_transaction( tx, 0 );
...
BOOST_AUTO_TEST_CASE( escrow_limit )
{
  try
  {
    ...
    op.to = "bob";
    op.agent = "sam";
    op.crea_amount = ASSET( "0.001 TESTS" );
    op.fee = ASSET( "0.000 TESTS" );
    op.ratification_deadline = db->head_block_time() + 2 * ( CREA_MAX_PENDING_TRANSFERS + 1 ) * CREA_BLOCK_INTERVAL;
    op.escrow_expiration = op.ratification_deadline + CREA_MAX_PENDING_TRANSFERS * CREA_BLOCK_INTERVAL;

    int i;
    //create fresh escrow transfers from "alice", one per block, up to the limit
    for( i = 0; i < CREA_MAX_PENDING_TRANSFERS; ++i )
    {
      signed_transaction tx;
      op.escrow_id = i;
      tx.operations.push_back( op );
    ...
    for( i = 0; i < CREA_MAX_PENDING_TRANSFERS; ++i )
    {
      ...
      generate_block();
      BOOST_REQUIRE_EQUAL( db->get_account( "alice" ).pending_transfers, CREA_MAX_PENDING_TRANSFERS - declined );
    }

    generate_block();
    //unratified transfers should expire by now
    BOOST_REQUIRE_EQUAL( db->get_account( "alice" ).pending_transfers, CREA_MAX_PENDING_TRANSFERS - declined - expired );
    ...
    alice_release.crea_amount = bob_release.crea_amount = ASSET( "0.001 TESTS" );

    int released = 0;
    for( i = 0; i < CREA_MAX_PENDING_TRANSFERS; i += 4 )
    {
      ...
      db->push_transaction( tx, 0 );
      generate_block();
      BOOST_REQUIRE_EQUAL( db->get_account( "alice" ).pending_transfers, CREA_MAX_PENDING_TRANSFERS - declined - expired - released );
    }
    ...
    int remaining = CREA_MAX_PENDING_TRANSFERS - declined - expired - released;
    for( i = 0; i < CREA_MAX_PENDING_TRANSFERS; i += 8 )
    {
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
  try
  {
    ...
    BOOST_TEST_MESSAGE( "success when amount is CREA" );
    op.amount = ASSET( "1.000 TESTS" );
    op.validate();
  }
  FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( get_balance( "alice" ) == ASSET( "10.000 TESTS" ) );
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == ASSET( "10.000 TBD" ) );
    ...
    BOOST_TEST_MESSAGE( "--- success transferring CREA to self" );
    op.to = "alice";
    op.amount = ASSET( "1.000 TESTS" );
    ...
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == ASSET( "9.000 TBD" ) );
    BOOST_REQUIRE( get_cbd_savings( "alice" ) == ASSET( "1.000 TBD" ) );
    validate_database();


    BOOST_TEST_MESSAGE( "--- success transferring CREA to other" );
    op.to = "bob";
    op.amount = ASSET( "1.000 TESTS" );
    ...
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == ASSET( "8.000 TBD" ) );
    BOOST_REQUIRE( get_cbd_savings( "bob" ) == ASSET( "1.000 TBD" ) );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
  try
  {
    ...
    sign( tx, alice_private_key );
    db->push_transaction( tx, 0 );
    BOOST_REQUIRE( get_cbd_savings( "alice" ) == ASSET( "9.000 TBD" ) );
    validate_database();

    BOOST_TEST_MESSAGE( "--- success withdrawing CREA to self" );
    op.to = "alice";
    op.amount = ASSET( "1.000 TESTS" );
    op.request_id++;
    ...
    BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).complete == db->head_block_time() + CREA_SAVINGS_WITHDRAW_TIME );
    validate_database();
    ...
    BOOST_REQUIRE( get_cbd_savings( "alice" ) == ASSET( "8.000 TBD" ) );
    BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == op.request_id + 1 );
    BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).from == op.from );
    BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).to == op.to );
    BOOST_REQUIRE( to_string( db->get_savings_withdraw( "alice", op.request_id ).memo ) == op.memo );
    ...
    BOOST_TEST_MESSAGE( "--- success withdrawing CREA to other" );
    op.to = "bob";
    op.amount = ASSET( "1.000 TESTS" );
    op.request_id = 3;
    ...
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == ASSET( "0.000 TBD" ) );
    BOOST_REQUIRE( get_cbd_savings( "alice" ) == ASSET( "7.000 TBD" ) );
    BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == op.request_id + 1 );
    BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).from == op.from );
    BOOST_REQUIRE( db->get_savings_withdraw( "alice", op.request_id ).to == op.to );
    BOOST_REQUIRE( to_string( db->get_savings_withdraw( "alice", op.request_id ).memo ) == op.memo );
    ...
    generate_blocks( db->head_block_time() + CREA_SAVINGS_WITHDRAW_TIME - fc::seconds( CREA_BLOCK_INTERVAL ), true );
    ...
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == ASSET( "1.000 TBD" ) );
    BOOST_REQUIRE( get_balance( "bob" ) == ASSET( "1.000 TESTS" ) );
    BOOST_REQUIRE( get_cbd_balance( "bob" ) == ASSET( "1.000 TBD" ) );
    ...
    for( int i = 0; i < CREA_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
    {
      op.request_id = i;
      tx.clear();
      tx.operations.push_back( op );
    ...
    op.request_id = CREA_SAVINGS_WITHDRAW_REQUEST_LIMIT;
    tx.clear();
    tx.operations.push_back( op );
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );
    BOOST_REQUIRE( db->get_account( "alice" ).savings_withdraw_requests == CREA_SAVINGS_WITHDRAW_REQUEST_LIMIT );
    validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( decline_voting_rights_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( itr->effective_date == db->head_block_time() + CREA_OWNER_AUTH_RECOVERY_PERIOD );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
  try
  {
    ...
    op.account = "alice";
    op.reward_crea = ASSET( "0.000 TESTS" );
    ...
    op.reward_crea.amount = 1000;
    op.validate();

    op.reward_crea.amount = 0;
    ...
    BOOST_TEST_MESSAGE( "Testing wrong CREA symbol" );
    op.reward_crea = ASSET( "1.000 TBD" );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );


    BOOST_TEST_MESSAGE( "Testing wrong CBD symbol" );
    op.reward_crea = ASSET( "1.000 TESTS" );
  ...
  try
  {
    ...
    db_plugin->debug_update( []( database& db )
    {
      ...
      db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
      {
        ...
        gpo.pending_rewarded_vesting_crea += ASSET( "10.000 TESTS" );
      });
    });
    ...
    auto alice_crea = get_balance( "alice" );
    auto alice_cbd = get_cbd_balance( "alice" );
    auto alice_vests = get_vesting( "alice" );


    BOOST_TEST_MESSAGE( "--- Attempting to claim more CREA than exists in the reward balance." );

    claim_reward_balance_operation op;
    signed_transaction tx;

    op.account = "alice";
    op.reward_crea = ASSET( "20.000 TESTS" );
    op.reward_cbd = ASSET( "0.000 TBD" );
    op.reward_vests = ASSET( "0.000000 VESTS" );
    ...
    BOOST_REQUIRE( get_balance( "alice" ) == alice_crea + op.reward_crea );
    BOOST_REQUIRE( get_rewards( "alice" ) == ASSET( "10.000 TESTS" ) );
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == alice_cbd + op.reward_cbd );
    BOOST_REQUIRE( get_cbd_rewards( "alice" ) == ASSET( "10.000 TBD" ) );
    BOOST_REQUIRE( get_vesting( "alice" ) == alice_vests + op.reward_vests );
    BOOST_REQUIRE( get_vest_rewards( "alice" ) == ASSET( "5.000000 VESTS" ) );
    BOOST_REQUIRE( get_vest_rewards_as_crea( "alice" ) == ASSET( "5.000 TESTS" ) );
    validate_database();
    ...
    op.reward_crea = ASSET( "10.000 TESTS" );
    ...
    BOOST_REQUIRE( get_cbd_balance( "alice" ) == alice_cbd + op.reward_cbd );
    BOOST_REQUIRE( get_cbd_rewards( "alice" ) == ASSET( "0.000 TBD" ) );
    BOOST_REQUIRE( get_vesting( "alice" ) == alice_vests + op.reward_vests );
    BOOST_REQUIRE( get_vest_rewards( "alice" ) == ASSET( "0.000000 VESTS" ) );
    BOOST_REQUIRE( get_vest_rewards_as_crea( "alice" ) == ASSET( "0.000 TESTS" ) );
        validate_database();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_authorities )
{
  try
  {
    ...
    signed_transaction tx;
    ACTORS( (alice)(bob) )
    vest( CREA_INIT_MINER_NAME, "alice", ASSET( "10000.000 TESTS" ) );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
  try
  {
    ...
    util::flowbar old_downvote_flowbar = DOWNVOTE_FLOWBAR( "alice" );
    params.max_mana = util::get_effective_vesting_shares( db->get_account( "alice" ) ) / 4;
    old_downvote_flowbar.regenerate_mana( params, db->head_block_time() );

    util::flowbar old_bob_flowbar = VOTING_FLOWBAR( "bob" );
    params.max_mana = util::get_effective_vesting_shares( db->get_account( "bob" ) );
    old_bob_flowbar.regenerate_mana( params, db->head_block_time() );

    util::flowbar old_bob_downvote_flowbar = DOWNVOTE_FLOWBAR( "bob" );
    params.max_mana = util::get_effective_vesting_shares( db->get_account( "bob" ) ) / 4;
    old_bob_downvote_flowbar.regenerate_mana( params, db->head_block_time() );
    ...
    BOOST_REQUIRE( alice_acc.voting_flowbar.current_mana == old_flowbar.current_mana - op.vesting_shares.amount.value );
    BOOST_REQUIRE( alice_acc.downvote_flowbar.current_mana == old_downvote_flowbar.current_mana - op.vesting_shares.amount.value / 4 );
    BOOST_REQUIRE( bob_acc.get_received_vesting() == ASSET( "10000000.000000 VESTS" ) );
    BOOST_REQUIRE( bob_acc.voting_flowbar.current_mana == old_bob_flowbar.current_mana + op.vesting_shares.amount.value );
    BOOST_REQUIRE( bob_acc.downvote_flowbar.current_mana == old_bob_downvote_flowbar.current_mana + op.vesting_shares.amount.value / 4 );
    ...
    old_bob_flowbar = VOTING_FLOWBAR( "bob" );
    params.max_mana = util::get_effective_vesting_shares( db->get_account( "bob" ) );
    old_bob_flowbar.regenerate_mana( params, db->head_block_time() );

    old_bob_downvote_flowbar = DOWNVOTE_FLOWBAR( "bob" );
    params.max_mana = util::get_effective_vesting_shares( db->get_account( "bob" ) ) / 4;
    old_bob_downvote_flowbar.regenerate_mana( params, db->head_block_time() );
    ...
    idump( (alice_acc.voting_flowbar)(old_flowbar)(delta) );
    ...
    BOOST_REQUIRE( alice_acc.voting_flowbar.current_mana == old_flowbar.current_mana - delta );
    BOOST_REQUIRE( alice_acc.downvote_flowbar.current_mana == old_downvote_flowbar.current_mana - delta / 4 );
    BOOST_REQUIRE( bob_acc.get_received_vesting() == ASSET( "20000000.000000 VESTS"));
    BOOST_REQUIRE( bob_acc.voting_flowbar.current_mana == old_bob_flowbar.current_mana + delta );
    BOOST_REQUIRE( bob_acc.downvote_flowbar.current_mana == old_bob_downvote_flowbar.current_mana + delta / 4 );
    ...
    old_flowbar = VOTING_FLOWBAR( "bob" );
    params.max_mana = util::get_effective_vesting_shares( db->get_account( "bob" ) );
    old_flowbar.regenerate_mana( params, db->head_block_time() );
    ...
    BOOST_REQUIRE( alice_comment_cashout->net_rshares.value == old_flowbar.current_mana - db->get_account( "bob" ).voting_flowbar.current_mana - CREA_VOTE_DUST_THRESHOLD );
    BOOST_REQUIRE( itr->rshares == old_flowbar.current_mana - db->get_account( "bob" ).voting_flowbar.current_mana - CREA_VOTE_DUST_THRESHOLD );

    generate_block();
    ACTORS( (sam)(dave) )
    generate_block();

    vest( CREA_INIT_MINER_NAME, "sam", ASSET( "1000.000 TESTS" ) );
    ...
    tx.set_expiration( db->head_block_time() + CREA_MAX_TIME_UNTIL_EXPIRATION );
    sign( tx, sam_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      ...
      {
        a.voting_flowbar.current_mana = a.downvote_flowbar.current_mana * 3 / 4;
        a.voting_flowbar.last_update_time = db.head_block_time().sec_since_epoch();
      });
    });
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get_account( "sam" ), [&]( account_object& a )
      {
        a.voting_flowbar.current_mana = util::get_effective_vesting_shares( a );
        a.voting_flowbar.last_update_time = db.head_block_time().sec_since_epoch();
        a.downvote_flowbar.current_mana = a.downvote_flowbar.current_mana * 3 / 4;
        a.downvote_flowbar.last_update_time = db.head_block_time().sec_since_epoch();
      });
    });
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get_account( "sam" ), [&]( account_object& a )
      {
        a.downvote_flowbar.current_mana = util::get_effective_vesting_shares( a ) / 4;
        a.downvote_flowbar.last_update_time = db.head_block_time().sec_since_epoch();
      });
    });
    ...
    util::flowbar old_sam_flowbar = VOTING_FLOWBAR( "sam" );
    util::flowbar old_sam_downvote_flowbar = DOWNVOTE_FLOWBAR( "sam" );
    util::flowbar old_dave_flowbar = VOTING_FLOWBAR( "dave" );
    util::flowbar old_dave_downvote_flowbar = DOWNVOTE_FLOWBAR( "dave" );

    util::flowbar_params sam_params( util::get_effective_vesting_shares( db->get_account( "sam" ) ), CREA_VOTING_MANA_REGENERATION_SECONDS );
    util::flowbar_params dave_params( util::get_effective_vesting_shares( db->get_account( "dave" ) ), CREA_VOTING_MANA_REGENERATION_SECONDS );
    ...
    BOOST_REQUIRE( gpo.delegation_return_period == CREA_DELEGATION_RETURN_PERIOD_HF20 );
    ...
    old_sam_flowbar.regenerate_mana( sam_params, db->head_block_time() );
    sam_params.max_mana /= 4;
    old_sam_downvote_flowbar.regenerate_mana( sam_params, db->head_block_time() );

    old_dave_flowbar.regenerate_mana( dave_params, db->head_block_time() );
    dave_params.max_mana /= 4;
    old_dave_downvote_flowbar.regenerate_mana( dave_params, db->head_block_time() );

    BOOST_REQUIRE( VOTING_FLOWBAR( "sam" ).current_mana == old_sam_flowbar.current_mana );
    BOOST_REQUIRE( DOWNVOTE_FLOWBAR( "sam" ).current_mana == old_sam_downvote_flowbar.current_mana );
    BOOST_REQUIRE( VOTING_FLOWBAR( "dave" ).current_mana == old_dave_flowbar.current_mana - sam_vest.amount.value );
    BOOST_REQUIRE( DOWNVOTE_FLOWBAR( "dave" ).current_mana == old_dave_downvote_flowbar.current_mana - sam_vest.amount.value / 4 );

    old_sam_flowbar = VOTING_FLOWBAR( "sam" );
    old_sam_downvote_flowbar = DOWNVOTE_FLOWBAR( "sam" );
    old_dave_flowbar = VOTING_FLOWBAR( "dave" );
    old_dave_downvote_flowbar = DOWNVOTE_FLOWBAR( "dave" );
    ...
    generate_blocks( exp_obj->expiration + CREA_BLOCK_INTERVAL );
    ...
    BOOST_REQUIRE( VOTING_FLOWBAR( "sam" ).current_mana == old_sam_flowbar.current_mana + sam_vest.amount.value );
    BOOST_REQUIRE( DOWNVOTE_FLOWBAR( "sam" ).current_mana == old_sam_downvote_flowbar.current_mana + sam_vest.amount.value / 4 );
    BOOST_REQUIRE( VOTING_FLOWBAR( "dave" ).current_mana == old_dave_flowbar.current_mana );
    BOOST_REQUIRE( DOWNVOTE_FLOWBAR( "dave" ).current_mana == old_dave_downvote_flowbar.current_mana );
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_validate )
{
  try
  {
    ...
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), CREA_100_PERCENT + 1 ) );
    op.extensions.insert( b );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );

    BOOST_TEST_MESSAGE( "--- Testing more than 100% total weight" );
    b.beneficiaries.clear();
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), CREA_1_PERCENT * 75 ) );
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), CREA_1_PERCENT * 75 ) );
    op.extensions.clear();
    op.extensions.insert( b );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back( beneficiary_route_type( "bob", CREA_1_PERCENT * 2 ) );
    b.beneficiaries.push_back( beneficiary_route_type( "bob", CREA_1_PERCENT ) );
    op.extensions.clear();
    op.extensions.insert( b );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back( beneficiary_route_type( "bob", CREA_1_PERCENT ) );
    b.beneficiaries.push_back( beneficiary_route_type( "alice", CREA_1_PERCENT ) );
    op.extensions.clear();
    op.extensions.insert( b );
    CREA_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
  try
  {
    ...
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), CREA_1_PERCENT ) );

    for( size_t i = 0; i < 8; i++ )
    {
      b.beneficiaries.push_back( beneficiary_route_type( account_name_type( CREA_INIT_MINER_NAME + fc::to_string( i ) ), CREA_1_PERCENT ) );
    }
    ...
    tx.operations.push_back( op );
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx ), chain::plugin_exception );


    BOOST_TEST_MESSAGE( "--- Test specifying a non-existent benefactor" );
    b.beneficiaries.clear();
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "doug" ), CREA_1_PERCENT ) );
    op.extensions.clear();
    op.extensions.insert( b );
    tx.clear();
    tx.operations.push_back( op );
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "bob" ), 25 * CREA_1_PERCENT ) );
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( db->get_treasury_name() ), 10 * CREA_1_PERCENT ) );
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "sam" ), 50 * CREA_1_PERCENT ) );
    op.extensions.clear();
    op.extensions.insert( b );
    ...
    b.beneficiaries.clear();
    b.beneficiaries.push_back( beneficiary_route_type( account_name_type( "dave" ), 25 * CREA_1_PERCENT ) );
    op.extensions.clear();
    op.extensions.insert( b );
    sign( tx, alice_private_key );
    CREA_REQUIRE_THROW( db->push_transaction( tx ), fc::assert_exception );
    ...
    generate_blocks( db->find_comment_cashout( db->get_comment( "alice", string( "test" ) ) )->cashout_time - CREA_BLOCK_INTERVAL );

    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get_dynamic_global_properties(), [=]( dynamic_global_property_object& gpo )
      {
        gpo.current_supply -= gpo.get_total_reward_fund_crea();
        gpo.total_reward_fund_crea = ASSET( "100.000 TESTS" );
        gpo.current_supply += gpo.get_total_reward_fund_crea();
      });
    });
    ...
    //note: below we are mixing CREA and CBD but it works due to use of amount instead of whole asset plus the exchange rate is 1-1
    const comment_object& _comment = db->get_comment( "alice", string( "test" ) );
    const comment_cashout_object* _comment_cashout = db->find_comment_cashout( _comment );
    BOOST_REQUIRE( _comment_cashout == nullptr );

    BOOST_REQUIRE( ( get_cbd_rewards( "alice" ).amount + get_vest_rewards_as_crea( "alice" ).amount + db->get_treasury().get_cbd_balance().amount ) == get_vest_rewards_as_crea( "bob" ).amount + get_cbd_rewards( "bob" ).amount + 1 );
    BOOST_REQUIRE( ( get_cbd_rewards( "alice" ).amount + get_vest_rewards_as_crea( "alice" ).amount + db->get_treasury().get_cbd_balance().amount ) == ( get_vest_rewards_as_crea( "sam" ).amount + get_cbd_rewards( "sam" ).amount ) / 2 + 1 );
    BOOST_REQUIRE( get_vest_rewards_as_crea( "bob" ).amount == get_cbd_rewards( "bob" ).amount + 1 );
    BOOST_REQUIRE( get_vest_rewards_as_crea( "sam" ).amount == get_cbd_rewards( "sam" ).amount + 1 );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_validate )
{
  try
  {
    ...
    BOOST_TEST_MESSAGE( "--- failure when setting maximum_block_size below CREA_MIN_BLOCK_SIZE_LIMIT" );
    prop_op.props.erase( "account_creation_fee" );
    prop_op.props[ "maximum_block_size" ] = fc::raw::pack_to_vector( CREA_MIN_BLOCK_SIZE_LIMIT - 1 );
    CREA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
    ...
    //ABW: works also with outdated "sbd_interest_rate" instead of "cbd_interest_rate"
    CREA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

    BOOST_TEST_MESSAGE( "--- failure when setting cbd_interest_rate to CREA_100_PERCENT + 1" );
    prop_op.props.erase( "sbd_interest_rate" );
    prop_op.props[ "cbd_interest_rate" ] = fc::raw::pack_to_vector( CREA_100_PERCENT + 1 );
    CREA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );

    BOOST_TEST_MESSAGE( "--- failure when setting new cbd_exchange_rate with CBD / CREA" );
    ...
    //ABW: works also with outdated "sbd_exchange_rate" instead of "cbd_exchange_rate"
    CREA_REQUIRE_THROW( prop_op.validate(), fc::assert_exception );
    ...
    uint64_t unit = uint64_t(1) << CREA_RD_DECAY_DENOM_SHIFT;

    BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one year" );
    prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / CREA_BLOCKS_PER_YEAR ) );
    prop_op.validate();

    BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one day" );
    prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / CREA_BLOCKS_PER_DAY ) );
    prop_op.validate();

    BOOST_TEST_MESSAGE( "--- success when account subsidy decay is one hour" );
    prop_op.props[ "account_subsidy_decay" ] = fc::raw::pack_to_vector( uint32_t( unit / ((60*60)/CREA_BLOCK_INTERVAL) ) );
    prop_op.validate();
  }
  FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( witness_set_properties_authorities )
{
  try
  {
    ...
    expected_keys.push_back( authority( 1, CREA_NULL_ACCOUNT, 1 ) );
    op.get_required_authorities( key_auths );
    BOOST_REQUIRE( key_auths == expected_keys );
...
BOOST_AUTO_TEST_CASE( witness_set_properties_apply )
{
  try
  {
    ...
    sign( tx, signing_key );
    db->push_transaction( tx, 0 );
    BOOST_REQUIRE( alice_witness.props.maximum_block_size == CREA_MIN_BLOCK_SIZE_LIMIT + 1 );
    ...
    sign( tx, signing_key );
    db->push_transaction( tx, 0 );
    BOOST_REQUIRE( alice_witness.get_cbd_exchange_rate() == price( ASSET( "1.000 TBD" ), ASSET( "100.000 TESTS" ) ) );
    BOOST_REQUIRE( alice_witness.get_last_cbd_exchange_update() == db->head_block_time() );
    ...
    prop_op.props[ "key" ].clear();
    prop_op.props[ "key" ] = fc::raw::pack_to_vector( signing_key.get_public_key() );
    prop_op.props[ "account_subsidy_budget" ] = fc::raw::pack_to_vector( CREA_ACCOUNT_SUBSIDY_PRECISION );
    tx.clear();
    tx.operations.push_back( prop_op );
    sign( tx, signing_key );
    db->push_transaction( tx, 0 );
    BOOST_REQUIRE( alice_witness.props.account_subsidy_budget == CREA_ACCOUNT_SUBSIDY_PRECISION );

    BOOST_TEST_MESSAGE( "--- Testing setting account subsidy pool cap" );
    uint64_t day_decay = ( uint64_t(1) << CREA_RD_DECAY_DENOM_SHIFT ) / CREA_BLOCKS_PER_DAY;
...
BOOST_AUTO_TEST_CASE( claim_account_apply )
{
  try
  {
    ...
    generate_blocks( CREA_BLOCKS_PER_HOUR / 2 );
    ...
    BOOST_REQUIRE( get_balance( CREA_NULL_ACCOUNT ) == ASSET( "5.000 TESTS" ) );
    ...
    BOOST_CHECK_EQUAL( c_subs, prev_c_subs - CREA_ACCOUNT_SUBSIDY_PRECISION );
    BOOST_CHECK_EQUAL( nc_subs, prev_nc_subs );
    ...
    int64_t new_value = prev_c_subs - CREA_ACCOUNT_SUBSIDY_PRECISION;     // Usage applied before decay
    new_value = new_value
        + 5000                                                             // Budget
        - ((new_value*249617279) >> CREA_RD_DECAY_DENOM_SHIFT);           // Decay
...
BOOST_AUTO_TEST_CASE( create_claimed_account_apply )
{
  try
  {
    ...
    BOOST_REQUIRE( bob.get_balance().amount.value == ASSET( "0.000 TESTS" ).amount.value );
    BOOST_REQUIRE( bob.get_cbd_balance().amount.value == ASSET( "0.000 TBD" ).amount.value );
    ...
    db_plugin->debug_update( [=]( database& db )
    {
      db.modify( db.get_account( CREA_TEMP_ACCOUNT ), [&]( account_object& a )
      {
