...
def prepare_work_dir(work_dir_path, block_log_path, config_file_path):
  ...
  if os.path.exists(work_dir_path):
    if os.path.isdir(work_dir_path):
      ...
      paths_to_delete = [work_dir_path + "/p2p", work_dir_path + "/logs", blockchain_dir + "/block_log.index", blockchain_dir + "/shared_memory.bin"]
...
if __name__ == '__main__':
  ...
  try:
    ...
    parser.add_argument("reference_node_bin_path", help = "Path to reference cread executable.")
    parser.add_argument("test_node_bin_path", help = "Path to test cread executable.")
    ...
    parser.add_argument("--ref-node-is-steem", action = 'store_true', default = False, help = "Set this flag if reference node is steem node")
    ...
    if args.compare_as_text:
      ...
      for k, v in reference_data.items():
        ...
        if json_diff:
          logger.error("Differences detected in data with key {}: {}".format(k, dumps(json_diff, sort_keys=True, indent=4)))
          sys.exit(1)
