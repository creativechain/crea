...
def print_comment_rewards(node_client, accounts):
    ...
    table = PrettyTable()
    table.field_names = ['author','permlink','last_payout','cashout_time','total_vote_weight','reward_weight','total_payout_value','curator_payout_value','author_rewards','net_votes','percent_cbd','pending_payout_value','total_pending_payout_value']
    ret = []
...
if __name__ == '__main__':
    ...
    parser.add_argument("cread_path", help = "Path to cread executable.")
    parser.add_argument("replay_cread_path", help = "Path to cread executable performing replay.")

    parser.add_argument("--node-url", dest="node_url", default="http://127.0.0.1:8090", help="Url of working hive node")
    parser.add_argument("--working-dir", dest="cread_working_dir", default="/tmp/cread-data/", help = "Path to cread working directory")
    parser.add_argument("--config-path", dest="cread_config_path", default="../../crea_utils/resources/config.ini.in",help = "Path to source config.ini file")
    ...
    if args.cread_path:
        logger.info("Running cread via {} in {} with config {}".format(args.cread_path,
            args.cread_working_dir,
            args.cread_config_path)
        )
        
        node = crea_utils.crea_node.HiveNodeInScreen(
            args.cread_path,
            args.cread_working_dir,
            args.cread_config_path
        )
    ...
    if node is not None:
        node.run_crea_node(["--enable-stale-production"])
    try:
        if node is not None and node.is_running():
            ...
            w = Witness("initminer", crea_instance=node_client)
            w.feed_publish(1000.000, account="initminer")
            crea_utils.common.wait_n_blocks(node_client.rpc.url, 5)

            logger.info("Chain prefix is: {}".format(node_client.prefix))
            logger.info("Chain ID is: {}".format(node_client.get_config()["CREA_CHAIN_ID"]))
            ...
            for account in accounts[1:]:
                acc = Account(account["name"], crea_instance=node_client)
                node_client.vote(100., "@{}/{}".format("tester001", test_utils.get_post_permlink("tester001")),account=acc)
            crea_utils.common.wait_n_blocks(node_client.rpc.url, 10)
            ...
            crea_utils.debug_generate_blocks_until(node_client.rpc.url, wif, last_cashout_time, False)
            crea_utils.debug_generate_blocks(node_client.rpc.url, wif, 100)
        ...
        if args.replay_cread_path:
            logger.info("Replaying with cread via {} in {} with config {}".format(args.replay_cread_path,
                args.cread_working_dir,
                args.cread_working_dir + "/config.ini")
            )
            
            node = crea_utils.crea_node.HiveNodeInScreen(
                args.replay_cread_path,
                args.cread_working_dir,
                args.cread_working_dir + "/config.ini",
                True
            )
        if node is not None:
            node.run_crea_node(["--enable-stale-production","--replay-blockchain"])
        if node is not None and node.is_running():
            logger.info("Connecting to testnet...")
            node_client = Hive(node = [node_url], no_broadcast = False, 
                keys = keys
