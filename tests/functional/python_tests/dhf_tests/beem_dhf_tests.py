...
def test_create_proposal(node, creator_account, receiver_account, wif, subject):
    ...
    try:
        creator = Account(creator_account, crea_instance=s)
    except Exception as ex:
        logger.error("Account: {} not found. {}".format(creator_account, ex))
        raise ex
    
    try:
        receiver = Account(receiver_account, crea_instance=s)
    except Exception as ex:
...
def test_vote_proposal(node, account, wif, subject):
    ...
    crea_utils.common.wait_n_blocks(s.rpc.url, 2)
...
def test_iterate_results_test(node, creator_account, receiver_account, wif, subject, remove):
    ...
    for start_end_pair in start_end_pairs:
        ...
        try:
            ...
            logger.exception("Exception: {}".format(ex))
            raise ex
    crea_utils.common.wait_n_blocks(node, 5)
    ...
    if not remove:
        ...
        for _ in range(0, 2):
            ...
            try:
                ...
                logger.exception("Exception: {}".format(ex))
                raise ex
            crea_utils.common.wait_n_blocks(node, 3)
...
if __name__ == '__main__':
    ...
    try:
        if node is None or node.is_running():
            ...
            crea_utils.common.wait_n_blocks(node_url, 3)
        ...
        if args.junit_output is not None:
            test_suite = TestSuite('dhf_tests', crea_utils.common.junit_test_cases)
