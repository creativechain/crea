...
def create_proposals(node, accounts, start_date, end_date, wif=None):
    ...
    if wif is not None:
        crea_utils.debug_generate_blocks(node.rpc.url, wif, 5)
    else:
        crea_utils.common.wait_n_blocks(node.rpc.url, 5)
...
if __name__ == '__main__':
    ...
    try:
        if node is None or node.is_running():
            ...
            crea_utils.debug_generate_blocks(node_client.rpc.url, wif, 30 * 24 * 3600 / 3 + 10)
            ...
            crea_utils.common.debug_generate_blocks_until(node_client.rpc.url, wif, start_date_str, False)
            ...
            while current_date < end_date:
                ...
                crea_utils.common.debug_generate_blocks_until(node_client.rpc.url, wif, current_date_str, False)
            ...
            crea_utils.common.debug_generate_blocks_until(node_client.rpc.url, wif, end_date_blocks_str, False)
