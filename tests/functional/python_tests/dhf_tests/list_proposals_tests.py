...
from crea_utils.common import junit_test_case
...
def create_proposals(node_client, creator_account, receiver_account):
    ...
    start_date, end_date = crea_utils.common.get_isostr_start_end_date(now, 10, 2)

    from beem.account import Account
    try:
        creator = Account(creator_account, crea_instance=node_client)
    except Exception as ex:
        logger.error("Account: {} not found. {}".format(creator_account, ex))
        raise ex
    
    try:
        receiver = Account(receiver_account, crea_instance=node_client)
    except Exception as ex:
    ...
    for start_end_subject in START_END_SUBJECTS:
        start_date, end_date = crea_utils.common.get_isostr_start_end_date(now, start_end_subject[0], start_end_subject[1])
        op = Create_proposal(
            **{
        ...
        crea_utils.common.wait_n_blocks(node_client.rpc.url, 1)
    crea_utils.common.wait_n_blocks(node_client.rpc.url, 2)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.junit_output is not None:
            test_suite = TestSuite('list_proposals_test', crea_utils.common.junit_test_cases)
