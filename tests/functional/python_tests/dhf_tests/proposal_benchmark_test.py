...
#example A:
#"/home/a/cread" Path to cread exe
# "/home/a/data"  Path to blockchain directory
# "../../crea_utils/resources/config.ini.in" Path to config.ini.in - usually ./crea_utils/resources/config.ini.in
#  1 - number of proposals for every account
#  200 - number of accounts
#  200000.000- number of CREA's for every account
...
#  ./proposal_benchmark_test.py "/home/a/cread" "/home/a/data" "../../crea_utils/resources/config.ini.in" initminer 1 200 200000.000 30000.000 100000.000
...
#  ./proposal_benchmark_test.py "/home/a/cread" "/home/a/data" "../../crea_utils/resources/config.ini.in" initminer 2 300 200000.000 30000.000 100000.000
...
CREA_PROPOSAL_MAX_IDS_NUMBER = 5
...
def create_accounts(node, creator, accounts):
    for account in accounts:
        ...
        node.commit.create_account(account['name'], 
            ...
            creator=creator,
            asset='TESTS')
    crea_utils.common.wait_n_blocks(node.url, delayed_blocks)
...
def create_posts(node, accounts):
    ...
    for acnt in accounts:
        ...
        i += 1
        if ( i % data_size ) == 0:
            crea_utils.crea_tools.wait_for_blocks_produced(delayed_blocks_ex, node.url)
...
def generate_dates( node ):
    ...
    #because of CREA_PROPOSAL_MAINTENANCE_CLEANUP
...
def vote_proposals(node, ids, accounts ):
    ...
    proposal_limit = CREA_PROPOSAL_MAX_IDS_NUMBER
...
def get_block_time( node ):
    global_properties = crea_utils.crea_tools.get_dynamic_global_properties( node.url )
...
if __name__ == '__main__':
    ...
    parser.add_argument("path", type=str, help = "Path to cread")
    parser.add_argument("dir", type=str, help = "Path to blockchain directory")
    parser.add_argument("ini", type=str, help = "Path to config.ini.in - usually ./crea_utils/resources/config.ini.in")
    ...
    parser.add_argument("hives", type=str, default="200000.000", help = "CREA's")
    parser.add_argument("hbds", type=str, default="30000.000", help = "CBD's")
    parser.add_argument("vests", type=str, default="100000.000", help = "VEST's")

    args = parser.parse_args()

    node = crea_utils.crea_node.HiveNodeInScreen( args.path, args.dir, args.ini )
    ...
    try:
        if node.is_running():
            ...
            transfer_assets_to_accounts(node_client, args.creator, accounts, args.creas, args.hbds )
