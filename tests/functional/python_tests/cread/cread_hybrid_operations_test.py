...
def transfer_assets_to_accounts(node, from_account, accounts, amount, asset, wif=None):
    ...
    for acnt in accounts:
        ...
        acc = Account(from_account, crea_instance=node)
...
if __name__ == '__main__':
    ...
    try:
        if node is None or node.is_running():
            ...
            node_client.post("Hivepy proposal title [{}]".format(accounts[0]['name']), 
                "Hivepy proposal body [{}]".format(accounts[0]['name']), 
                accounts[0]['name'], 
            ...
            try:
                exit_code = 1
                tx = TransactionBuilder(crea_instance=node_client)
                ops = []
